...
const scriptsInEvents = {
	...
	{
		// Открываем группу строго в новой вкладке/окне. Никаких переходов поверх игры.
		// Определение площадки сделано только здесь, по клику, чтобы не трогать загрузку проекта.
		(() => {
		    const OK_URL = 'https://ok.ru/group/70000042375809';
		    const VK_URL = 'https://vk.com/public162959143';
		    ...
		    // URL и referrer обычно содержат реальные признаки VK/OK даже внутри iframe.
		    try { add(window.location.href); } catch (_) {}
		    try { add(window.location.search); } catch (_) {}
		    try { add(document.referrer); } catch (_) {}
		    try {
		        const origins = window.location.ancestorOrigins;
		        if (origins) {
		            for (let i = 0; i < origins.length; i++) add(origins[i]);
		        }
		    } catch (_) {}
		
		    // GamePush-платформа к моменту клика обычно уже инициализирована.
		    try {
		        const gpObj = (typeof gp !== 'undefined' && gp) ? gp : (window.gp || window.GamePush || window.GP);
		        if (gpObj && gpObj.platform) {
		            add(gpObj.platform.type);
		            add(gpObj.platform.tag);
		            add(gpObj.platform.name);
		            add(gpObj.platform.id);
		        }
		    } catch (_) {}
		
		    // CurrentPlatform используем только как дополнительную подсказку, не как единственный источник.
		    try { add(runtime.globalVars.CurrentPlatform); } catch (_) {}
		
		    const blob = parts.join(' ').toLowerCase();
		    const hasOK = blob.includes('ok.ru') || blob.includes('odnoklassniki') || blob.includes('odkl') || /(^|[^a-z0-9а-я])ok([^a-z0-9а-я]|$)/i.test(blob) || blob.includes('platform=ok') || blob.includes('gp_platform=ok');
		    const hasVK = blob.includes('vk.com') || blob.includes('vkontakte') || blob.includes('vk_client') || blob.includes('vk_app') || /(^|[^a-z0-9а-я])vk([^a-z0-9а-я]|$)/i.test(blob) || blob.includes('platform=vk') || blob.includes('gp_platform=vk');
		
		    // Важно: если площадка не определилась, НЕ уводим в VK. Это исправляет случай OK внутри CDN/iframe.
		    const url = hasVK && !hasOK ? VK_URL : OK_URL;
