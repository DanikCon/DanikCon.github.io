...
// Версия: GamePush v1.45.4 — локальное сохранение каждого изменения, облачная синхронизация раз в 15 секунд, соцмеханики VK/OK.
(async () => {
  "use strict";
  ...
  const platform = {
    ...
    async save(payload) {
      // The caller already writes locally before entering the cloud queue. Keep this extra write
      // as a safety net for direct adapter calls, but report cloud success/failure to the queue.
      localWrite(payload);
  ...
  async function bootstrap() {
    ...
    // The loaded cloud/local state is the baseline. Future comparisons ignore savedAt and only
    // send another request after the actual game/profile data changes.
    lastCloudSnapshot = payloadSignature(buildPayload());
