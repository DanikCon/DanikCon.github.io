...
// Public build marker for cache/debug verification.
globalThis.__SLITHER_BUILD__ = "29-fixed-no-scroll";
  Snake: King of the Arena — stable no-scroll responsive UI + OK payment recovery v0.35
  ---------------------------------------------------------------
...
class PlatformBridge {
  ...
  shouldShowSocialSticky() {
    // The host SDK owns the sticky placement. Do not close it merely because the
    // phone rotated; closing on resize caused the one-second flash in VK/OK.
    return this.platform === "vk" || this.platform === "ok";
  }
...
class SlitherArena {
  ...
  async start() {
    ...
    const handleViewportResize = () => {
      ...
      cancelAnimationFrame(viewportFitTimer);
      viewportFitTimer = requestAnimationFrame(() => this.fitOverlayPanel(true));
      // Social hosts resize the iframe in several steps while rotating/showing a banner.
      for (const delay of [120, 360, 900]) {
        globalThis.setTimeout?.(() => {
          this.resize();
          this.applyPlatformUi();
          this.fitOverlayPanel(delay === 120);
        }, delay);
      }
    };
  ...
  createDom() {
    ...
    this.root.innerHTML = `
      <style>
        ...
        /* v29 authoritative layout: no document/panel scrolling. Each screen owns its
           responsive geometry; a final uniform fit is only a safety net. */
        #slither-arena-root,
        #slither-arena-root .sa-shell,
        #slither-arena-root .sa-overlay { width:100%; height:100%; overflow:hidden !important; }
        #slither-arena-root { position:fixed !important; inset:0 !important; }
        ...
        #slither-arena-root.sa-sticky-safe .sa-overlay { padding-bottom:max(8px,env(safe-area-inset-bottom)) !important; }
        #slither-arena-root .sa-overlay > .sa-panel {
          --sa-fit-scale:1;
          ...
          max-height:none !important;
          overflow:hidden !important;
          transform:scale(var(--sa-fit-scale)) !important;
          transform-origin:center center !important;
          margin:0 !important;
          scrollbar-width:none !important;
        }
        #slither-arena-root .sa-overlay > .sa-panel::-webkit-scrollbar { display:none !important; }
        #slither-arena-root .sa-panel *,
        #slither-arena-root .sa-panel button { min-width:0; }
        #slither-arena-root .sa-panel button { touch-action:manipulation; }

        /* Home — compact portrait dashboard. */
        #slither-arena-root .sa-overlay > .sa-panel.sa-menu-panel { width:min(520px,calc(100vw - 16px)) !important; padding:16px !important; }
        #slither-arena-root .sa-menu-panel .sa-menu-hero { min-height:0 !important; }
        #slither-arena-root .sa-menu-panel .sa-menu-emblem { display:none !important; }
        #slither-arena-root .sa-menu-panel .sa-title { white-space:normal !important; font-size:clamp(30px,8vw,46px) !important; }
        #slither-arena-root .sa-menu-panel .sa-subtitle { margin:7px 0 0 !important; }
        #slither-arena-root .sa-menu-panel .sa-menu-progress { margin-top:10px !important; gap:8px !important; }
        #slither-arena-root .sa-menu-panel .sa-stat-row { display:grid !important; grid-template-columns:repeat(3,minmax(0,1fr)) !important; margin:9px 0 0 !important; gap:7px !important; }
        #slither-arena-root .sa-menu-panel .sa-stat { display:flex !important; flex-direction:column !important; min-height:48px !important; padding:7px 8px !important; }
        #slither-arena-root .sa-menu-panel .sa-stat small { white-space:nowrap !important; overflow:hidden !important; text-overflow:ellipsis !important; }
        #slither-arena-root .sa-menu-panel .sa-stat strong { white-space:nowrap !important; overflow:hidden !important; text-overflow:ellipsis !important; }
        #slither-arena-root .sa-menu-panel .sa-menu-actions { display:grid !important; grid-template-columns:repeat(2,minmax(0,1fr)) !important; grid-auto-rows:40px !important; gap:7px !important; margin-top:9px !important; }
        #slither-arena-root .sa-menu-panel .sa-menu-actions .sa-play { grid-column:1/-1 !important; grid-row:auto !important; }
        #slither-arena-root .sa-menu-panel .sa-menu-actions .sa-btn { min-height:40px !important; height:40px !important; margin:0 !important; padding:7px 9px !important; font-size:12px !important; line-height:1.05 !important; }

        /* Wardrobe — fixed grid; never scrolls. */
        #slither-arena-root .sa-overlay > .sa-panel.sa-shop-panel { width:min(620px,calc(100vw - 16px)) !important; padding:14px !important; }
        #slither-arena-root .sa-shop-panel .sa-title { font-size:32px !important; }
        #slither-arena-root .sa-shop-panel .sa-subtitle { margin:5px 0 8px !important; font-size:11px !important; }
        #slither-arena-root .sa-shop-grid { display:grid !important; grid-template-columns:repeat(2,minmax(0,1fr)) !important; gap:8px !important; overflow:hidden !important; }
        #slither-arena-root .sa-skin { min-height:126px !important; height:126px !important; padding:6px !important; overflow:hidden !important; }
        #slither-arena-root .sa-skin-preview-wrap,
        #slither-arena-root .sa-skin-preview { height:58px !important; min-height:58px !important; max-height:58px !important; }
        #slither-arena-root .sa-skin-name { white-space:nowrap !important; overflow:hidden !important; text-overflow:ellipsis !important; }
        #slither-arena-root .sa-skin-meta em { display:block !important; white-space:nowrap !important; overflow:hidden !important; text-overflow:ellipsis !important; }
        #slither-arena-root .sa-shop-nav { margin-top:7px !important; }
        #slither-arena-root .sa-shop-back { margin-top:7px !important; min-height:38px !important; padding:8px 12px !important; }

        /* Missions, controls, leaderboard. */
        #slither-arena-root .sa-overlay > .sa-panel.sa-missions-panel,
        #slither-arena-root .sa-overlay > .sa-panel.sa-controls-panel,
        #slither-arena-root .sa-overlay > .sa-panel.sa-leaderboard-panel { width:min(620px,calc(100vw - 16px)) !important; padding:15px !important; }
        #slither-arena-root .sa-missions-panel .sa-title,
        #slither-arena-root .sa-controls-panel .sa-title,
        #slither-arena-root .sa-leaderboard-panel .sa-title { font-size:32px !important; }
        #slither-arena-root .sa-missions-panel .sa-subtitle,
        #slither-arena-root .sa-controls-panel .sa-subtitle,
        #slither-arena-root .sa-leaderboard-panel .sa-subtitle { margin:5px 0 8px !important; font-size:11px !important; }
        #slither-arena-root .sa-mission-grid { overflow:hidden !important; }
        #slither-arena-root .sa-control-layout { display:grid !important; grid-template-columns:repeat(2,minmax(0,1fr)) !important; gap:9px !important; }
        #slither-arena-root .sa-control-card { min-height:170px !important; overflow:hidden !important; }
        #slither-arena-root .sa-leaderboard-list { display:grid !important; grid-template-columns:1fr !important; gap:5px !important; overflow:hidden !important; }
        #slither-arena-root .sa-leaderboard-row { min-height:30px !important; padding:6px 8px !important; }

        /* Short landscape host (VK/OK browser + banner): use width instead of shrinking. */
        @media (orientation:landscape) {
          #slither-arena-root .sa-overlay { padding:6px !important; }
          #slither-arena-root.sa-sticky-safe .sa-overlay { padding:6px !important; }

          #slither-arena-root .sa-overlay > .sa-panel.sa-menu-panel {
            width:min(1180px,calc(100vw - 16px)) !important;
            padding:12px 16px !important;
            display:grid !important;
            grid-template-columns:.82fr 1.12fr 1.28fr !important;
            grid-template-areas:"hero progress stats" "hero progress actions" !important;
            align-items:stretch !important;
            gap:8px 14px !important;
          }
          #slither-arena-root .sa-menu-panel .sa-menu-hero { grid-area:hero; display:block !important; align-self:center !important; }
          #slither-arena-root .sa-menu-panel .sa-menu-progress { grid-area:progress; margin:0 !important; align-self:center !important; }
          #slither-arena-root .sa-menu-panel .sa-stat-row { grid-area:stats; margin:0 !important; align-self:end !important; }
          #slither-arena-root .sa-menu-panel .sa-menu-actions { grid-area:actions; align-self:start !important; margin:0 !important; grid-template-columns:repeat(3,minmax(0,1fr)) !important; grid-auto-rows:36px !important; gap:6px !important; }
          #slither-arena-root .sa-menu-panel .sa-menu-actions .sa-play { grid-column:1/-1 !important; }
          #slither-arena-root .sa-menu-panel .sa-menu-actions .sa-btn { height:36px !important; min-height:36px !important; padding:5px 7px !important; font-size:10px !important; }
          #slither-arena-root .sa-menu-panel .sa-brand { margin-bottom:4px !important; font-size:9px !important; }
          #slither-arena-root .sa-menu-panel .sa-title { font-size:clamp(28px,4vw,46px) !important; line-height:.96 !important; }
          #slither-arena-root .sa-menu-panel .sa-subtitle { margin-top:7px !important; font-size:11px !important; line-height:1.25 !important; }
          #slither-arena-root .sa-menu-panel .sa-league-card { padding:8px 10px !important; gap:4px !important; }
          #slither-arena-root .sa-menu-panel .sa-mission-teaser { padding:7px 9px !important; }
          #slither-arena-root .sa-menu-panel .sa-stat { min-height:42px !important; padding:6px 7px !important; }
          #slither-arena-root .sa-menu-panel .sa-stat small { font-size:8px !important; }
          #slither-arena-root .sa-menu-panel .sa-stat strong { font-size:14px !important; }

          #slither-arena-root .sa-overlay > .sa-panel.sa-shop-panel { width:min(1080px,calc(100vw - 16px)) !important; padding:10px 14px !important; }
          #slither-arena-root .sa-shop-panel .sa-brand,
          #slither-arena-root .sa-shop-panel .sa-subtitle { display:none !important; }
          #slither-arena-root .sa-shop-panel .sa-title { font-size:27px !important; margin:0 0 7px !important; }
          #slither-arena-root .sa-shop-grid { grid-template-columns:repeat(4,minmax(0,1fr)) !important; gap:7px !important; }
          #slither-arena-root .sa-skin { min-height:116px !important; height:116px !important; }
          #slither-arena-root .sa-skin-preview-wrap,
          #slither-arena-root .sa-skin-preview { height:50px !important; min-height:50px !important; max-height:50px !important; }
          #slither-arena-root .sa-skin-meta em { display:none !important; }
          #slither-arena-root .sa-shop-nav { display:grid !important; grid-template-columns:44px 1fr 44px !important; height:34px !important; margin-top:6px !important; }
          #slither-arena-root .sa-shop-nav .sa-btn { min-height:34px !important; height:34px !important; padding:4px !important; margin:0 !important; }
          #slither-arena-root .sa-shop-back { min-height:34px !important; height:34px !important; margin-top:5px !important; padding:5px 10px !important; }

          #slither-arena-root .sa-missions-panel,
          #slither-arena-root .sa-controls-panel,
          #slither-arena-root .sa-leaderboard-panel { width:min(980px,calc(100vw - 16px)) !important; padding:10px 14px !important; }
          #slither-arena-root .sa-missions-panel .sa-brand,
          #slither-arena-root .sa-controls-panel .sa-brand,
          #slither-arena-root .sa-leaderboard-panel .sa-brand { display:none !important; }
          #slither-arena-root .sa-missions-panel .sa-title,
          #slither-arena-root .sa-controls-panel .sa-title,
          #slither-arena-root .sa-leaderboard-panel .sa-title { font-size:27px !important; margin:0 0 5px !important; }
          #slither-arena-root .sa-missions-panel .sa-subtitle,
          #slither-arena-root .sa-controls-panel .sa-subtitle,
          #slither-arena-root .sa-leaderboard-panel .sa-subtitle { display:none !important; }
          #slither-arena-root .sa-missions-panel .sa-mission-teaser { margin:5px 0 !important; padding:6px 9px !important; }
          #slither-arena-root .sa-missions-panel .sa-mission-grid { margin:6px 0 !important; }
          #slither-arena-root .sa-missions-panel .sa-mission-card { min-height:104px !important; padding:10px 12px 9px 42px !important; }
          #slither-arena-root .sa-missions-panel .sa-mission-actions { display:grid !important; grid-template-columns:1fr 1fr !important; gap:7px !important; }
          #slither-arena-root .sa-missions-panel .sa-mission-actions .sa-btn { min-height:34px !important; height:34px !important; margin:0 !important; padding:5px 9px !important; }
          #slither-arena-root .sa-mission-pager { margin:5px 0 !important; }

          #slither-arena-root .sa-controls-panel .sa-control-note { display:none !important; }
          #slither-arena-root .sa-controls-panel .sa-control-layout { grid-template-columns:repeat(2,minmax(0,1fr)) !important; gap:8px !important; }
          #slither-arena-root .sa-controls-panel .sa-control-card { min-height:142px !important; height:142px !important; padding:8px !important; }
          #slither-arena-root .sa-controls-panel > .sa-btn { min-height:34px !important; height:34px !important; margin-top:6px !important; padding:5px 10px !important; }

          #slither-arena-root .sa-leaderboard-list { grid-template-columns:repeat(2,minmax(0,1fr)) !important; gap:4px 7px !important; margin:5px 0 !important; }
          #slither-arena-root .sa-leaderboard-row { min-height:29px !important; height:29px !important; padding:4px 7px !important; }
          #slither-arena-root .sa-leaderboard-panel > .sa-btn { display:inline-block !important; width:calc(50% - 4px) !important; min-height:34px !important; height:34px !important; margin:5px 2px 0 !important; padding:5px 8px !important; }
          #slither-arena-root .sa-leaderboard-empty { min-height:95px !important; }

          #slither-arena-root .sa-overlay > .sa-panel.sa-gameover-panel,
          #slither-arena-root .sa-overlay > .sa-panel.sa-pause-panel { width:min(760px,calc(100vw - 16px)) !important; padding:12px 16px !important; }
        }
        ...
        @media (orientation:landscape) and (max-width:900px) {
          ...
          #slither-arena-root .sa-menu-panel .sa-subtitle { display:none !important; }
        }

        @media (orientation:landscape) and (max-height:330px) {
          #slither-arena-root .sa-menu-panel .sa-subtitle { display:none !important; }
          #slither-arena-root .sa-menu-panel .sa-mission-teaser { display:none !important; }
          #slither-arena-root .sa-menu-panel .sa-menu-actions { grid-auto-rows:32px !important; }
          #slither-arena-root .sa-menu-panel .sa-menu-actions .sa-btn { min-height:32px !important; height:32px !important; }
          #slither-arena-root .sa-control-card { min-height:124px !important; height:124px !important; }
        }
    ...
    const preventGameTouch = (event) => {
      ...
      // Buttons still receive taps; blocking the browser gesture only prevents the
      // embedded page/body from panning or rubber-banding.
      if (event.type === "touchmove" || this.mode === "game") event.preventDefault?.();
    };
  ...
  fitOverlayPanel(renderPreviews = false) {
    ...
    const padX = (Number.parseFloat(overlayStyle?.paddingLeft) || 0) + (Number.parseFloat(overlayStyle?.paddingRight) || 0);
    const padY = (Number.parseFloat(overlayStyle?.paddingTop) || 0) + (Number.parseFloat(overlayStyle?.paddingBottom) || 0);
    const availableWidth = Math.max(1, overlay.clientWidth - padX - 2);
    const availableHeight = Math.max(1, overlay.clientHeight - padY - 2);
    const rawWidth = Math.max(panel.offsetWidth, 1);
    const rawHeight = Math.max(panel.offsetHeight, 1);
    const scale = clamp(Math.min(1, availableWidth / rawWidth, availableHeight / rawHeight), .35, 1);

    panel.style.setProperty("--sa-fit-scale", scale.toFixed(4));
    panel.classList.toggle("sa-auto-fit", scale < .995);
  }
