/*
  Snake: King of the Arena — Construct 3 JS runtime (ads/reward/leaderboard/OK payment return/recovery fixes v0.35 dashboard-banner-only)
  ---------------------------------------------------------------
...
const CFG = Object.freeze({
  ...
  BANNER_MODE: "dashboard-only", // Sticky is never controlled by game code; platform/GamePush panel owns it
...
class PlatformBridge {
  ...
  shouldShowSocialSticky() {
    // VK/OK sticky should stay visible across rotation as long as the host viewport
    // is large enough to keep the game usable. The UI now fits itself to the visual
    // viewport, so a portrait-only rule is no longer necessary and caused the banner
    // to disappear immediately after rotating the phone.
  ...
  async refreshBannerForViewport() {
    // Dashboard-only mode: do not show, refresh or close sticky banners from the game.
    this.syncBannerSafeArea(false);
    return false;
  }

  async setBannerVisible(visible) {
    // Keep the UI calls harmless. Banner permissions and any automatic embedded banner
    // are controlled by the GamePush/platform panel, not by this exported game build.
...
class SlitherArena {
  ...
  fitOverlayPanel(renderPreviews = false) {
    ...
    const reserve = 12;
    // getBoundingClientRect includes overlay padding. Subtract it explicitly: on VK/OK
    // the sticky-safe bottom padding is the area that must remain free of controls.
