    ...
    if (globalThis.portalTimer <= 0) {
        ...
        if (!document.getElementById('victory-screen')) {
            ...
            if (currentNum >= TOTAL_LEVELS) {
                ...
                // 1. УМНЫЙ ДЕТЕКТОР ПЛАТФОРМЫ
                let gpPlatform = 'NONE';

                // Сначала вежливо спрашиваем GamePush
                if (typeof gp !== 'undefined' && gp.platform) {
                    gpPlatform = gp.platform.type;
                } else if (typeof gamepush !== 'undefined' && gamepush.platform) {
                    gpPlatform = gamepush.platform.type;
                }

                // Запасной план: читаем URL. ВК всегда добавляет viewer_id
                if (window.location.search.includes('viewer_id') || window.location.search.includes('api_url')) {
                    gpPlatform = 'VK';
                }
                // ОК всегда добавляет logged_user_id
                if (window.location.search.includes('logged_user_id') || window.location.search.includes('session_key')) {
                    gpPlatform = 'OK';
                }

                console.log("🌍 Платформа определена как:", gpPlatform);
                ...
                if (gpPlatform === 'VK' || gpPlatform === 'OK') {
                    ...
                    document.getElementById('btn-invite').onclick = () => {
                        ...
                        else if (typeof gamepush !== 'undefined') gamepush.social.invite();
                        else console.warn("GamePush не готов для инвайта!");
                    };
                    ...
                    document.getElementById('btn-subscribe').onclick = () => {
                        ...
                        else if (typeof gamepush !== 'undefined') gamepush.social.joinCommunity({ groupId: myGroupId });
                        else console.warn("GamePush не готов для подписки!");
                    };
                }
