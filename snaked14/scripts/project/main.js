...
class SlitherArena {
  ...
  syncRootViewportBox() {
    ...
    const width = Math.max(1, Math.round(viewport?.width ?? layoutWidth));
    ...
    const occludedRight = Math.max(0, layoutWidth - width - offsetLeft);
    const occludedBottom = Math.max(0, layoutHeight - height - offsetTop);

    this.root.style.left = `${offsetLeft}px`;
    this.root.style.top = `${offsetTop}px`;
    this.root.style.width = `${width}px`;
    this.root.style.height = `${height}px`;
    this.root.style.setProperty("--sa-vv-width", `${width}px`);
    this.root.style.setProperty("--sa-vv-height", `${height}px`);
    this.root.style.setProperty("--sa-vv-offset-left", `${offsetLeft}px`);
    this.root.style.setProperty("--sa-vv-offset-top", `${offsetTop}px`);
