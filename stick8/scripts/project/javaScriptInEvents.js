            ...
            if (currentNum >= TOTAL_LEVELS) {
                ...
                if (socialContainer) {
                   ...
                   // 🔥 Отправляем чистый сигнал в Event Sheet!
                    ...
                    // 🔥 Отправляем сигнал Инвайта (он работает нормально)
                    document.getElementById('btn-invite').onclick = () => {
                        console.log("📲 Передаю сигнал Инвайта в Construct 3...");
                        runtime.callFunction("GamePush_Invite"); 
                    };

                    // 🔥 Отправляем готовую ссылку для Подписки
                    document.getElementById('btn-subscribe').onclick = () => {
                        ...
                        const groupUrl = gpPlatform === 'VK' 
                            ? 'https://vk.com/club162959143' 
                            : 'https://ok.ru/group/70000042375809';
                            
                        // Передаем ссылку как параметр в Конструкт
