/*
  Snake: King of the Arena — Construct 3 JS runtime (release payments/recovery v0.37)
  ---------------------------------------------------------------
...
const I18N = Object.freeze({
  ...
  en: {
    ...
    home: { brand: "HYPERGRID // SECTOR 09", title: "SNAKE // HYPERDRIVE", subtitle: "Collect energy. Amplify your signal. Survive the digital grid.", daily: "Daily goal • +{reward} ★", complete: "complete", missions: "Daily tasks • {done}/{total}", missionsHint: "Finish all — streak and +{stars} ★", dayStreak: "day streak", record: "Record", skins: "Skins", league: "League", play: "Play", tasks: "Tasks", wardrobe: "Wardrobe", rating: "Leaderboard", controls: "Controls: {mode}", touch: "Touch", joystick: "Joystick", invite: "Invite a friend", joinVk: "Follow VK", joinOk: "Follow OK" },
...
function sanitizeProgress(raw) {
  ...
  return {
    ...
    purchaseSkins: Array.isArray(raw.purchaseSkins) ? [...new Set(raw.purchaseSkins.filter((id) => allowed.has(id)))] : [],
...
class PlatformBridge {
  constructor(runtime) {
    ...
    this.initialVkBannerRequested = false;
    this.initialized = false;
    this.initializing = null;
    this.products = [];
    this.productByTag = new Map();
    this.purchasesReady = false;
    this.purchaseTransactions = new Set();
    this.lastPurchaseRestoreChanged = false;
  ...
  get supportsPurchases() {
    ...
    if (this.isPlaygama) return false;
    if (typeof this.adapter?.purchaseSkin === "function") return true;
    if (!this.isGamePushPlatform) return CFG.DEBUG_FAKE_PURCHASES;
    const payments = (this.gp ?? this.getGamePush())?.payments;
    if (!payments || typeof payments.purchase !== "function") return false;
    // Current GamePush SDKs expose isAvailable. Undefined is accepted only for
    // compatibility with older Construct addon builds that still implement purchase().
    return payments.isAvailable !== false;
  }
  ...
  getProductForSkin(skin = null) {
    ...
    const cached = this.productByTag?.get?.(skin.sku) ?? this.productByTag?.get?.(skin.id) ?? null;
    if (cached) return cached;
    const payments = (this.gp ?? this.getGamePush())?.payments;
    if (typeof payments?.getProduct === "function") {
      try {
        const product = payments.getProduct(skin.sku);
        if (product) {
          this.registerProducts([product]);
          return product;
        }
      } catch (_error) {
        // Product can be absent in Preview or while the platform catalog is loading.
      }
    }
    return null;
  }
  ...
  getProductCurrencyLabel(product = null) {
    const candidates = [
      product?.currencySymbol,
      product?.currency_symbol,
      product?.currency,
      product?.priceCurrencyCode,
      product?.platformData?.currencySymbol,
      product?.platformData?.currency,
  ...
  getPriceLabel(skin = null) {
    // Read the real product price from the platform catalog. Hardcoded values are
    // used only as a last-resort fallback when Preview has no payment catalog.
    const product = this.getProductForSkin(skin);
    if (this.platform === "yandex") {
      ...
      if (priceValue !== undefined && priceValue !== null && String(priceValue).trim()) return String(priceValue).trim();
    ...
    const rawPrice = product?.priceValue ?? product?.price_value ?? product?.price ?? product?.product?.price ?? product?.platformData?.price;
    const price = String(rawPrice ?? "").trim();
    const currency = this.getProductCurrencyLabel(product);
    if (price) {
      if (!currency || price.toLocaleLowerCase().includes(currency.toLocaleLowerCase())) return price;
      return `${price} ${currency}`.trim();
    }
    if (this.platform === "vk") return PLATFORM_SDK.STORE_PRICE_LABELS.vk;
    if (this.platform === "ok") return PLATFORM_SDK.STORE_PRICE_LABELS.ok;
  ...
  async _initialize() {
    ...
    if (this.gp && this.isGamePushPlatform) {
      ...
      // The built-in VK sticky configured in the GamePush dashboard is requested
      // exactly once after the SDK/session is ready. No later screen or resize event
      // issues another banner command, and OK/Yandex receive no sticky call here.
      await this.showInitialVkBanner();
      try {
        // Current SDKs preload products and purchases during initialization. Register
        // that list first, then refresh for older addon builds and late purchases.
        this.registerProducts(this.gp.payments?.products);
  ...
  hasPermanentPurchase(payments, skin) {
    if (!payments || !skin?.sku) return false;
    try {
      if (typeof payments.has === "function" && payments.has(skin.sku) === true) return true;
    } catch (_error) {
      // Fall through to ID/list compatibility checks.
    }
    const product = this.getProductForSkin(skin);
    try {
      if (product?.id != null && typeof payments.has === "function" && payments.has(product.id) === true) return true;
    } catch (_error) {
      // Some older wrappers only accept the tag.
    }
    const purchases = Array.isArray(payments.purchases) ? payments.purchases : [];
    const productIds = new Set(this.getProductKeys(product));
    productIds.add(String(skin.sku));
    return purchases.some((purchase) => {
      const keys = this.getProductKeys(purchase);
      keys.push(String(purchase?.productId ?? purchase?.productID ?? ""));
      return keys.some((key) => productIds.has(String(key)));
    });
  }

  async reconcilePermanentPurchases(progress) {
    const clean = sanitizeProgress(progress);
    this.lastPurchaseRestoreChanged = false;
    if (!this.isGamePushPlatform) return clean;
    const payments = (this.gp ?? this.getGamePush())?.payments;
    if (!payments) return clean;

    try {
      this.registerProducts(payments.products);
      // Refresh both catalog and player purchase list when the SDK supports it.
      if (typeof payments.fetchProducts === "function") {
        const fetched = await payments.fetchProducts();
        this.registerProducts(fetched);
      }
    } catch (error) {
      console.warn("[SlitherArena] Purchase restore refresh failed; cached purchases are checked.", error);
    }

    const unlocked = new Set(clean.unlockedSkins);
    const purchased = new Set(clean.purchaseSkins);
    for (const skin of SKINS) {
      if (skin.kind !== "purchase" || !skin.sku) continue;
      if (!this.hasPermanentPurchase(payments, skin)) continue;
      unlocked.add(skin.id);
      purchased.add(skin.id);
    }

    const reconciled = sanitizeProgress({
      ...clean,
      unlockedSkins: [...unlocked],
      purchaseSkins: [...purchased]
    });
    this.lastPurchaseRestoreChanged = reconciled.unlockedSkins.length !== clean.unlockedSkins.length
      || reconciled.purchaseSkins.length !== clean.purchaseSkins.length;
    if (this.lastPurchaseRestoreChanged) {
      this.writeLocal(reconciled);
      console.info("[SlitherArena] Permanent purchases restored from GamePush.", reconciled.purchaseSkins);
    }
    return reconciled;
  }
  ...
  async loadProgress() {
    ...
    try {
      if (this.adapter?.loadProgress) {
        loaded = (await this.adapter.loadProgress()) ?? local;
      } else if (this.isPlaygama) {
        loaded = (await this.loadPlaygamaProgress()) ?? local;
      } else if (this.gp?.player?.get) {
        ...
        if (saved) loaded = saved;
          ...
          if (mirrored) loaded = { ...(local ?? {}), ...mirrored };
        }
      }
    } catch (error) {
      console.warn("[SlitherArena] Cloud progress load failed; local progress is used.", error);
    }
    const clean = sanitizeProgress(loaded);
    return await this.reconcilePermanentPurchases(clean);
  }
  ...
  async showGamePushRewarded(gp) {
    const ads = gp?.ads;
    // GamePush documents showRewardedVideo(); keep showRewarded only as an alias
    // for older addon builds.
    ...
    return await new Promise((resolve) => {
      ...
      let timeoutId = 0;
      let closeGraceId = 0;
      let rewardFallbackId = 0;
      // Only documented rewarded-specific events participate in this transaction.
      // In particular, rewarded:finish is deliberately excluded: some social
      // wrappers emit it when the placement merely ends, including an early close.
      const subscriptions = [
      ...
      const cleanup = () => {
        ...
        if (rewardFallbackId) clearTimeout(rewardFallbackId);
      };
      const finish = (outcome) => {
        if (settled) return;
        settled = true;
      ...
      function onReward() {
        rewardReceived = true;
        // Do not resume gameplay beneath a still-open native ad. Normally close follows
        // immediately. The fallback only handles wrappers that omit rewarded:close.
        if (closeSeen) {
          finish("completed");
          return;
        }
        if (rewardFallbackId) clearTimeout(rewardFallbackId);
        rewardFallbackId = setTimeout(() => {
          if (!settled && rewardReceived) finish("completed");
        }, 2200);
      }
      function onClose(_success) {
        closeSeen = true;
        // VK/OK are strict: a close signal or a truthy promise never grants a reward.
        // Give the SDK a short grace period because some builds dispatch reward just
        // after close; without the actual rewarded:reward event the attempt is denied.
        if (closeGraceId) clearTimeout(closeGraceId);
        closeGraceId = setTimeout(() => {
          if (!settled) finish(rewardReceived ? "completed" : "closed");
        }, strictCompletion ? 700 : 250);
      }
      ...
      try {
        for (const [event, handler] of subscriptions) {
          try { ads.on?.(event, handler); } catch (_error) { /* optional event */ }
        }
        const result = show.call(ads, { showFailedOverlay: true });
        Promise.resolve(result).then((value) => {
          if (settled) return;
          const explicitReward = value?.rewarded === true
            || value?.state === "rewarded"
            || value?.type === "rewarded:reward";
          const inferred = this.inferRewardOutcome(value, "");

          if (strictCompletion) {
            // The official event is the source of truth on VK/OK. A bare true/success,
            // state=completed, or close(success=true) can describe the ad window rather
            // than a fully watched video in some embedded providers.
            if (value === false || inferred === "closed") onClose(false);
            return;
          }

          // Other GamePush platforms may use the documented promise result when their
          // wrapper does not expose events, while still preferring rewarded:reward.
          if (value === true || value?.success === true || explicitReward) {
            rewardReceived = true;
            if (closeSeen) finish("completed");
            else {
              if (rewardFallbackId) clearTimeout(rewardFallbackId);
              rewardFallbackId = setTimeout(() => {
                if (!settled && rewardReceived) finish("completed");
          ...
          } else if (inferred === "closed") {
            onClose(false);
          } else if (value !== undefined && value !== null) {
            setTimeout(() => {
              if (!settled && !rewardReceived && !closeSeen) finish("failed");
            }, 500);
          }
      ...
      timeoutId = setTimeout(() => finish(rewardReceived ? "completed" : "closed"), 90000);
    });
  }
  ...
  async showInitialVkBanner() {
    if (this.captureMode || isCaptureMode() || this.initialVkBannerRequested || this.platform !== "vk") return false;
    this.initialVkBannerRequested = true;
    try {
      const gp = this.gp ?? this.getGamePush();
      if (typeof gp?.ads?.showSticky !== "function") return false;
      await gp.ads.showSticky();
      // Keep menu panels above the persistent bottom sticky without issuing any
      // additional show/hide calls while the player navigates or starts a match.
      globalThis.document?.getElementById?.("slither-arena-root")?.classList?.add?.("sa-sticky-safe");
      const refreshViewport = () => {
        try { globalThis.dispatchEvent?.(new Event("slither-host-viewport-change")); } catch (_error) { /* no-op */ }
      };
      try { gp.ads.on?.("sticky:render", refreshViewport); } catch (_error) { /* older SDK wrapper */ }
      refreshViewport();
      setTimeout(refreshViewport, 250);
      setTimeout(refreshViewport, 700);
      return true;
    } catch (error) {
      console.warn("[SlitherArena] Initial VK banner was not shown.", error);
      return false;
    }
  }
  ...
  async purchaseSkin(sku) {
    await this.initialize();
    const tag = String(sku ?? "").trim();
    if (!tag || !this.supportsPurchases || this.purchaseTransactions.has(tag)) return false;
    this.purchaseTransactions.add(tag);
    try {
      if (this.adapter?.purchaseSkin) return (await this.adapter.purchaseSkin(tag)) === true;
      const gp = this.gp ?? this.getGamePush();
      const payments = gp?.payments;
      if (payments?.isAvailable === false || typeof payments?.purchase !== "function") return false;

      // GamePush resolves purchase() only after the platform confirms payment; cancellation,
      // insufficient funds and product errors reject and are handled by catch below.
      await payments.purchase({ tag });

      // Refresh the permanent-purchase cache so a later progress reload or device switch
      // restores the skin even if the normal cloud save is interrupted immediately after pay.
      try {
        const fetched = await payments.fetchProducts?.();
        this.registerProducts(fetched);
      } catch (_error) {
        // The fulfilled purchase itself is the success signal; refresh is best effort.
      }
      return true;
    } catch (error) {
      console.warn("[SlitherArena] Purchase failed or was cancelled.", error);
      return false;
    } finally {
      this.purchaseTransactions.delete(tag);
    }
  }
...
class SlitherArena {
  ...
  async start() {
    ...
    const scheduleViewportRefresh = () => {
      ...
      // VK/OK mobile WebViews and the sticky host area often settle their final
      // dimensions a few frames after rotation or banner rendering.
      for (const delay of [60, 180, 420, 800]) {
        setTimeout(handleViewportResize, delay);
      }
    };
    globalThis.addEventListener?.("resize", scheduleViewportRefresh);
    globalThis.addEventListener?.("orientationchange", scheduleViewportRefresh);
    globalThis.addEventListener?.("slither-host-viewport-change", scheduleViewportRefresh);
    globalThis.visualViewport?.addEventListener?.("resize", scheduleViewportRefresh);
    globalThis.visualViewport?.addEventListener?.("scroll", handleViewportResize);
    this.runtime?.addEventListener?.("resize", scheduleViewportRefresh);
    ...
    void (async () => {
      ...
      // Mirror restored permanent purchases into the normal cloud progress. The
      // platform purchase record remains the source of truth, so this is only a cache.
      if (this.bridge.lastPurchaseRestoreChanged && !this.bridge.isPlaygama) {
  ...
  createDom() {
    ...
    this.root.innerHTML = `
      <style>
        ...
        /* v0.35: VK/OK mobile WebViews can leave a very short iframe after their
           browser chrome and sticky banner are added. Every overlay remains fully
           visible: the complete card scales as one unit and never becomes scrollable. */
        #slither-arena-root .sa-overlay,
        #slither-arena-root .sa-overlay > .sa-panel {
          touch-action:none !important;
          overscroll-behavior:none !important;
        }

        /* GamePush/VK places the sticky in a separate host area and reduces the iframe.
           Do not reserve a second fake banner gap inside the already reduced game frame. */
        #slither-arena-root.sa-sticky-safe .sa-overlay {
        ...
        #slither-arena-root.sa-sticky-safe .sa-panel {
          max-height:none !important;
        }

        /* Wide phones in landscape: turn the home card into a readable two-column
           dashboard. VK and OK often leave only 280-420 CSS px of game height here. */
        #slither-arena-root.sa-touch-landscape .sa-menu-panel {
        ...
        #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-menu-hero {
          grid-area:hero;
          display:block !important;
          min-width:0;
        }
        #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-menu-copy {
          display:block !important;
        }
        #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-menu-emblem {
          display:none !important;
        }
        #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-brand {
          margin:0 0 2px !important;
          font-size:8px !important;
        }
        #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-title {
          margin:0 !important;
          font-size:clamp(24px,4.2vw,32px) !important;
          line-height:.96 !important;
          white-space:normal !important;
        }
        #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-subtitle {
          display:none !important;
        }
        #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-menu-progress {
          grid-area:progress;
          display:grid !important;
          gap:5px !important;
          margin:0 !important;
        ...
        #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-league-card {
          padding:7px 8px !important;
          gap:3px !important;
          border-radius:12px !important;
        }
        #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-league-kicker {
          display:none !important;
        }
        #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-league-title {
          font-size:11px !important;
          line-height:1.12 !important;
        }
        #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-league-row {
          font-size:8px !important;
          line-height:1.1 !important;
        }
        #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-progress-bar {
          height:6px !important;
        }
        #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-mission-teaser {
          display:flex !important;
          min-height:0 !important;
          margin:0 !important;
        ...
        #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-mission-teaser strong {
          font-size:10px !important;
        }
        #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-mission-teaser span {
          margin-top:1px !important;
          font-size:8px !important;
        }
        #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-mission-mini {
          min-width:50px !important;
          padding:4px 5px !important;
          font-size:8px !important;
        }
        #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-stat-row {
          grid-area:stats;
          display:grid !important;
          grid-template-columns:repeat(3,minmax(0,1fr)) !important;
          gap:5px !important;
        ...
        #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-stat {
          min-height:42px !important;
          padding:6px !important;
          border-radius:11px !important;
        }
        #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-stat small {
          font-size:7px !important;
          line-height:1.05 !important;
        }
        #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-stat strong {
        ...
        #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-menu-actions {
          grid-area:actions;
          display:grid !important;
          grid-template-columns:repeat(3,minmax(0,1fr)) !important;
        ...
        #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-menu-actions .sa-play {
          grid-column:span 3 !important;
          grid-row:auto !important;
        }
        #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-menu-actions .sa-controls-btn {
          grid-column:auto !important;
        }
        #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-menu-actions .sa-btn {
          min-width:0 !important;
        ...
        #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-social-btn {
          gap:4px !important;
        }
        #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-social-btn svg,
        #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-rating-btn svg {
          width:14px !important;
          height:14px !important;
          flex:0 0 14px;
        }
        #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-social-label {
          min-width:0;
          overflow:hidden;
          text-overflow:ellipsis;
          white-space:nowrap;
        ...
        /* v0.36: control cards keep a stable internal grid after whole-panel scaling.
           Generic span rules previously added margins to the preview and check badge,
           which made the selected mark and illustrations look slightly displaced. */
        #slither-arena-root .sa-controls-panel {
          text-align:left !important;
        }
        #slither-arena-root .sa-controls-panel .sa-control-layout {
          width:100% !important;
          align-items:stretch !important;
          justify-items:stretch !important;
        }
        #slither-arena-root .sa-controls-panel .sa-control-card {
        ...
        #slither-arena-root .sa-controls-panel .sa-control-demo {
          box-sizing:border-box !important;
          width:100% !important;
          margin:0 !important;
          align-self:start !important;
        }
        #slither-arena-root .sa-controls-panel .sa-control-check {
          box-sizing:border-box !important;
          margin:0 !important;
          line-height:1 !important;
        }
        #slither-arena-root .sa-controls-panel .sa-control-card > strong {
          width:100% !important;
          margin:7px 0 0 !important;
          line-height:1.12 !important;
        }
        #slither-arena-root .sa-controls-panel .sa-control-description {
          width:100% !important;
          margin:4px 0 0 !important;
          align-self:start !important;
        }
        #slither-arena-root .sa-controls-panel > .sa-btn {
          box-sizing:border-box !important;
          width:100% !important;
        }
        @media (pointer:coarse) and (orientation:landscape) {
        ...
        @media (pointer:coarse) and (orientation:portrait) and (max-width:440px) {
          #slither-arena-root .sa-controls-panel .sa-control-layout {
        ...
        /* v0.50 — HYPERGRID visual identity. This layer intentionally overrides the
           earlier glass-card styling without touching gameplay or platform integrations. */
        #slither-arena-root {
          --hg-ink:#f7f4ff;
          --hg-muted:#a8a2be;
          --hg-pink:#ff46c4;
          ...
          font-family:"Arial Narrow","Roboto Condensed",Inter,system-ui,sans-serif !important;
          color:var(--hg-ink) !important;
        }
        #slither-arena-root .sa-shell {
          background:
            radial-gradient(circle at 14% 12%,rgba(255,70,196,.14),transparent 27%),
            radial-gradient(circle at 88% 18%,rgba(0,246,219,.09),transparent 25%),
            linear-gradient(155deg,#180520 0%,#0a0c1c 48%,#050710 100%) !important;
        }
        #slither-arena-root .sa-overlay {
          background:
            linear-gradient(90deg,rgba(255,70,196,.035) 1px,transparent 1px),
            linear-gradient(rgba(0,246,219,.025) 1px,transparent 1px),
            radial-gradient(circle at 50% 40%,rgba(19,20,48,.42),rgba(3,4,10,.82) 72%) !important;
          background-size:34px 34px,34px 34px,auto !important;
        }
        #slither-arena-root .sa-panel {
          border:1px solid rgba(255,255,255,.12) !important;
          border-radius:4px !important;
          background:linear-gradient(145deg,rgba(17,18,38,.985),rgba(7,8,19,.99)) !important;
          box-shadow:18px 18px 0 rgba(0,0,0,.18),0 28px 90px rgba(0,0,0,.58) !important;
          clip-path:polygon(0 18px,18px 0,100% 0,100% calc(100% - 24px),calc(100% - 24px) 100%,0 100%) !important;
        }
        #slither-arena-root .sa-panel::before {
          width:150px !important;height:5px !important;left:22px !important;right:auto !important;top:0 !important;
          border-radius:0 !important;background:linear-gradient(90deg,var(--hg-pink),var(--hg-violet),var(--hg-cyan)) !important;
          box-shadow:0 0 22px rgba(255,70,196,.38) !important;
        }
        #slither-arena-root .sa-brand {
          display:flex !important;align-items:center;gap:8px;color:var(--hg-cyan) !important;
          font-family:ui-monospace,SFMono-Regular,Menlo,monospace !important;font-size:10px !important;
          letter-spacing:.19em !important;text-shadow:0 0 14px rgba(0,246,219,.24) !important;
        }
        #slither-arena-root .sa-brand::before {content:"●";color:var(--hg-pink);font-size:8px;animation:hg-pulse 1.2s steps(2,end) infinite;}
        @keyframes hg-pulse {50%{opacity:.25}}
        #slither-arena-root .sa-title {
          font-family:"Arial Black","Roboto Condensed",Impact,sans-serif !important;
          font-weight:950 !important;letter-spacing:-.055em !important;text-transform:uppercase !important;
          text-shadow:3px 3px 0 rgba(255,70,196,.18) !important;
        }
        #slither-arena-root .sa-subtitle {color:#aaa6bd !important;font-family:ui-monospace,SFMono-Regular,Menlo,monospace !important;}
        #slither-arena-root .sa-btn,
        #slither-arena-root .sa-icon-btn {
          border-radius:3px !important;border:1px solid rgba(255,255,255,.13) !important;
          text-transform:uppercase !important;letter-spacing:.055em !important;
          font-family:"Arial Narrow","Roboto Condensed",system-ui,sans-serif !important;font-weight:900 !important;
          clip-path:polygon(0 0,calc(100% - 10px) 0,100% 10px,100% 100%,10px 100%,0 calc(100% - 10px)) !important;
        }
        #slither-arena-root .sa-btn {
          background:linear-gradient(100deg,var(--hg-pink),#b542ff 52%,var(--hg-violet)) !important;
          box-shadow:none !important;
        }
        #slither-arena-root .sa-btn.secondary,
        #slither-arena-root .sa-icon-btn {
          background:rgba(255,255,255,.035) !important;color:#e9e5f5 !important;
          box-shadow:inset 3px 0 0 rgba(0,246,219,.55) !important;
        }
        #slither-arena-root .sa-btn.secondary:hover {background:rgba(124,92,255,.12) !important;}
        #slither-arena-root .sa-btn.video {background:linear-gradient(100deg,#ff9d00,#ff3d7f) !important;box-shadow:none !important;}
        #slither-arena-root .sa-command-deck {display:grid;grid-template-columns:minmax(0,1.15fr) minmax(220px,.85fr);gap:16px;align-items:stretch;}
        #slither-arena-root .sa-menu-panel {width:min(760px,calc(100vw - 28px)) !important;padding:22px !important;}
        #slither-arena-root .sa-menu-panel .sa-menu-hero {
          min-height:245px !important;display:flex !important;flex-direction:column !important;justify-content:space-between !important;
          padding:18px !important;border:1px solid rgba(255,255,255,.10) !important;
          background:linear-gradient(145deg,rgba(255,70,196,.075),rgba(124,92,255,.025)) !important;
          clip-path:polygon(0 0,100% 0,100% calc(100% - 36px),calc(100% - 36px) 100%,0 100%) !important;
        }
        #slither-arena-root .sa-menu-panel .sa-menu-copy {display:block !important;}
        #slither-arena-root .sa-menu-panel .sa-title {font-size:clamp(39px,6.7vw,72px) !important;line-height:.82 !important;max-width:470px;}
        #slither-arena-root .sa-menu-panel .sa-subtitle {max-width:420px;margin:14px 0 0 !important;font-size:12px !important;line-height:1.55 !important;}
        #slither-arena-root .sa-menu-emblem {
          display:block !important;position:absolute !important;right:18px !important;bottom:18px !important;width:88px !important;height:88px !important;
          border:1px solid rgba(0,246,219,.36) !important;border-radius:50% !important;background:transparent !important;
          box-shadow:0 0 0 10px rgba(0,246,219,.025),0 0 34px rgba(0,246,219,.08) inset !important;
        }
        #slither-arena-root .sa-menu-emblem::before {content:"" !important;position:absolute !important;inset:14px !important;width:auto !important;height:auto !important;left:14px !important;top:14px !important;border:1px dashed rgba(255,70,196,.65) !important;border-radius:50% !important;background:none !important;box-shadow:none !important;animation:hg-spin 8s linear infinite;}
        #slither-arena-root .sa-menu-emblem span {position:absolute !important;left:50% !important;top:50% !important;width:42px !important;height:6px !important;border-radius:0 !important;background:var(--hg-cyan) !important;box-shadow:0 0 14px rgba(0,246,219,.65) !important;transform:translate(-50%,-50%) rotate(-24deg) !important;}
        #slither-arena-root .sa-menu-emblem b,#slither-arena-root .sa-menu-emblem i {display:block;position:absolute;left:50%;top:50%;width:6px;height:6px;margin:-3px;border-radius:50%;background:var(--hg-pink);box-shadow:0 0 12px var(--hg-pink);}
        #slither-arena-root .sa-menu-emblem b {transform:translate(25px,-18px)}
        #slither-arena-root .sa-menu-emblem i {transform:translate(-27px,20px)}
        @keyframes hg-spin {to{transform:rotate(360deg)}}
        #slither-arena-root .sa-menu-side {display:flex !important;flex-direction:column !important;gap:10px !important;min-width:0;}
        #slither-arena-root .sa-menu-progress {margin:0 !important;gap:9px !important;}
        #slither-arena-root .sa-league-card,
        #slither-arena-root .sa-mission-teaser,
        #slither-arena-root .sa-stat,
        #slither-arena-root .sa-skin,
        #slither-arena-root .sa-control-card,
        #slither-arena-root .sa-mission-card,
        #slither-arena-root .sa-leaderboard-row {
          border-radius:2px !important;border:1px solid rgba(255,255,255,.10) !important;
          background:rgba(255,255,255,.025) !important;box-shadow:inset 2px 0 0 rgba(0,246,219,.30) !important;
        }
        #slither-arena-root .sa-league-card {padding:13px !important;}
        #slither-arena-root .sa-progress-bar {height:5px !important;border-radius:0 !important;background:rgba(255,255,255,.07) !important;}
        #slither-arena-root .sa-progress-bar i {border-radius:0 !important;background:linear-gradient(90deg,var(--hg-pink),var(--hg-cyan)) !important;box-shadow:0 0 12px rgba(0,246,219,.25);}
        #slither-arena-root .sa-mission-teaser {padding:11px 12px !important;}
        #slither-arena-root .sa-mission-mini {border-radius:1px !important;background:rgba(255,70,196,.08) !important;color:#ffd8f4 !important;}
        #slither-arena-root .sa-menu-panel .sa-stat-row {margin:0 !important;gap:6px !important;grid-template-columns:1fr !important;}
        #slither-arena-root .sa-menu-panel .sa-stat {min-height:46px !important;padding:8px 10px !important;display:flex !important;align-items:center !important;justify-content:space-between !important;}
        #slither-arena-root .sa-menu-panel .sa-stat small {font-family:ui-monospace,SFMono-Regular,Menlo,monospace !important;color:#8f8aa3 !important;font-size:8px !important;letter-spacing:.12em !important;}
        #slither-arena-root .sa-menu-panel .sa-stat strong {font-family:"Arial Black",Impact,sans-serif !important;font-size:17px !important;color:#fff !important;}
        #slither-arena-root .sa-menu-panel .sa-menu-actions {
          margin-top:14px !important;display:grid !important;grid-template-columns:2fr repeat(3,minmax(0,1fr)) !important;grid-auto-rows:46px !important;gap:7px !important;
        }
        #slither-arena-root .sa-menu-panel .sa-menu-actions .sa-play {grid-column:auto !important;grid-row:span 2 !important;display:flex !important;align-items:flex-end !important;justify-content:space-between !important;padding:14px !important;font-size:18px !important;text-align:left !important;}
        #slither-arena-root .sa-menu-panel .sa-menu-actions .sa-play b {font-size:28px;line-height:.8;font-weight:400;}
        #slither-arena-root .sa-menu-panel .sa-menu-actions .sa-btn {min-height:46px !important;height:auto !important;padding:8px !important;font-size:10px !important;}
        #slither-arena-root .sa-hud-card {
          border-radius:2px !important;background:rgba(7,8,20,.84) !important;border:1px solid rgba(255,255,255,.11) !important;
          box-shadow:inset 3px 0 0 var(--hg-pink),8px 8px 0 rgba(0,0,0,.15) !important;backdrop-filter:blur(10px) !important;
        }
        #slither-arena-root .sa-hud-label {color:var(--hg-cyan) !important;font-family:ui-monospace,monospace !important;}
        #slither-arena-root .sa-score-row strong {font-family:"Arial Black",Impact,sans-serif !important;}
        #slither-arena-root .sa-hint {border-radius:2px !important;background:rgba(7,8,20,.78) !important;box-shadow:inset 2px 0 0 var(--hg-cyan) !important;font-family:ui-monospace,monospace !important;}
        #slither-arena-root .sa-toast {border-radius:2px !important;background:#0d0d1e !important;box-shadow:inset 3px 0 0 var(--hg-pink),0 14px 38px rgba(0,0,0,.35) !important;}
        #slither-arena-root .sa-skin {padding:8px !important;clip-path:polygon(0 0,100% 0,100% calc(100% - 13px),calc(100% - 13px) 100%,0 100%) !important;}
        #slither-arena-root .sa-skin-preview {border-radius:1px !important;background:#080914 !important;}
        #slither-arena-root .sa-skin.selected {border-color:rgba(0,246,219,.72) !important;background:rgba(0,246,219,.055) !important;box-shadow:inset 3px 0 0 var(--hg-cyan),0 0 18px rgba(0,246,219,.08) !important;}
        #slither-arena-root .sa-control-demo {border-radius:1px !important;background:linear-gradient(145deg,#120a1d,#080b18) !important;}
        #slither-arena-root .sa-control-card.selected {border-color:rgba(255,70,196,.58) !important;background:rgba(255,70,196,.055) !important;box-shadow:inset 3px 0 0 var(--hg-pink) !important;}
        #slither-arena-root .sa-gameover-daily {border-radius:2px !important;background:rgba(255,70,196,.04) !important;box-shadow:inset 2px 0 0 var(--hg-pink) !important;}
        #slither-arena-root .sa-leaderboard-row:nth-child(1) {border-color:rgba(0,246,219,.38) !important;background:rgba(0,246,219,.05) !important;}
        #slither-arena-root .sa-leaderboard-row.me {border-color:rgba(255,70,196,.48) !important;background:rgba(255,70,196,.05) !important;}
        #slither-arena-root .sa-loading-orb {border-radius:2px !important;transform:rotate(45deg);border-color:rgba(255,70,196,.2) !important;border-top-color:var(--hg-cyan) !important;}

        @media (max-width:700px) {
          #slither-arena-root .sa-menu-panel {width:calc(100vw - 12px) !important;padding:12px !important;}
          #slither-arena-root .sa-command-deck {grid-template-columns:1fr !important;gap:8px !important;}
          #slither-arena-root .sa-menu-panel .sa-menu-hero {min-height:126px !important;padding:12px !important;}
          #slither-arena-root .sa-menu-panel .sa-title {font-size:clamp(31px,10vw,46px) !important;max-width:82%;}
          #slither-arena-root .sa-menu-emblem {width:60px !important;height:60px !important;right:10px !important;bottom:10px !important;}
          #slither-arena-root .sa-menu-panel .sa-subtitle {max-width:78%;font-size:9px !important;line-height:1.35 !important;}
          #slither-arena-root .sa-menu-side {display:grid !important;grid-template-columns:1.25fr .75fr !important;gap:7px !important;}
          #slither-arena-root .sa-menu-panel .sa-stat-row {grid-template-columns:1fr !important;gap:5px !important;}
          #slither-arena-root .sa-menu-panel .sa-stat {min-height:35px !important;padding:5px 7px !important;}
          #slither-arena-root .sa-menu-panel .sa-stat strong {font-size:13px !important;}
          #slither-arena-root .sa-menu-panel .sa-menu-actions {grid-template-columns:repeat(3,minmax(0,1fr)) !important;grid-auto-rows:38px !important;margin-top:8px !important;gap:5px !important;}
          #slither-arena-root .sa-menu-panel .sa-menu-actions .sa-play {grid-column:1/-1 !important;grid-row:auto !important;min-height:42px !important;align-items:center !important;font-size:15px !important;}
          #slither-arena-root .sa-menu-panel .sa-menu-actions .sa-btn {min-height:38px !important;font-size:9px !important;}
        }
        @media (orientation:landscape) and (max-height:520px) {
          #slither-arena-root .sa-menu-panel {width:min(760px,calc(100vw - 12px)) !important;padding:8px 10px !important;}
          #slither-arena-root .sa-command-deck {grid-template-columns:1.12fr .88fr !important;gap:8px !important;}
          #slither-arena-root .sa-menu-panel .sa-menu-hero {min-height:0 !important;height:100% !important;padding:10px !important;}
          #slither-arena-root .sa-menu-panel .sa-title {font-size:clamp(28px,5.6vw,48px) !important;max-width:80%;}
          #slither-arena-root .sa-menu-panel .sa-subtitle {display:block !important;font-size:8px !important;max-width:72%;}
          #slither-arena-root .sa-menu-emblem {display:block !important;width:54px !important;height:54px !important;}
          #slither-arena-root .sa-menu-side {display:grid !important;grid-template-columns:1fr .48fr !important;gap:6px !important;}
          #slither-arena-root .sa-menu-panel .sa-menu-actions {grid-template-columns:1.7fr repeat(5,minmax(0,1fr)) !important;grid-auto-rows:35px !important;margin-top:6px !important;}
          #slither-arena-root .sa-menu-panel .sa-menu-actions .sa-play {grid-column:auto !important;grid-row:auto !important;min-height:35px !important;padding:6px 8px !important;font-size:12px !important;}
          #slither-arena-root .sa-menu-panel .sa-menu-actions .sa-play b {font-size:18px;}
          #slither-arena-root .sa-menu-panel .sa-menu-actions .sa-btn {min-height:35px !important;height:35px !important;padding:5px !important;font-size:8px !important;}
          #slither-arena-root .sa-menu-panel .sa-stat {min-height:31px !important;padding:4px 6px !important;}
          #slither-arena-root .sa-menu-panel .sa-stat small {font-size:6px !important;}
          #slither-arena-root .sa-menu-panel .sa-stat strong {font-size:11px !important;}
          #slither-arena-root .sa-league-card {padding:7px !important;}
          #slither-arena-root .sa-mission-teaser {padding:6px 7px !important;}
        }


        /* HYPERGRID touch-landscape override: the old two-column menu rules used the
           previous flat DOM. The new command-deck wrapper owns the layout instead. */
        #slither-arena-root.sa-touch-landscape .sa-menu-panel {
          display:block !important;
        ...
        #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-command-deck {
          display:grid !important;
        ...
        #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-menu-copy { display:block !important; }
        #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-menu-emblem {
          display:block !important;width:54px !important;height:54px !important;right:9px !important;bottom:9px !important;
        }
        #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-brand {font-size:7px !important;margin:0 0 2px !important;}
        #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-title {font-size:clamp(28px,5.6vw,48px) !important;line-height:.82 !important;max-width:80%;}
        #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-subtitle {display:block !important;font-size:8px !important;line-height:1.25 !important;max-width:72%;margin:6px 0 0 !important;}
        #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-menu-side {display:grid !important;grid-template-columns:1fr .48fr !important;gap:6px !important;}
        #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-menu-progress {display:grid !important;gap:5px !important;margin:0 !important;}
        #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-league-card {padding:7px !important;border-radius:2px !important;}
        #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-league-kicker {display:block !important;font-size:7px !important;}
        #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-league-title {font-size:10px !important;}
        #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-mission-teaser {display:flex !important;padding:6px 7px !important;border-radius:2px !important;}
        #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-stat-row {display:grid !important;grid-template-columns:1fr !important;gap:4px !important;margin:0 !important;}
        #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-stat {display:flex !important;min-height:31px !important;padding:4px 6px !important;border-radius:2px !important;}
        #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-stat small {font-size:6px !important;}
        #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-stat strong {font-size:11px !important;white-space:nowrap !important;overflow:hidden !important;text-overflow:ellipsis !important;}
        #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-menu-actions {
          display:grid !important;grid-template-columns:1.7fr repeat(5,minmax(0,1fr)) !important;grid-template-rows:35px !important;grid-auto-rows:35px !important;gap:5px !important;margin-top:6px !important;
        }
        #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-menu-actions .sa-play {grid-column:auto !important;grid-row:auto !important;min-height:35px !important;height:35px !important;padding:6px 8px !important;font-size:12px !important;align-items:center !important;}
        #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-menu-actions .sa-btn {min-height:35px !important;height:35px !important;padding:5px !important;border-radius:3px !important;font-size:8px !important;}
        #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-menu-actions .sa-controls-btn {grid-column:auto !important;}
        #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-social-label {font-size:8px !important;}
  ...
  fitOverlayPanel(renderPreviews = false) {
    ...
    if (!overlay || !panel || !overlay.classList.contains("visible")) {
      this.root?.classList.remove("sa-scrollable-overlay");
      return;
    }

    // Always measure the panel at its natural size. VK/OK can update the iframe
    // height several times while mobile browser chrome and the sticky banner settle.
    panel.style.setProperty("--sa-fit-scale", "1");
    panel.style.removeProperty("max-height");
    panel.style.removeProperty("overflow-y");
    panel.style.removeProperty("overflow-x");
    panel.classList.remove("sa-auto-fit", "sa-scroll-fit");
    this.root?.classList.remove("sa-scrollable-overlay");
    if (renderPreviews) this.renderSkinPreviews();

    const bounds = overlay.getBoundingClientRect();
    const reserve = this.isTouchInputEnvironment() ? 12 : 20;
    ...
    // No scrolling and no clipping: scale the complete overlay by whichever axis
    // is more constrained. A very small lower guard only prevents invalid CSS if a
    // WebView briefly reports a near-zero viewport during rotation.
    const scale = Math.max(.01, Math.min(1, availableWidth / rawWidth, availableHeight / rawHeight));
  ...
  async purchaseSkin(skin) {
    if (!skin?.sku || this.purchaseSkinBusy) return;
    ...
    const returnPage = this.shopPage;
    const button = this.ui.overlay.querySelector(`[data-action="purchase-skin"][data-skin="${skin.id}"]`);
    this.purchaseSkinBusy = true;
    if (button) {
      button.disabled = true;
      button.setAttribute("aria-busy", "true");
    }
    ...
    let success = false;
    try {
      success = await this.bridge.purchaseSkin(skin.sku);
    } finally {
      this.purchaseSkinBusy = false;
      if (button) {
        button.disabled = false;
    ...
    if (!this.progress.unlockedSkins.includes(skin.id)) this.progress.unlockedSkins.push(skin.id);
    if (!this.progress.purchaseSkins.includes(skin.id)) this.progress.purchaseSkins.push(skin.id);
    this.progress.selectedSkin = skin.id;
    await this.persistProgress();
    ...
    this.showOverlay(`
      <div class="sa-command-deck">
        <div class="sa-menu-hero">
          ...
          <div class="sa-menu-emblem" aria-hidden="true"><span></span><b></b><i></i></div>
        </div>
        <div class="sa-menu-side">
      ...
      <div class="sa-menu-actions">
        <button class="sa-btn sa-play" data-action="play"><span>${this.t("home.play")}</span><b aria-hidden="true">↗</b></button>
  ...
  showControls() {
    ...
    const card = (mode, title, description) => `
      <button type="button" class="sa-control-card ${selected === mode ? "selected" : ""}" data-action="control-mode" data-control="${mode}">
        ...
        <strong>${title}</strong><span class="sa-control-description">${description}</span>
      </button>`;
    this.showOverlay(`
  ...
  draw() {
    ...
    background.addColorStop(0, "#17051f");
    background.addColorStop(.48, "#0b0d22");
    ...
    ctx.save();
    ctx.lineWidth = Math.max(1, dpr);
    ctx.strokeStyle = "rgba(255,70,196,.072)";
    ctx.beginPath();
    const diagonalSpan = width + height;
    const phaseA = ((origin.x + origin.y) % grid + grid) % grid;
    const phaseB = ((origin.x - origin.y) % grid + grid) % grid;
    for (let d = -height + phaseA; d < width + height; d += grid) { ctx.moveTo(d, 0); ctx.lineTo(d - height, height); }
    for (let d = -height + phaseB; d < width + height; d += grid) { ctx.moveTo(d, 0); ctx.lineTo(d + height, height); }
    ctx.stroke();
    ctx.globalAlpha = .55;
    ctx.strokeStyle = "rgba(0,246,219,.11)";
    ctx.beginPath();
    const major = grid * 4;
    const majorPhase = ((origin.y) % major + major) % major;
    for (let y = majorPhase; y < height; y += major) { ctx.moveTo(0, y); ctx.lineTo(width, y); }
    ctx.stroke();
    const half = CFG.WORLD_SIZE / 2;
    const topLeft = this.worldToScreen(-half, -half);
    const bottomRight = this.worldToScreen(half, half);
    ctx.globalAlpha = 1;
    ctx.strokeStyle = "rgba(0,246,219,.38)";
    ctx.lineWidth = 2 * dpr;
    ...
    for (const food of this.food) {
      ...
      ctx.rotate(food.shimmer * .18 + food.id * .13);
      ctx.beginPath();
      ctx.moveTo(0, -r * 1.45);
      ctx.lineTo(r * .92, 0);
      ctx.lineTo(0, r * 1.45);
