...
// Версия: GamePush v1.49.4d — диагностика покупки OK для deck_frost5/deck_amber5.
(async () => {
  "use strict";
  ...
  const CONFIG = {
    ...
    products: {
      ...
      frostDeck: "deck_frost5",
      amberDeck: "deck_amber5"
    },
  ...
  const DECKS = {
    ...
    frost5: { titleKey: "frost", kind: "purchase", product: "frostDeck", theme: { tableA: "#5f8fa8", tableB: "#183a52", accent: "#dff7ff", backA: "#b9ecff", backB: "#4f88b8", frontA: "#ffffff", frontB: "#e8f7ff" } },
    amber5: { titleKey: "amber", kind: "purchase", product: "amberDeck", theme: { tableA: "#8a5a1f", tableB: "#3d220d", accent: "#ffd77a", backA: "#d9902f", backB: "#7a4215", frontA: "#fffaf0", frontB: "#f6e4bf" } }
  };
  ...
  const I18N = Object.freeze({
    ...
    pt: {
      ...
      hintsTitle:"Dicas", hintsCap:"Você já tem o máximo: <b>{cap}</b>. Use uma dica em uma partida e depois poderá recarregar.", hintsAvailable:"Você tem <b>{balance}</b> de {cap}. Cada novo jogo começa com 3 dicas.", hintsEmpty:"As dicas grátis acabaram. Assista a um anúncio curto e ganhe <b>+{amount}</b> dicas.", hintsInfo:"A dica não faz a jogada por você: ela só destaca o próximo passo mais útil.", watchAdPlus:"Assistir anúncio em vídeo — +{amount}", backToGame:"Voltar ao jogo", notNow:"Agora não", hintsEnough:"Você já tem dicas suficientes", openingAd:"Abrindo anúncio…", adNoReward:"O anúncio não foi concluído — nenhuma recompensa recebida", hintsReceived:"Dicas recebidas: +{amount}",
  ...
  const purchaseDiagnostics = {
    active:false, startedAt:0, productTag:"", lines:[],
    reset(productTag="") { this.active=true; this.startedAt=performance.now(); this.productTag=productTag; this.lines=[]; this.log("DIAG START", {productTag, visibility:document.visibilityState, focused:document.hasFocus?.()}); },
    log(label, detail=null) {
      if (!this.active) return;
      const elapsed=Math.max(0,Math.round(performance.now()-this.startedAt));
      let suffix="";
      if (detail!==null && detail!==undefined) {
        try { const seen=new WeakSet(); suffix=" "+JSON.stringify(detail,(k,v)=>{ if(typeof v==="function") return `[fn ${v.name||"anonymous"}]`; if(typeof v==="object"&&v!==null){ if(seen.has(v)) return "[circular]"; seen.add(v);} return v;}); }
        catch(_) { suffix=" "+String(detail); }
      }
      if (suffix.length>360) suffix=suffix.slice(0,357)+"...";
      const line=`[+${String(elapsed).padStart(5,"0")}ms] ${label}${suffix}`;
      this.lines.push(line); if(this.lines.length>42) this.lines.splice(0,this.lines.length-42);
      console.log("[Kosynka PurchaseDiag]",line); this.render();
    },
    render() {
      if(!this.active) return;
      const root=document.querySelector("#kosynka-c3-root")||document.body;
      let panel=document.querySelector("#kosynka-purchase-diagnostic");
      if(!panel){ panel=document.createElement("div"); panel.id="kosynka-purchase-diagnostic"; panel.style.cssText="position:fixed;right:8px;bottom:8px;z-index:2147483646;width:min(480px,calc(100vw - 16px));max-height:min(44vh,330px);padding:8px 10px;border:1px solid rgba(255,255,255,.3);border-radius:10px;background:rgba(0,18,14,.94);color:#eafff7;font:11px/1.35 ui-monospace,SFMono-Regular,Consolas,monospace;box-shadow:0 8px 30px rgba(0,0,0,.45);pointer-events:none;overflow:auto;white-space:pre-wrap;word-break:break-word"; root.appendChild(panel); }
      panel.textContent=`ДИАГНОСТИКА ПОКУПКИ — ${this.productTag||"?"}
${this.lines.join("\n")}`; panel.scrollTop=panel.scrollHeight;
      const inline=document.querySelector("#purchase-diagnostic-log"); if(inline){ inline.textContent=this.lines.slice(-12).join("\n"); inline.scrollTop=inline.scrollHeight; }
    }
  };

  function purchaseDiagState(label, productTag="") {
    if(!purchaseDiagnostics.active) return;
    const tag=productTag||purchaseDiagnostics.productTag||platform.activePurchaseTag||"";
    let has=null; try{has=tag?platform.hasPurchase(tag):null;}catch(_){has="error";}
    let rawPurchases=[]; try{rawPurchases=Array.isArray(platform.gp?.payments?.purchases)?platform.gp.payments.purchases.map(item=>platform.resolvePurchaseTag(item)||platform.productIdFrom(item)||"?"):[];}catch(_){rawPurchases=["read-error"];}
    purchaseDiagnostics.log(label,{tag,has,pending:[...(platform.pendingPurchaseTags||[])],active:platform.activePurchaseTag||"",normalized:[...(platform.purchasedProducts||[])],rawPurchases,visibility:document.visibilityState,focused:document.hasFocus?.()});
  }
  ...
  const platform = {
    ...
    activePurchaseTag: "",
    purchaseWaiters: new Map(),
    purchaseVerificationTasks: new Map(),
    postCheckoutCheckTimers: new Map(),
    ...
    resolveGamePushPlatform() {
      ...
      const candidates = [
        ...
        // VK Mini Apps launched inside Odnoklassniki receive vk_client=ok. This is the
        // most reliable discriminator after the VK/OK platform unification.
        query.get("vk_client"),
      ...
      const raw = candidates.map(value => String(value || "").toLowerCase()).filter(Boolean).join(" ");
      // Prefer OK when both generic VK Mini Apps markers and vk_client=ok are present.
    ...
    async refreshCommerce({ preserveSession = true, forceFetch = false } = {}) {
      ...
      try {
        ...
        if ((forceFetch || !products || !purchases) && typeof payments.fetchProducts === "function") {
          ...
          // IMPORTANT FOR OK: some adapters refresh payments.products/payments.purchases internally
          // but return only the product catalogue from fetchProducts(). Re-read the live SDK arrays
          // *after* the await, otherwise we keep the stale pre-checkout array until a focus/reload event.
          if (!fetchedProducts && Array.isArray(payments.products)) products = payments.products;
          if (!fetchedPurchases && Array.isArray(payments.purchases)) purchases = payments.purchases;
        }
        products = products || [];
        purchases = purchases || [];
    ...
    async forceGamePushPurchaseResumeSync(source = "purchase-resume", { fullCycle = false } = {}) {
      if (this.kind !== "gamepush" || this.gamePushPlatform !== "ok" || !this.gp) return;
      try {
        // Empirically on OK the purchase cache becomes current only after the app lifecycle resumes
        // (the same thing that happens when the player switches browser tabs and comes back).
        // First try the public GamePush resume() hook. On a later retry do one explicit pause/resume
        // cycle to mirror the real hidden -> visible transition more closely.
        if (fullCycle && typeof this.gp.pause === "function") {
          try { await this.gp.pause(); } catch (_) { /* Best-effort lifecycle nudge. */ }
          await new Promise(resolve => setTimeout(resolve, 40));
        }
        if (typeof this.gp.resume === "function") this.gp.resume();
        await new Promise(resolve => setTimeout(resolve, 120));
      } catch (error) {
        console.warn("[Kosynka] GamePush purchase resume sync failed", source, error);
      }
    },

    async confirmPurchase(productTag, { resumeSync = false, fullCycle = false, source = "confirm-purchase" } = {}) {
      const before = this.hasPurchase(productTag);
      purchaseDiagState(`confirmPurchase BEFORE source=${source}`, productTag);
      if (before) return true;
      if (resumeSync) {
        purchaseDiagnostics.log("FORCED SDK RESUME SYNC", { source, fullCycle });
        await this.forceGamePushPurchaseResumeSync(source, { fullCycle });
        purchaseDiagState(`STATE after forced resume source=${source}`, productTag);
      }
      await this.refreshCommerce({ preserveSession: true, forceFetch: true });
      const after = this.hasPurchase(productTag);
      purchaseDiagState(`confirmPurchase AFTER=${after} source=${source}`, productTag);
      return after;
    },

    verifyPendingPurchase(productTag, { maxAttempts = 24, intervalMs = 600 } = {}) {
      if (this.kind !== "gamepush" || !productTag) return Promise.resolve(false);
      const existing = this.purchaseVerificationTasks.get(productTag);
      if (existing) return existing;

      const task = (async () => {
        for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
          // A purchase event or a resolved checkout may have completed while this verifier slept.
          if (this.purchasedProducts.has(productTag)) {
            this.pendingPurchaseTags.delete(productTag);
            if (this.activePurchaseTag === productTag) this.activePurchaseTag = "";
            this.settlePurchaseWaiter(productTag, {
              ok: true, code: "success", productTag, source: "session-purchase-cache"
            });
          ...
          try {
            ...
            if (this.activePurchaseTag === productTag) this.activePurchaseTag = "";
            this.settlePurchaseWaiter(productTag, {
              ok: true, code: "success", productTag, source: "verified-purchase-cache"
            });
            await handlePlatformPurchaseSuccess(productTag);
            return true;
          } catch (error) {
            console.warn("[Kosynka] Purchase verification attempt failed", productTag, error);
          }
        }
        return false;
      })().finally(() => {
        this.purchaseVerificationTasks.delete(productTag);
      });

      this.purchaseVerificationTasks.set(productTag, task);
      return task;
    },
    ...
    async reconcilePendingPurchases() {
      ...
      try {
        // In OK the native payment UI can close/return focus before payments.purchase() settles.
        // Verify every pending tag independently from that Promise. This is intentionally safe:
        // the entitlement is granted only after GamePush reports the product in purchases/has().
        await Promise.all([...this.pendingPurchaseTags].map(tag =>
          this.verifyPendingPurchase(tag, {
            maxAttempts: this.gamePushPlatform === "ok" ? 24 : 8,
            intervalMs: this.gamePushPlatform === "ok" ? 600 : 500
          })
        ));
      } catch (error) {
    ...
    clearPostCheckoutChecks(productTag) {
      const timers = this.postCheckoutCheckTimers.get(productTag) || [];
      for (const timer of timers) clearTimeout(timer);
      this.postCheckoutCheckTimers.delete(productTag);
    },

    async verifyPurchaseOnce(productTag, source = "post-checkout") {
      if (this.kind !== "gamepush" || !productTag) return false;
      if (this.purchasedProducts.has(productTag)) {
        this.clearPostCheckoutChecks(productTag);
        return true;
      }
      try {
        const confirmed = await this.confirmPurchase(productTag, {
          resumeSync: this.gamePushPlatform === "ok",
          source
        });
        ...
        this.clearPostCheckoutChecks(productTag);
        this.settlePurchaseWaiter(productTag, {
          ok: true, code: "success", productTag, source
        });
        await handlePlatformPurchaseSuccess(productTag);
        return true;
      } catch (error) {
        console.warn("[Kosynka] One-shot purchase verification failed", productTag, source, error);
        return false;
      }
    },

    schedulePostCheckoutChecks(productTag) {
      if (this.kind !== "gamepush" || !productTag) return;
      this.clearPostCheckoutChecks(productTag);
      // OK updates its GamePush purchase cache on an application resume lifecycle transition.
      // Reproduce that automatically instead of requiring the player to switch browser tabs.
      ...
      const timers = delays.map(delay => setTimeout(async () => {
        ...
        try {
          if (this.gamePushPlatform === "ok") {
            await this.forceGamePushPurchaseResumeSync(`post-checkout-${delay}ms`, {
              // If a plain resume was not enough, the 1s retry mirrors a real hidden -> visible cycle.
              fullCycle: delay === 1000
            });
          }
          await this.verifyPurchaseOnce(productTag, `post-checkout-${delay}ms`);
        } catch (error) {
          console.warn("[Kosynka] Scheduled purchase resume refresh failed", error);
        }
      }, delay));
      this.postCheckoutCheckTimers.set(productTag, timers);
    },
    ...
    async syncEntitlements(targetProfile) {
      ...
      const productToSkin = {
        ...
        [CONFIG.products.frostDeck]: "frost5",
        [CONFIG.products.amberDeck]: "amber5"
      };
    ...
    settlePurchaseWaiter(productTag, result) {
      const waiter = this.purchaseWaiters.get(productTag);
      if (!waiter) return false;
      this.purchaseWaiters.delete(productTag);
      clearTimeout(waiter.timer);
      waiter.resolve(result);
      return true;
    },

    waitForPurchaseSignal(productTag, timeoutMs = 180000) {
      const oldWaiter = this.purchaseWaiters.get(productTag);
      if (oldWaiter) {
        clearTimeout(oldWaiter.timer);
        oldWaiter.resolve({ ok: false, code: "replaced" });
        this.purchaseWaiters.delete(productTag);
      }
      return new Promise(resolve => {
        const timer = setTimeout(() => {
          this.purchaseWaiters.delete(productTag);
          purchaseDiagState("waitForPurchaseSignal TIMEOUT -> pending", productTag);
          // Do not grant anything on timeout. Keep the tag pending so a delayed purchase event or
          // a focus recovery can still reconcile the permanent entitlement safely.
          resolve({ ok: false, code: "pending" });
        }, timeoutMs);
        this.purchaseWaiters.set(productTag, { resolve, timer });
      });
    },

    resolveActivePurchaseTag(...args) {
      let tag = this.resolvePurchaseTag(...args);
      if (tag) return tag;
      // The checkout button stores the exact active product before opening the native payment UI.
      // OK may emit a successful purchase event without a useful tag, and there may also be stale
      // pending tags from previous attempts. Prefer the active checkout instead of guessing by set size.
      if (this.activePurchaseTag) return this.activePurchaseTag;
      const pending = [...this.pendingPurchaseTags];
    ...
    bindPurchaseEvents() {
      ...
      this.gp.payments.on("purchase", (...args) => {
        purchaseDiagnostics.log("GamePush payments EVENT purchase", args);
        const tag = this.resolveActivePurchaseTag(...args);
        if (!tag) {
          console.warn("[Kosynka] Purchase SUCCESS event received, but no active product could be resolved", ...args);
          return;
        }
        this.pendingPurchaseTags.delete(tag);
        if (this.activePurchaseTag === tag) this.activePurchaseTag = "";
        this.clearPostCheckoutChecks(tag);
        this.purchasedProducts.add(tag);
        this.settlePurchaseWaiter(tag, { ok: true, code: "success", productTag: tag, source: "event", result: args[0] });
        // Apply the permanent entitlement directly from the documented success event. Do not wait
        // for payments.purchases, payments.has() or a cloud refresh; those caches may update only on
        // the next SDK initialization on some platform adapters.
      ...
      this.gp.payments.on("error:purchase", (...args) => {
        purchaseDiagnostics.log("GamePush payments EVENT error:purchase", args);
        const tag = this.resolveActivePurchaseTag(...args);
        if (tag) {
          this.pendingPurchaseTags.delete(tag);
          if (this.activePurchaseTag === tag) this.activePurchaseTag = "";
          this.clearPostCheckoutChecks(tag);
          this.settlePurchaseWaiter(tag, { ok: false, code: "cancelled", source: "error", error: args[0] });
        }
      });
      this.gp.payments.on("close", () => {
        purchaseDiagnostics.log("GamePush payments EVENT close");
        const tag = this.resolveActivePurchaseTag();
        purchaseDiagState("STATE on payments close", tag);
        if (!tag || !this.pendingPurchaseTags.has(tag)) return;
        // Do not wait for a browser focus/visibility event. Poll the refreshed purchase catalogue
        // immediately after checkout closes: now, +0.5s, +1s, +2s and +4s.
        this.schedulePostCheckoutChecks(tag);
      });
      if (typeof this.gp?.on === "function") {
        try { this.gp.on("pause", (...args) => purchaseDiagnostics.log("GamePush CORE EVENT pause", args)); } catch (_) {}
        try { this.gp.on("resume", (...args) => { purchaseDiagnostics.log("GamePush CORE EVENT resume", args); purchaseDiagState("STATE on GamePush resume"); }); } catch (_) {}
      }
    ...
    async purchase(productTag) {
      try {
        ...
        if (this.kind === "gamepush" && this.gp?.payments?.purchase) {
          ...
          // Keep one authoritative active checkout. OK callbacks may omit the product payload, so
          // the button click itself remains the source of truth for which product is being processed.
          this.activePurchaseTag = productTag;
          this.pendingPurchaseTags.add(productTag);
          purchaseDiagnostics.log("platform.purchase START", { productTag, platform: this.gamePushPlatform });
          purchaseDiagState("STATE before checkout", productTag);

          if (this.gamePushPlatform === "ok") {
            // IMPORTANT FOR OK:
            // Do not await payments.purchase() directly. In the OK/VK Mini Apps adapter the native
            // payment flow may finish while this Promise remains pending. The UI instead waits for
            // one of three independent confirmations: the documented `purchase` event, a resolved
            // checkout result, or purchases/has() becoming true after close/focus reconciliation.
            const signal = this.waitForPurchaseSignal(productTag, 20000);

            let checkoutPromise;
            try {
              purchaseDiagnostics.log("payments.purchase CALL", { productTag });
              checkoutPromise = this.gp.payments.purchase({ tag: productTag });
            } catch (error) {
              purchaseDiagnostics.log("payments.purchase THROW", { message: error?.message || String(error) });
              this.pendingPurchaseTags.delete(productTag);
              if (this.activePurchaseTag === productTag) this.activePurchaseTag = "";
              this.settlePurchaseWaiter(productTag, { ok: false, code: "unavailable", source: "checkout-throw", error });
              return await signal;
            }

            Promise.resolve(checkoutPromise).then(receipt => {
              purchaseDiagnostics.log("payments.purchase RESOLVE", receipt);
              purchaseDiagState("STATE on purchase Promise resolve", productTag);
              const status = String(receipt?.status || receipt?.state || "").toLowerCase();
              const explicitFailure = receipt === false || receipt?.success === false ||
                ["cancelled", "canceled", "failed", "error", "rejected"].includes(status);
              if (explicitFailure) {
                this.pendingPurchaseTags.delete(productTag);
                if (this.activePurchaseTag === productTag) this.activePurchaseTag = "";
                this.settlePurchaseWaiter(productTag, {
                  ok: false, code: status.includes("cancel") ? "cancelled" : "unavailable",
                  source: "resolved-checkout-failure", result: receipt
                });
                return;
              }

              // An OK adapter can resolve with an empty/ambiguous value when the native window closes.
              // Do NOT grant on that alone. Only an explicit successful result, the `purchase` event,
              // or a positive purchases/has() verification may unlock a permanent product.
              const resolvedTag = this.resolvePurchaseTag(receipt) || productTag;
              const explicitSuccess = receipt === true || receipt?.success === true ||
                ["success", "succeeded", "paid", "completed", "complete"].includes(status) ||
                Boolean(receipt && (receipt.product || receipt.purchase || this.resolvePurchaseTag(receipt)));
              if (!explicitSuccess) {
                this.schedulePostCheckoutChecks(productTag);
                return;
              }

              this.purchasedProducts.add(productTag);
              this.purchasedProducts.add(resolvedTag);
              this.clearPostCheckoutChecks(productTag);
              this.clearPostCheckoutChecks(resolvedTag);
              this.pendingPurchaseTags.delete(productTag);
              this.pendingPurchaseTags.delete(resolvedTag);
              if (this.activePurchaseTag === productTag || this.activePurchaseTag === resolvedTag) this.activePurchaseTag = "";
              this.settlePurchaseWaiter(productTag, {
                ok: true, code: "success", productTag: resolvedTag, requestedProductTag: productTag,
                source: "resolved-ok-checkout", result: receipt
              });
              Promise.resolve(handlePlatformPurchaseSuccess(resolvedTag)).catch(error =>
                console.warn("[Kosynka] Resolved OK purchase entitlement update failed", error)
              );
            }).catch(error => {
              purchaseDiagnostics.log("payments.purchase REJECT", { message: error?.message || String(error) });
              purchaseDiagState("STATE on purchase Promise reject", productTag);
              // A rejected Promise in OK is not authoritative either: a platform success event/cache
              // confirmation can still arrive independently. Keep the checkout pending and verify it.
              if (this.purchasedProducts.has(productTag)) return;
              console.warn("[Kosynka] OK checkout promise rejected; verifying purchase state", error);
              this.schedulePostCheckoutChecks(productTag);
            });

            // Some OK clients do not deliver a useful close callback. Start a background verifier
            // as a fallback; close/focus events call the same verifier and reuse the in-flight task.
            setTimeout(() => {
              if (!this.pendingPurchaseTags.has(productTag)) return;
              this.schedulePostCheckoutChecks(productTag);
            }, 1000);
          ...
          // VK/Yandex: keep the normal direct Promise path, which already behaves correctly there.
          const receipt = await this.gp.payments.purchase({ tag: productTag });
          const status = String(receipt?.status || receipt?.state || "").toLowerCase();
          const explicitFailure = receipt === false || receipt?.success === false ||
            ["cancelled", "canceled", "failed", "error", "rejected"].includes(status);
          if (explicitFailure) {
            this.pendingPurchaseTags.delete(productTag);
            if (this.activePurchaseTag === productTag) this.activePurchaseTag = "";
            return { ok: false, code: status.includes("cancel") ? "cancelled" : "unavailable", result: receipt };
          }
          ...
          this.purchasedProducts.add(productTag);
          this.purchasedProducts.add(resolvedTag);
          Promise.resolve(this.refreshCommerce({ preserveSession: true, forceFetch: true })).catch(error =>
            console.warn("[Kosynka] Purchase background catalogue refresh failed", error)
          );
          return { ok: true, code: "success", productTag: resolvedTag, requestedProductTag: productTag, source: "resolved-checkout", result: receipt };
        }
      } catch (error) {
        console.warn("[Kosynka] Purchase failed", error);
        if (this.purchasedProducts.has(productTag)) {
          this.pendingPurchaseTags.delete(productTag);
          if (this.activePurchaseTag === productTag) this.activePurchaseTag = "";
          return { ok: true, code: "success", productTag, source: "purchase-event", error };
        }
        this.pendingPurchaseTags.delete(productTag);
        if (this.activePurchaseTag === productTag) this.activePurchaseTag = "";
  ...
  function deckAccessKind(deck) {
    ...
    // Playgama does not use purchases. Premium themes are rewarded-video themes there.
  ...
  async function startFreshGame() {
    ...
    game = newGameState();
    // Требование модерации VK/OK: каждая новая партия начинается ровно с 3 подсказок.
    setHintBalance(CONFIG.hintStartBalance);
  ...
  async function maybeShowInterstitial(placement) {
    ...
    try {
      const shown = await runExternalInteraction(
        () => platform.showInterstitial(`solitaire_${placement}`),
        false
      );
      if (shown) {
  ...
  function purchaseLabel(productTag) {
    ...
    return price ? `<span class="purchase-cta"><span class="purchase-verb">${escapeHtml(t("buy"))}</span><span class="purchase-divider" aria-hidden="true">·</span>${price}</span>` : escapeHtml(t("buy"));
  }
  ...
  function openStore(tab = shopTab) {
    ...
    const paid = ["royal", "noir", "ruby", "gold", "cosmos", "frost5", "amber5"];
  ...
  async function handlePlatformPurchaseSuccess(productTag, forcedSkinId = "") {
    if (!profile || !productTag) return;
    purchaseDiagnostics.log("ENTITLEMENT SUCCESS", { productTag, forcedSkinId });
    platform.purchasedProducts.add(productTag);
    platform.pendingPurchaseTags.delete(productTag);
    if (platform.activePurchaseTag === productTag) platform.activePurchaseTag = "";
    let changed = false;
    // purchaseSkin() passes the exact clicked deck ID. This is authoritative for the current checkout
    // and avoids depending on a platform callback returning the correct product tag.
    const skinId = forcedSkinId && DECKS[forcedSkinId] ? forcedSkinId : purchasedSkinFromTag(productTag);
    if (skinId) {
  ...
  function showPurchaseProcessing(kind = "skin") {
    const message = kind === "noads" ? t("purchaseProcessingNoAds") : t("purchaseProcessingCopy");
    showModal(t("purchaseProcessingTitle"), `
      <div class="purchase-wait" role="status" aria-live="polite">
        <div class="purchase-wait-spinner" aria-hidden="true"></div>
        <p>${escapeHtml(message)}</p>
        <div class="purchase-diagnostic-inline"><small>Диагностика событий покупки</small><pre id="purchase-diagnostic-log"></pre></div>
      </div>
    `, true);
    app?.querySelector("#k-modal")?.classList.add("purchase-processing-modal");
    purchaseDiagnostics.render();
  }
  ...
  async function purchaseSkin(id) {
    ...
    purchaseDiagnostics.reset(productTag);
    purchaseDiagnostics.log("purchaseSkin CLICK", { id, productTag, platform: platform.gamePushPlatform });
    showPurchaseProcessing("skin");
    const purchase = await runExternalInteraction(
      () => platform.purchase(productTag),
      { ok: false, code: "unavailable" }
    );
    purchaseDiagnostics.log("purchaseSkin platform.purchase RESULT", purchase);
    purchaseDiagState("STATE after platform.purchase result", productTag);
    if (purchase?.code === "login_required") {
      showPurchaseLogin("skin", id);
      return;
    }
    if (!purchase?.ok) {
      // If OK did not deliver a decisive callback and the 20s waiter expired, start the same
      // automatic post-checkout refresh sequence at the exact moment the processing overlay closes.
      // This removes the need to switch browser tabs just to trigger visibility/focus recovery.
      if (purchase?.code === "pending" && platform.kind === "gamepush" && platform.gamePushPlatform === "ok") {
        platform.schedulePostCheckoutChecks(productTag);
      }
      openStore("purchase");
    ...
    // DIRECT UNLOCK: the checkout was started from this exact deck tile, so grant this exact deck ID.
    // No catalogue refresh, focus event, menu transition or re-open is allowed to gate the visible result.
    const confirmedTag = purchase?.productTag || productTag;
    await handlePlatformPurchaseSuccess(confirmedTag, id);
    openStore("purchase");
  ...
  async function purchaseNoAds() {
    ...
    purchaseDiagnostics.reset(CONFIG.products.removeAds);
    purchaseDiagnostics.log("purchaseNoAds CLICK", { productTag: CONFIG.products.removeAds, platform: platform.gamePushPlatform });
    showPurchaseProcessing("noads");
    const purchase = await runExternalInteraction(
  ...
  function showModal(title, content, locked = false) {
    ...
    pauseGameClock();
    modal.classList.remove("purchase-processing-modal");
  ...
  function bindEvents() {
    ...
    window.addEventListener("blur", () => {
      purchaseDiagnostics.log("WINDOW blur");
      purchaseDiagState("STATE on window blur");
      pauseGameClock();
      if (drag) {
        clearDragVisual();
        render();
    ...
    const recoverAfterPlatformUi = (source = "unknown") => {
      purchaseDiagnostics.log(`RECOVER after platform UI: ${source}`);
      purchaseDiagState(`STATE before recover ${source}`);
    ...
    document.addEventListener("visibilitychange", () => {
      purchaseDiagnostics.log(`DOCUMENT visibilitychange -> ${document.visibilityState}`);
      purchaseDiagState(`STATE visibility=${document.visibilityState}`);
      if (document.visibilityState === "hidden") {
        pauseGameClock();
        saveNow();
      } else {
        recoverAfterPlatformUi("visibility-visible");
        resumeGameClock();
      }
    });
    window.addEventListener("focus", () => {
      purchaseDiagnostics.log("WINDOW focus");
      purchaseDiagState("STATE on window focus");
      recoverAfterPlatformUi("window-focus");
      resumeGameClock();
    });
  ...
  async function bootstrap() {
    ...
    applySkin();
    syncViewportVariables();
    // Open the main menu before the board gets its first rendered frame. This prevents
    // the gameplay table from flashing for 1–2 seconds while Game Ready/banner SDK calls finish.
    showMainMenu();
    render();
    requestViewportReflow(true);
