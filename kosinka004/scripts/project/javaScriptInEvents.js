...
// Версия: GamePush v1.49.7 — deck_frost3/deck_amber3 + обновление покупки ОК при возврате фокуса без перезапуска.
(async () => {
  "use strict";
  ...
  const CONFIG = {
    ...
    products: {
      ...
      frostDeck: "deck_frost3",
      amberDeck: "deck_amber3"
    },
  ...
  const DECKS = {
    ...
    frost3: { titleKey: "frost", kind: "purchase", product: "frostDeck", theme: { tableA: "#5f8fa8", tableB: "#183a52", accent: "#dff7ff", backA: "#b9ecff", backB: "#4f88b8", frontA: "#ffffff", frontB: "#e8f7ff" } },
    amber3: { titleKey: "amber", kind: "purchase", product: "amberDeck", theme: { tableA: "#8a5a1f", tableB: "#3d220d", accent: "#ffd77a", backA: "#d9902f", backB: "#7a4215", frontA: "#fffaf0", frontB: "#f6e4bf" } }
  };
  ...
  const platform = {
    ...
    async syncEntitlements(targetProfile) {
      ...
      const productToSkin = {
        ...
        [CONFIG.products.frostDeck]: "frost3",
        [CONFIG.products.amberDeck]: "amber3"
      };
    ...
    async purchase(productTag) {
      try {
        ...
        if (this.kind === "gamepush" && this.gp?.payments?.purchase) {
          ...
          // Standard GamePush checkout: no custom OK waiters, polling loops or purchase-event handlers.
          ...
          return { ok: true, code: "success", productTag: resolvedTag, requestedProductTag: productTag, result: receipt };
        }
      } catch (error) {
  ...
  let shopTab = "reward";
  // The OK payment window may finish before GamePush refreshes payments.purchases.
  // Keep the exact clicked purchase around so a focus/visibility return can re-check it
  // without requiring a page reload or blocking the UI with a custom waiting overlay.
  let pendingPurchaseIntent = null;
  let pendingPurchaseIntentToken = 0;
  ...
  function openStore(tab = shopTab) {
    ...
    const paid = ["royal", "noir", "ruby", "gold", "cosmos", "frost3", "amber3"];
  ...
  function beginPendingPurchaseIntent(kind, productTag, skinId = "") {
    const intent = {
      token: ++pendingPurchaseIntentToken,
      kind,
      productTag,
      skinId,
      startedAt: Date.now(),
  ...
  function clearPendingPurchaseIntent(intent) {
    if (!intent || pendingPurchaseIntent === intent) pendingPurchaseIntent = null;
  }

  async function confirmPendingPurchaseFromCatalogue(intent = pendingPurchaseIntent) {
    if (!intent || intent.confirmed || intent.checking || pendingPurchaseIntent !== intent) return false;
    if (platform.kind !== "gamepush") return false;
    intent.checking = true;
    try {
      await platform.refreshCommerce({ preserveSession: true, forceFetch: true });
      if (!platform.hasPurchase(intent.productTag)) return false;

      intent.confirmed = true;
      clearPendingPurchaseIntent(intent);
      if (intent.kind === "skin") {
        await handlePlatformPurchaseSuccess(intent.productTag, intent.skinId);
        const deck = DECKS[intent.skinId];
        if (deck) toast(t("deckBought", { title: deckTitle(intent.skinId) }));
      } else if (intent.kind === "noads") {
        await handlePlatformPurchaseSuccess(intent.productTag);
        toast(t("adsDisabled"));
      }
      return true;
    } catch (error) {
      console.warn("[Kosynka] Purchase refresh after platform return failed", error);
      return false;
    } finally {
  ...
  function schedulePendingPurchaseRefresh() {
    const intent = pendingPurchaseIntent;
    if (!intent || intent.confirmed || platform.kind !== "gamepush") return;
    // OK can update the purchase cache shortly after its payment window closes.
    // These checks are bounded and invisible: no infinite polling and no custom processing modal.
    for (const delay of [0, 450, 1100, 2200, 4000, 6500]) {
      setTimeout(() => {
        if (pendingPurchaseIntent !== intent || intent.confirmed) return;
        void confirmPendingPurchaseFromCatalogue(intent);
      }, delay);
    }
  }
  ...
  async function purchaseSkin(id) {
    ...
    // Remember the exact tile before opening the native checkout. If OK returns focus while
    // purchase() is still unresolved, the focus handler can independently confirm this purchase.
    const intent = beginPendingPurchaseIntent("skin", productTag, id);
    const purchase = await runExternalInteraction(
      () => platform.purchase(productTag),
      { ok: false, code: "unavailable" }
    );

    // A focus/visibility refresh may already have confirmed the purchase while the Promise was pending.
  ...
  async function purchaseNoAds() {
    ...
    const intent = beginPendingPurchaseIntent("noads", CONFIG.products.removeAds);
    const purchase = await runExternalInteraction(
  ...
  function bindEvents() {
    ...
    document.addEventListener("visibilitychange", () => {
      if (document.visibilityState === "hidden") {
        ...
        resumeGameClock();
        schedulePendingPurchaseRefresh();
      }
    });
