/*
  Snake: King of the Arena — Construct 3 JS runtime (ads/reward/leaderboard fixes v0.29)
  ---------------------------------------------------------------
...
function getStartupLanguageHint() {
  // Before the platform SDK finishes loading, use only cheap signals. The real
  // GamePush/Yandex/VK/OK language is applied before the first menu is rendered.
  try {
...
class PlatformBridge {
  ...
  async showInterstitial() {
    ...
    try {
      ...
      if (gp?.ads?.showFullscreen) {
        ...
        const result = await this.promiseWithTimeout(gp.ads.showFullscreen({ showCountdownOverlay: true }), 65000, { success: false, timeout: true });
  ...
  isMobileLikeViewport() {
    ...
    return coarse || /android|iphone|ipad|ipod|mobile/.test(ua) || Math.min(width, height) <= 620;
  }

  shouldShowSocialSticky() {
    // VK/OK: show sticky on desktop/tablet windows and on phones only in portrait.
    // Phone landscape is deliberately disabled so the banner cannot cover gameplay/UI.
    if (this.platform !== "vk" && this.platform !== "ok") return false;
    const { width, height } = this.getViewportSize();
    if (!this.isMobileLikeViewport()) return true;
    return height >= width;
  }
  ...
  async refreshBannerForViewport() {
    return await this.setBannerVisible(this.bannerRequestedVisible, true);
  }

  async setBannerVisible(visible, force = false) {
    // Policy by platform:
    ...
    const desired = this.getDesiredBannerVisibility(visible);
    this.syncBannerSafeArea(desired);
    if (!force && this.bannerVisible === desired) return;
    ...
    try {
      ...
      const gp = this.gp ?? this.getGamePush();
      if (!gp?.ads) return;
      if (desired) await gp.ads.showSticky?.();
