...
class SlitherArena {
  ...
  getSkinPreviewSvg(skin, title = "") {
    ...
    const accents = accentPatterns.has(skin.pattern)
      ? `<g fill="${accent}" opacity=".86"><circle cx="70" cy="54" r="2.5"/><circle cx="105" cy="43" r="2.5"/><circle cx="140" cy="40" r="2.5"/><circle cx="174" cy="47" r="2.5"/></g>`
      : "";
    const crown = skin.pattern === "crown"
      ? `<path d="M190 31 L197 15 L204 26 L213 14 L219 31 Z" fill="#ffd43b" stroke="rgba(255,255,255,.55)" stroke-width="1"/>`
      : "";
    return `<svg class="sa-skin-preview" data-skin-preview="${safeId}" viewBox="0 0 240 96" preserveAspectRatio="xMidYMid slice" role="img" aria-label="${this.escapeHtml(title)}" xmlns="http://www.w3.org/2000/svg">
      ...
      <path d="M31 61 C54 51 67 72 88 56 S124 35 145 45 S172 64 196 48" fill="none" stroke="${primary}" stroke-width="19" stroke-linecap="round" stroke-linejoin="round" filter="url(#skin-glow-${safeId})" opacity=".96"/>
      <path d="M31 61 C54 51 67 72 88 56 S124 35 145 45 S172 64 196 48" fill="none" stroke="${secondary}" stroke-width="6" stroke-linecap="round" stroke-linejoin="round" opacity=".78"${dash}/>
      ${accents}
