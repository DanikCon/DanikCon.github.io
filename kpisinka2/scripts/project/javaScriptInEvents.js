...
// Версия: GamePush v1.45.5 — надёжное локальное восстановление, выбор свежего local/cloud сохранения, облачная синхронизация раз в 15 секунд.
(async () => {
  "use strict";
  ...
  const CONFIG = {
    ...
    cloudSyncIntervalMs: 15000, // Local state is written immediately; changed state is sent to the platform cloud at most once per 15 seconds.
  ...
  const SAVE_BACKUP_KEY = "kosynka_c3_local_v3_backup";
  ...
  const UI_ICONS = Object.freeze({
    ...
    invite: '<svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="9" cy="8" r="3"/><path d="M3.8 19c.5-3.2 2.4-5 5.2-5s4.7 1.8 5.2 5M18 7v6M15 10h6"/></svg>',
    community: '<svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="8" cy="9" r="3"/><circle cx="17" cy="10" r="2.4"/><path d="M2.8 19c.5-3.3 2.4-5.1 5.2-5.1s4.7 1.8 5.2 5.1M14 15c2.9-.7 5.6.7 6.4 3.8"/></svg>',
  ...
  const I18N = Object.freeze({
    ...
    pt: {
      ...
      hintsTitle:"Dicas", hintsCap:"Você já tem o máximo: <b>{cap}</b>. Use uma dica em uma partida e depois poderá recarregar.", hintsAvailable:"Você tem <b>{balance}</b> de {cap}. Cada nova partida redefine o saldo para <b>3</b>.", hintsEmpty:"As dicas grátis acabaram. Assista a um anúncio curto e ganhe <b>+{amount}</b> dicas.", hintsInfo:"A dica não faz a jogada por você: ela só destaca o próximo passo mais útil.", watchAdPlus:"Assistir anúncio em vídeo — +{amount}", backToGame:"Voltar ao jogo", notNow:"Agora não", hintsEnough:"Você já tem dicas suficientes", openingAd:"Abrindo anúncio…", adNoReward:"O anúncio não foi concluído — nenhuma recompensa recebida", hintsReceived:"Dicas recebidas: +{amount}",
  ...
  function parseStoredPayload(raw) {
    ...
    if (!("game" in value) && !("profile" in value)) return null;
    return value;
  }

  function saveTimestamp(payload) {
    const value = Number(payload?.savedAt);
    return Number.isFinite(value) && value > 0 ? value : 0;
  }

  function localRead() {
    const candidates = [];
    const keys = [SAVE_KEY, SAVE_BACKUP_KEY, PREVIOUS_SAVE_KEY, LEGACY_SAVE_KEY];
    for (let priority = 0; priority < keys.length; priority++) {
      const key = keys[priority];
      try {
        const payload = parseStoredPayload(localStorage.getItem(key));
        if (payload) candidates.push({ payload, priority });
      } catch (error) {
        console.warn("[Kosynka] Не удалось прочитать локальное сохранение", error);
      }
    }
    if (!candidates.length) return null;
    candidates.sort((a, b) => saveTimestamp(b.payload) - saveTimestamp(a.payload) || a.priority - b.priority);
  ...
  function localWrite(payload) {
    ...
    try {
      ...
      const now = Date.now();
      // Keep a recoverable older copy, but do not duplicate a large synchronous write after every move.
      if (previous && previous !== text && (!localStorage.getItem(SAVE_BACKUP_KEY) || now - lastLocalBackupAt >= CONFIG.cloudSyncIntervalMs)) {
        localStorage.setItem(SAVE_BACKUP_KEY, previous);
        lastLocalBackupAt = now;
      }
      localStorage.setItem(SAVE_KEY, text);
      if (localStorage.getItem(SAVE_KEY) !== text) throw new Error("localStorage verification failed");
      return true;
    } catch (error) {
  ...
  const platform = {
    ...
    resolveGamePushPlatform() {
      ...
      const candidates = [
        ...
        query.get("vk_client"), query.get("_platform"), query.get("platform"),
      ...
      // Check OK before VK: OK runs inside the VK Mini Apps launcher and exposes vk_client=ok,
      // while other SDK fields can still contain VK launcher identifiers.
    ...
    arePurchasesDisabled() {
      return this.kind === "gamepush" && this.gamePushPlatform === "ok";
    },
    ...
    async disableGamePushAutoSync() {
      const disable = this.gp?.player?.disableAutoSync;
      if (typeof disable !== "function") return;
      try {
        const result = disable.call(this.gp.player, { storage: "cloud" });
        if (result && typeof result.then === "function") await result;
      } catch (error) {
        // Older SDK builds may not expose configurable autosync. Our own queue still prevents
        // parallel explicit sync() calls and remains the only caller in this project.
        console.warn("[Kosynka] GamePush autosync could not be disabled", error);
      }
    },
    ...
    async initGamePush() {
      try {
        ...
        if (existing) {
          ...
          await this.disableGamePushAutoSync();
    ...
    async load() {
      // Read local and cloud independently. Returning the cloud value first used to overwrite a
      // newer local game after reload whenever the 15-second cloud timer had not fired yet.
      ...
      return { local, cloud: parseStoredPayload(raw) };
    },

    async save(payload) {
      // Local persistence is handled synchronously by the save queue before this adapter is called.
    ...
    resolveActivePurchaseTag(...args) {
      ...
      if (tag) return tag;
      // GamePush documents `purchase` as a SUCCESS event. Some host adapters emit it with an empty
      // or platform-specific payload. Because the game serializes external interactions, there can
      // only be one checkout initiated by this UI at a time; the single pending tag is therefore the
      // exact product that succeeded. This is the crucial fallback that avoids requiring a reload.
    ...
    isVkOrOk() {
      return this.kind === "gamepush" && (this.gamePushPlatform === "vk" || this.gamePushPlatform === "ok");
    },

    getCommunityUrl() {
      return this.isVkOrOk() ? String(CONFIG.communityUrls?.[this.gamePushPlatform] || "") : "";
    },

    canInviteFriend() {
      return this.isVkOrOk() && typeof this.gp?.socials?.invite === "function";
    },

    async inviteFriend() {
      if (!this.canInviteFriend()) return false;
      try {
        // GamePush selects the native VK/OK invite UI when available and its own share fallback otherwise.
        await this.gp.socials.invite({ text: t("inviteText") });
        return true;
      } catch (error) {
        console.warn("[Kosynka] Friend invite unavailable", error);
        return false;
      }
    },
    ...
    async purchase(productTag) {
      try {
        if (this.arePurchasesDisabled()) return { ok: false, code: "unavailable" };
        ...
        if (this.kind === "gamepush" && this.gp?.payments?.purchase) {
          ...
          // Keep the purchase flow identical to the working snake2 project:
          // await the GamePush checkout itself, then immediately trust its successful receipt.
          // Do not wait for payments.purchases, fetchProducts(), focus recovery or a separate event.
          ...
          // Same success test used by snake2.c3p.
          const success = receipt === true || receipt?.success === true || Boolean(receipt?.purchase || receipt?.product);
          if (success) {
            ...
            return { ok: true, code: "success", productTag: resolvedTag, result: receipt };
          }

          // Same fallback as snake2: ask GamePush whether this permanent product is already owned.
          let owned = false;
          try {
            const objectResult = this.gp.payments.has?.({ tag: productTag });
            const stringResult = this.gp.payments.has?.(productTag);
            owned = objectResult === true || stringResult === true;
          } catch (_) { /* A missing/unsupported has() must not crash checkout handling. */ }
          if (owned) {
            this.purchasedProducts.add(productTag);
            return { ok: true, code: "success", productTag, result: receipt };
          }

          return { ok: false, code: "unconfirmed", result: receipt };
        }
      } catch (error) {
  ...
  let cloudSyncForcePending = false;
  ...
  function mergeLoadedProfiles(primaryRaw, secondaryRaw) {
    const primary = validProfile(primaryRaw);
    const secondary = validProfile(secondaryRaw);
    const merged = { ...primary };
    merged.unlockedSkins = Array.from(new Set([...(primary.unlockedSkins || []), ...(secondary.unlockedSkins || [])]));
    merged.achievements = Array.from(new Set([...(primary.achievements || []), ...(secondary.achievements || [])]));
    merged.noAds = Boolean(primary.noAds || secondary.noAds);
    merged.games = Math.max(primary.games || 0, secondary.games || 0);
    merged.wins = Math.max(primary.wins || 0, secondary.wins || 0);
    merged.bestScore = Math.max(primary.bestScore || 0, secondary.bestScore || 0);
    merged.hintCount = Math.max(primary.hintCount || 0, secondary.hintCount || 0);
    merged.lastInterstitialAt = Math.max(primary.lastInterstitialAt || 0, secondary.lastInterstitialAt || 0);
    const bestTimes = [primary.bestWinSeconds, secondary.bestWinSeconds].map(Number).filter(value => value > 0);
    merged.bestWinSeconds = bestTimes.length ? Math.min(...bestTimes) : 0;
    // Mutable settings and consumable hint balance come from the newest save only.
    return validProfile(merged);
  }

  function resolveLoadedSaves(localPayload, cloudPayload) {
    const local = parseStoredPayload(localPayload);
    const cloud = parseStoredPayload(cloudPayload);
    const localTime = saveTimestamp(local);
    const cloudTime = saveTimestamp(cloud);
    let source = "none";
    let primary = null;
    let secondary = null;

    if (local && cloud) {
      if (localTime > cloudTime) source = "local";
      else if (cloudTime > localTime) source = "cloud";
      else {
        const localGame = normalizeGame(local.game);
        const cloudGame = normalizeGame(cloud.game);
        const localUnfinished = Boolean(localGame && !localGame.finished);
        const cloudUnfinished = Boolean(cloudGame && !cloudGame.finished);
        source = localUnfinished !== cloudUnfinished ? (localUnfinished ? "local" : "cloud") : "local";
      }
      primary = source === "local" ? local : cloud;
      secondary = source === "local" ? cloud : local;
    } else if (local) {
      source = "local";
      primary = local;
    } else if (cloud) {
    ...
    if (!primary) return { payload: null, source, localSignature: "", cloudSignature: "" };
    const primaryGame = normalizeGame(primary.game);
    const secondaryGame = normalizeGame(secondary?.game);
    const chosenRawGame = primaryGame ? primary.game : (secondaryGame ? secondary.game : null);
    const payload = {
      schema: 3,
      savedAt: Math.max(localTime, cloudTime, Date.now()),
      game: chosenRawGame ? deepCopy(chosenRawGame) : null,
      profile: mergeLoadedProfiles(primary.profile, secondary?.profile)
    };
    return {
      payload,
      source,
      localSignature: local ? payloadSignature(local) : "",
      cloudSignature: cloud ? payloadSignature(cloud) : ""
    };
  }
  ...
  async function startFreshGame() {
    ...
    // Every new deal starts with exactly three hints, regardless of the previous balance.
    ...
    closeModal(true);
    afterGameChange(false);
    // A new deal is a durable checkpoint and is sent to cloud immediately.
    void saveNow();
  ...
  function payloadSignature(payload) {
    // savedAt is deliberately excluded: otherwise an unchanged game would always look new.
    return JSON.stringify({
      schema: payload?.schema || 3,
  ...
  function scheduleCloudSync() {
    if (!cloudStateDirty || cloudSyncTimer || cloudSyncInFlight) return;
  ...
  async function flushCloudSave(force = false) {
    ...
    if (signature === lastCloudSnapshot && !cloudSyncInFlight) {
      cloudStateDirty = false;
      return true;
    }
    cloudStateDirty = true;

    // Never start a second player.sync() while the first one is pending. Remember whether the
    // next flush must happen immediately (purchase/result/background) or can wait 15 seconds.
    if (cloudSyncInFlight) {
      cloudSyncPending = true;
      cloudSyncForcePending = cloudSyncForcePending || force;
      return cloudSyncPromise || false;
    }

    cloudSyncInFlight = true;
    cloudStateDirty = false;
    cloudSyncPromise = Promise.resolve(platform.save(payload));
    let synced = false;
    try {
      synced = (await cloudSyncPromise) !== false;
      if (synced) lastCloudSnapshot = signature;
      else cloudStateDirty = true;
    } finally {
      cloudSyncInFlight = false;
      cloudSyncPromise = null;
      const shouldRepeat = cloudSyncPending || cloudStateDirty;
      const repeatImmediately = cloudSyncForcePending;
      cloudSyncPending = false;
      cloudSyncForcePending = false;
      if (shouldRepeat) {
        if (repeatImmediately) queueMicrotask(() => { void flushCloudSave(true); });
  ...
  function scheduleSave() {
    ...
    const payload = buildPayload();
    // Every move is committed locally immediately. Only the network part is throttled.
    localWrite(payload);
    const signature = payloadSignature(payload);
    if (signature === lastCloudSnapshot && !cloudSyncInFlight) {
      cloudStateDirty = false;
    ...
    cloudStateDirty = true;
    if (cloudSyncInFlight) cloudSyncPending = true;
    scheduleCloudSync();
  }

  async function saveNow() {
    if (!game || !profile) return false;
    // flushCloudSave writes the latest state locally before any network await, then either sends it
    // immediately or queues one forced follow-up when a previous sync is still in flight.
    cloudStateDirty = true;
    return flushCloudSave(true);
  }
  ...
  async function inviteFriend() {
    if (!platform.canInviteFriend()) {
      toast(t("inviteUnavailable"));
      return;
    }
    const opened = await runExternalInteraction(() => platform.inviteFriend(), false);
    if (!opened) toast(t("inviteUnavailable"));
  }

  function showMainMenu() {
    clearDragVisual();
    const canContinue = hasSavedGame && game && !game.finished;
    const communityUrl = platform.getCommunityUrl();
    showModal(t("brand"), `
      ...
      <div class="menu-actions">
        ...
        <div class="modal-grid menu-grid">
          ...
          ${platform.canInviteFriend() ? `<button class="modal-btn secondary social-menu-btn" data-action="invite-friend">${uiIcon("invite", "button-icon")}<span>${t("inviteFriend")}</span></button>` : ""}
          ${communityUrl ? `<a class="modal-btn secondary social-link social-menu-btn" href="${escapeHtml(communityUrl)}" target="_blank" rel="noopener noreferrer">${uiIcon("community", "button-icon")}<span>${t("joinGroup")}</span></a>` : ""}
  ...
  function leaderboardRowsHtml(result) {
    ...
    return rows.slice(0, CONFIG.leaderboardLimit).map((player, index) => {
      ...
      return `<div class="achievement-row leaderboard-row"><span class="achievement-mark">${position}</span><div><strong title="${escapeHtml(name)}">${escapeHtml(name)}</strong><small>${escapeHtml(t("leaderboardWins"))}: <b>${wins}</b></small></div></div>`;
    }).join("");
  }
  ...
  function openStore(tab = shopTab) {
    ...
    const purchasesDisabled = platform.arePurchasesDisabled();
    const rewardIds = platform.kind === "playgama" ? [...rewardBase, ...paid] : rewardBase;
    const allowedTabs = platform.kind === "playgama" || purchasesDisabled ? ["reward", "achievement"] : ["reward", "achievement", "purchase"];
    ...
    showModal(t("store"), `
      <div class="shop-tabs">
        ...
        ${platform.kind === "playgama" || purchasesDisabled ? "" : `<button class="shop-tab ${shopTab === "purchase" ? "active" : ""}" data-action="shop-tab" data-tab="purchase">${t("storePurchases")}</button>`}
      </div>
  ...
  async function handlePlatformPurchaseSuccess(productTag) {
    ...
    localWrite(payload);
    if (changed) {
      // Purchases are durable checkpoints. Use the same single-flight queue instead of bypassing it.
      void saveNow();
    } else {
      scheduleSave();
    }
  ...
  async function purchaseSkin(id) {
    const deck = DECKS[id];
    if (platform.arePurchasesDisabled()) return;
    ...
    handlePlatformPurchaseSuccess(productTag);
    openStore("purchase");
  ...
  async function purchaseNoAds() {
    ...
    handlePlatformPurchaseSuccess(CONFIG.products.removeAds);
    openStore("purchase");
    toast(t("adsDisabled"));
  }
  ...
  function showModal(title, content, locked = false, cardClass = "") {
    ...
    pauseGameClock();
    modal.innerHTML = `<div class="modal-backdrop"></div><section class="modal-card${safeCardClass ? ` ${safeCardClass}` : ""}" role="dialog" aria-modal="true" aria-label="${escapeHtml(title)}">
  ...
  function buildApp() {
    ...
    style.textContent = `
      ...
      .leaderboard-modal{width:min(590px,100%);overflow:hidden;padding:clamp(11px,2.2vh,20px)}.leaderboard-modal h2{margin-bottom:clamp(4px,1vh,8px)}.leaderboard-modal .modal-content{min-height:0}.leaderboard-modal .modal-lead{margin-bottom:clamp(4px,1vh,8px);font-size:clamp(10px,1.7vh,13px)}#k-leaderboard-content{gap:clamp(2px,.65vh,5px)}#k-leaderboard-content .achievement-row{min-height:26px;padding:clamp(3px,.65vh,7px) clamp(6px,1.5vw,10px);gap:7px;opacity:1}#k-leaderboard-content .achievement-mark{width:clamp(20px,3.8vh,25px);height:clamp(20px,3.8vh,25px);font-size:clamp(9px,1.6vh,12px)}#k-leaderboard-content .achievement-row>div{display:flex;align-items:baseline;justify-content:space-between;gap:8px;flex:1;min-width:0}#k-leaderboard-content .achievement-row strong{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:clamp(10px,1.75vh,12px)}#k-leaderboard-content .achievement-row small{flex:0 0 auto;margin-top:0;font-size:clamp(8px,1.45vh,10px);white-space:nowrap}.leaderboard-modal .modal-btn.full{min-height:clamp(32px,5.7vh,40px);margin-top:clamp(4px,.9vh,7px);padding:6px 9px}
      ...
      @media (max-height:620px){.leaderboard-modal{padding:9px 10px}.leaderboard-modal h2{font-size:19px;margin-bottom:3px}.leaderboard-modal .modal-lead{margin-bottom:3px;font-size:9px;line-height:1.2}#k-leaderboard-content{gap:2px}#k-leaderboard-content .achievement-row{min-height:22px;padding:2px 5px}#k-leaderboard-content .achievement-mark{width:19px;height:19px;font-size:9px}#k-leaderboard-content .achievement-row strong{font-size:9.5px}#k-leaderboard-content .achievement-row small{font-size:8px}.leaderboard-modal .modal-btn.full{min-height:30px;margin-top:3px;padding:4px 7px;font-size:10px}}
  ...
  function bindEvents() {
    ...
    app.addEventListener("click", async event => {
      ...
      if (button) {
        ...
        else if (action === "invite-friend") await inviteFriend();
  ...
  async function bootstrap() {
    ...
    const resolvedSave = resolveLoadedSaves(loaded?.local, loaded?.cloud);
    const saved = resolvedSave.payload;
    ...
    // Always mirror the selected state locally. The cloud baseline remains the actual cloud value,
    // so a newer local game is uploaded on the next 15-second flush instead of being treated as synced.
    const startupPayload = buildPayload();
    localWrite(startupPayload);
    lastCloudSnapshot = resolvedSave.cloudSignature || "";
    const startupSignature = payloadSignature(startupPayload);
    cloudStateDirty = entitlementsChanged || noAdsSkinsChanged || startupSignature !== lastCloudSnapshot;
    if (cloudStateDirty) scheduleCloudSync();
    console.info("[Kosynka Save] loaded", {
      source: resolvedSave.source,
      localSavedAt: saveTimestamp(loaded?.local),
      cloudSavedAt: saveTimestamp(loaded?.cloud),
      cloudUpdatePending: cloudStateDirty
    });
