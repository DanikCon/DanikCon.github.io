/*
  Snake: King of the Arena — Construct 3 JS runtime (ads/reward/leaderboard fixes v0.28)
  ---------------------------------------------------------------
...
const CFG = Object.freeze({
  ...
  BANNER_MODE: "platform-policy", // Yandex sticky stays on; VK/OK/Playgama never request one
...
const PLATFORM_SDK = Object.freeze({
  ...
  // Fallback is a compact CSS badge so the shop page never breaks because of an image URL.
  YAN_ICON_URL: "",
  SDK_WAIT_MS: 5200,
...
class PlatformBridge {
  ...
  async showGamePushRewarded(gp) {
    const ads = gp?.ads;
    const show = ads?.showRewarded ?? ads?.showRewardedVideo;
    if (typeof show !== "function") {
      this.setAdOutcome("rewarded", "unavailable");
      return false;
    }
    const available = ads.isRewardedAvailable ?? ads.isRewardedVideoAvailable ?? true;
    if (available === false) {
      this.setAdOutcome("rewarded", "unavailable");
      return false;
    }
    ...
    return await new Promise((resolve) => {
      ...
      let timeoutId = 0;
      const cleanup = () => {
        for (const [event, handler] of [["rewarded:reward", onReward], ["rewarded:close", onClose], ["rewarded:finish", onReward], ["rewarded:error", onError]]) {
      ...
      const onClose = (success) => {
        closeSeen = true;
        // GamePush passes success=true to rewarded:close when the video was watched.
      ...
      try {
        ads.on?.("rewarded:reward", onReward);
        ads.on?.("rewarded:close", onClose);
        ads.on?.("rewarded:finish", onReward);
        ads.on?.("rewarded:error", onError);
        const result = show.call(ads);
        Promise.resolve(result).then((value) => {
          const outcome = this.inferRewardOutcome(value, value === false ? "closed" : "closed");
          if (outcome === "completed") finish("completed");
          else if (value === false || outcome === "closed") {
            // Some webviews resolve the promise before the rewarded:reward event.
            setTimeout(() => {
  ...
  async showRewarded(reason) {
    ...
    try {
      return await this.rewardAdPromise;
    } finally {
      this.rewardAdPromise = null;
  ...
  async setBannerVisible(visible) {
    ...
    // - VK/OK: no sticky banner requested; only fullscreen/rewarded are used.
    // - Playgama: banner remains disabled to protect compact mobile UI.
    const yandexSticky = this.platform === "yandex";
    const desired = yandexSticky ? true : !!visible;
    if (this.bannerVisible === desired) return;
    this.bannerVisible = desired;
    if (this.isPlaygama || this.platform === "vk" || this.platform === "ok") return;
    try {
      ...
      const gp = this.gp ?? this.getGamePush();
      if (!gp?.ads) return;
      if (yandexSticky || desired) await gp.ads.showSticky?.();
  ...
  openCommunityUrl() {
    ...
    if (!url) return false;
    try {
      // This method is called directly from a button tap/click. Opening it before
      // any awaited SDK work preserves the user gesture in VK/OK webviews and Safari.
      const opened = globalThis.open?.(url, "_blank", "noopener,noreferrer");
      if (opened) {
        try { opened.opener = null; } catch (_error) { /* cross-window guard */ }
        return true;
      }
      // Some embedded webviews disallow a new tab. They can still navigate to the group.
  ...
  async joinCommunity() {
    ...
    // on optional GamePush social configuration.
    if (this.openCommunityUrl()) return true;
    await this.initialize();
    if (!this.isSocialPlatform) return false;
    try {
      const socials = (this.gp ?? this.getGamePush())?.socials;
...
class SlitherArena {
  constructor(runtime) {
    ...
    this.language = this.bridge.getLanguage();
  ...
  async start() {
      ...
      requestAnimationFrame(() => {
        ...
        if (this.mode === "shop") {
          ...
          if (this.rewardSkinBusy && this.bridge.lastRewardOutcome === "showing") {
            ...
            this.bridge.setAdOutcome("rewarded", "closed");
    ...
    // Render something instantly on every host. Yandex gets a compact boot screen while
    // its one-time preloader is requested; VK/OK still go straight to the local menu.
    this.progress = sanitizeProgress(this.bridge.readLocal() ?? defaultProgress());
    this.ensureDailyProgress();
    this.ensureMissionProgress();
    this.syncUi();
    if (this.bridge.platform === "yandex") this.showLoadingScreen();
    else this.showMenu();
  ...
  async continueAfterGameOver(destination) {
    if (this.mode !== "over" || this.postDeathActionPending) return;
    this.postDeathActionPending = true;
    for (const button of this.ui.overlay.querySelectorAll("[data-action='restart'],[data-action='menu']")) {
      button.disabled = true;
    }
    try {
      // Natural break: the result remains on screen until the player chooses
      // New run or Menu, and the 180-second gate is checked exactly here.
