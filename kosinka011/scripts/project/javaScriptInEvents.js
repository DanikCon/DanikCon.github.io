...
// Версия: GamePush v1.49.4c — deck_frost5/deck_amber5 + принудительный GamePush resume после оплаты OK.
(async () => {
  "use strict";
  ...
  const platform = {
    ...
    async confirmPurchase(productTag, { resumeSync = false, fullCycle = false, source = "confirm-purchase" } = {}) {
      ...
      if (resumeSync) await this.forceGamePushPurchaseResumeSync(source, { fullCycle });
      await this.refreshCommerce({ preserveSession: true, forceFetch: true });
      return this.hasPurchase(productTag);
    },
  ...
  async function purchaseSkin(id) {
    ...
    // Keep a blocking progress window visible for the whole platform checkout round-trip.
    // On OK this prevents the shop from looking "stuck" while the platform finalizes the purchase.
