...
// Версия: GamePush v1.49.8 — deck_frost6/deck_amber6 + чистая проверка факта покупки после GamePush timeout/reject.
(async () => {
  "use strict";
  ...
  const platform = {
    ...
    async purchase(productTag) {
      ...
      // GamePush on OK can reject with purchase_timeout even when OK has already completed the
      // transaction and payments.has()/payments.purchases already contain the product. Therefore
      // the Promise result is not authoritative on its own: after any failure we verify the actual
      // entitlement that is already present in the SDK cache before reporting a failed purchase.
      ...
      try {
        ...
        // Even an explicit-looking failure from the wrapper is overridden by a real entitlement.
        // This handles the observed OK case where the payment is in purchases but GamePush times out.
        if (explicitFailure) {
        ...
        console.warn("[Kosynka] GamePush purchase rejected; checking recorded entitlement", error);
        if (purchaseIsRecorded()) {
          return confirmedSuccess("entitlement-after-reject", undefined, error);
        }
