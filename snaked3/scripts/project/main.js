/*
  Snake: King of the Arena — Construct 3 JS runtime (ads/reward/leaderboard/stability fixes v0.33)
  ---------------------------------------------------------------
...
class PlatformBridge {
  ...
  async refreshPayments(gp) {
    try {
      if (typeof gp?.payments?.fetchProducts !== "function") return null;
      const result = await gp.payments.fetchProducts();
      this.registerProducts(result);
      return result;
    } catch (error) {
      // OK can need a short delay between the native payment callback and the SDK list update.
      console.warn("[SlitherArena] Could not refresh purchases after payment.", error);
      return null;
    }
  }

  async waitForPermanentPurchase(gp, sku, timeout = 8000) {
    const started = performance.now();
    let refreshed = false;
    while (performance.now() - started < timeout) {
      if (this.hasPermanentPurchase(gp, sku)) return true;
      if (!refreshed && performance.now() - started > 700) {
        refreshed = true;
    ...
    return this.hasPermanentPurchase(gp, sku);
  }
  ...
  async purchaseSkin(sku) {
    ...
    try {
      ...
      if (payments?.purchase) {
        ...
        const onClose = () => resolveCloseEvent({ type: "close" });
        const onError = (error) => console.warn("[SlitherArena] GamePush purchase error:", error);
        ...
        const purchaseCall = Promise.resolve()
          ...
          .then((receipt) => ({ type: "result", receipt }))
...
class SlitherArena {
  ...
  async start() {
    ...
    const restoreUiAfterExternalOverlay = () => {
      ...
      if (this.pendingPurchaseSkinId) void this.reconcileOwnedPurchaseSkins({ refresh: true });
    };
    ...
    void (async () => {
      ...
      if (!this.running && (this.mode === "menu" || this.mode === "boot")) {
        this.progress = cloud;
        await this.reconcileOwnedPurchaseSkins();
        this.ensureDailyProgress();
        this.ensureMissionProgress();
        this.syncUi();
        this.showMenu();
  ...
  async purchaseSkin(skin) {
    ...
    const restoredOnReturn = this.progress.unlockedSkins.includes(skin.id);
