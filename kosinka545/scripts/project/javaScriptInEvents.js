...
// Версия: GamePush v1.49.15 — OK-only immediate synchronous reload after confirmed purchase; VK purchase flow unchanged; deck_frost7/deck_amber7.
(async () => {
  "use strict";
  ...
  let okPurchaseReloadScheduled = false;

  function scheduleOkPurchaseReload() {
    // IMPORTANT: reload synchronously in the same task that confirms the entitlement.
    // OK hides/suspends the game iframe during checkout, so a setTimeout-based reload can be
    // frozen until the player manually switches tabs. VK never enters this branch.
    if (okPurchaseReloadScheduled || platform.kind !== "gamepush" || !isOkPurchaseContext()) return false;
    okPurchaseReloadScheduled = true;
    try {
  ...
  async function handlePlatformPurchaseSuccess(productTag, forcedSkinId = "") {
    ...
    // OK only: once the entitlement has been confirmed and persisted locally, restart immediately
    // in this same JavaScript task. Do not defer through a timer: OK suspends hidden iframe timers.
    // VK is intentionally excluded and continues through the existing in-place purchase flow.
    if (platform.kind === "gamepush" && isOkPurchaseContext()) {
      scheduleOkPurchaseReload();
      return;
    }
