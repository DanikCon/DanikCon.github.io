/*
  Snake: King of the Arena — Construct 3 JS runtime (ads/reward/leaderboard/OK payment return/recovery fixes v0.36 sticky-menu-fit)
  ---------------------------------------------------------------
...
const CFG = Object.freeze({
  ...
  BANNER_MODE: "sticky", // GamePush sticky is requested on menu screens; UI scales to the visible host viewport
  DEBUG_FAKE_ADS: false,
  DEBUG_FAKE_PURCHASES: false,
  SAVE_KEY: "slither_arena_progress_v1",
  PENDING_PURCHASE_KEY: "slither_arena_pending_purchase_v2",
  PAYMENT_TRACE_KEY: "slither_arena_payment_trace_v1"
});
...
function mergeProgressSnapshots(localRaw, cloudRaw) {
  const local = sanitizeProgress(localRaw);
  const cloud = sanitizeProgress(cloudRaw);
  const unlockedSkins = [...new Set([...(cloud.unlockedSkins || []), ...(local.unlockedSkins || [])])];
  const purchaseSkins = [...new Set([...(cloud.purchaseSkins || []), ...(local.purchaseSkins || [])])];
  let selectedSkin = cloud.selectedSkin;
  // A just-purchased local skin must survive an OK redirect even when the page was
  // reloaded before the cloud sync promise finished.
  if (purchaseSkins.includes(local.selectedSkin)) selectedSkin = local.selectedSkin;
  else if (!unlockedSkins.includes(selectedSkin) && unlockedSkins.includes(local.selectedSkin)) selectedSkin = local.selectedSkin;
  return sanitizeProgress({
    ...cloud,
    bestMass: Math.max(local.bestMass, cloud.bestMass),
    unlockedSkins,
    purchaseSkins,
    selectedSkin,
    rewardClaims: Math.max(local.rewardClaims, cloud.rewardClaims),
    totalMatches: Math.max(local.totalMatches, cloud.totalMatches),
    totalFood: Math.max(local.totalFood, cloud.totalFood),
    totalKills: Math.max(local.totalKills, cloud.totalKills),
    leagueStars: Math.max(local.leagueStars, cloud.leagueStars),
    missionStreak: Math.max(local.missionStreak, cloud.missionStreak)
  });
}
...
class PlatformBridge {
  ...
  tracePayment(stage, data = null) {
    let safeData = data;
    try {
      const seen = new WeakSet();
      safeData = JSON.parse(JSON.stringify(data, (_key, value) => {
      ...
      safeData = String(data?.message ?? data ?? "");
    }
    const entry = {
    ...
    try {
      const current = JSON.parse(globalThis.localStorage?.getItem(CFG.PAYMENT_TRACE_KEY) || "[]");
      const list = Array.isArray(current) ? current : [];
      list.push(entry);
      globalThis.localStorage?.setItem(CFG.PAYMENT_TRACE_KEY, JSON.stringify(list.slice(-80)));
    } catch (_error) { /* diagnostic storage is optional */ }
    console.info("[SlitherArena][Payment]", entry.stage, entry);
  }

  normalizePlayerPurchases(payload) {
    if (!payload) return [];
    if (Array.isArray(payload)) return payload;
    if (Array.isArray(payload?.playerPurchases)) return payload.playerPurchases;
    if (Array.isArray(payload?.purchases)) return payload.purchases;
    if (Array.isArray(payload?.player_purchases)) return payload.player_purchases;
    if (payload?.purchase && typeof payload.purchase === "object") return [payload.purchase];
    return [];
  }

  registerPlayerPurchases(payload) {
    const incoming = this.normalizePlayerPurchases(payload);
    const sdkPurchases = Array.isArray((this.gp ?? this.getGamePush())?.payments?.purchases)
      ? (this.gp ?? this.getGamePush()).payments.purchases
      : [];
    const merged = [...this.playerPurchases, ...sdkPurchases, ...incoming];
    const seen = new Set();
    this.playerPurchases = merged.filter((purchase) => {
      if (!purchase || typeof purchase !== "object") return false;
      const key = String(
        purchase.transactionId
        ?? purchase.transaction_id
        ?? purchase.id
        ?? `${purchase.productId ?? purchase.productID ?? purchase.tag ?? "unknown"}:${purchase.createdAt ?? purchase.created_at ?? ""}:${JSON.stringify(purchase.payload ?? {})}`
      );
  ...
  registerPaymentState(payload) {
    const products = this.registerProducts(payload);
    const purchases = this.registerPlayerPurchases(payload);
  ...
  async _initialize() {
    ...
    if (this.gp && this.isGamePushPlatform) {
      ...
      try {
        ...
        this.registerPaymentState(fetched);
        this.tracePayment("sdk-initial-purchases", {
          products: this.products.length,
          purchases: this.playerPurchases.length,
          sdkPurchases: Array.isArray(this.gp.payments?.purchases) ? this.gp.payments.purchases.length : 0
        });
      } catch (error) {
  ...
  globalThis.__SLITHER_BUILD__ = "27-landscape-no-overflow";
                this.gp = sdk;
                finish(sdk);
  ...
  shouldShowSocialSticky() {
    // Never close a social sticky just because the phone rotated. Some iOS VK/OK
    // webviews briefly report a very small visualViewport during rotation; using that
    // transient height made the banner flash for a second and then disappear.
    return this.platform === "vk" || this.platform === "ok";
  }
  ...
  notifyViewportChangedAfterSticky() {
    // Native social banners can resize the host iframe after showSticky() resolves.
    // Re-run the menu fitter several times because iOS Safari/OK applies that resize
    // over multiple animation frames.
    for (const delay of [0, 180, 520, 1100]) {
      globalThis.setTimeout?.(() => {
        try { globalThis.dispatchEvent?.(new Event("resize")); } catch (_error) { /* no-op */ }
      }, delay);
    }
  }
  ...
  async setBannerVisible(visible) {
    ...
    // Serialize show/close calls. VK/OK can dispatch resize and orientation events
    // while the SDK promise is still pending.
    this.bannerTask = this.bannerTask.catch(() => false).then(async () => {
      ...
      try {
        if (this.adapter?.setBannerVisible && (this.platform === "yandex" || this.platform === "vk" || this.platform === "ok")) {
          ...
          this.notifyViewportChangedAfterSticky();
          return desired;
        }
        ...
        } else if (typeof gp.ads.closeSticky === "function") {
          await gp.ads.closeSticky();
        }
  ...
  paymentPayloadMatchesSku(payload, sku) {
    if (!payload || !sku) return false;
    ...
    const configured = this.productByTag?.get?.(wanted);
    if (!configured) return false;
    return this.getProductKeys(configured).some((key) => keys.has(key));
  }

  hasPermanentPurchase(gp, sku) {
    const payments = gp?.payments;
    if (!payments || !sku) return false;

    // GamePush expects a product tag/id directly: gp.payments.has('VIP').
    // Passing { tag } returns false in some social-platform wrappers, notably OK.
    try {
      if (typeof payments.has === "function" && payments.has(sku) === true) return true;
    } catch (error) {
      console.warn("[SlitherArena] Could not check purchase ownership.", error);
    }
    ...
    return purchases.some((purchase) => this.paymentPayloadMatchesSku(purchase, sku));
  }

  async refreshPayments(gp) {
    try {
      if (typeof gp?.payments?.fetchProducts !== "function") {
        this.registerPlayerPurchases(gp?.payments?.purchases ?? []);
        return null;
      }
      const result = await gp.payments.fetchProducts();
      // fetchProducts() returns both products and playerPurchases. The previous build
      // registered only products, leaving OK ownership permanently stale after return.
      this.registerPaymentState(result);
      this.registerPlayerPurchases(gp?.payments?.purchases ?? []);
      this.tracePayment("payments-refreshed", {
        resultPurchases: this.normalizePlayerPurchases(result).length,
        cachedPurchases: this.playerPurchases.length,
        sdkPurchases: Array.isArray(gp?.payments?.purchases) ? gp.payments.purchases.length : 0
      });
      return result;
    } catch (error) {
      // OK can need a delay between the native payment callback and the SDK list update.
      this.tracePayment("payments-refresh-error", String(error?.message ?? error));
      console.warn("[SlitherArena] Could not refresh purchases after payment.", error);
      return null;
    }
  }

  async waitForPermanentPurchase(gp, sku, timeout = 22000) {
    const started = performance.now();
    let nextRefreshAt = 0;
    while (performance.now() - started < timeout) {
      if (this.hasPermanentPurchase(gp, sku)) {
        this.tracePayment("ownership-confirmed", { sku, afterMs: Math.round(performance.now() - started) });
        return true;
      }
      const elapsed = performance.now() - started;
      if (elapsed >= nextRefreshAt) {
        nextRefreshAt = elapsed + 1200;
        await this.refreshPayments(gp);
        if (this.hasPermanentPurchase(gp, sku)) {
          this.tracePayment("ownership-confirmed-after-refresh", { sku, afterMs: Math.round(performance.now() - started) });
          return true;
        }
      }
      await new Promise((resolve) => setTimeout(resolve, 300));
    }
    const owned = this.hasPermanentPurchase(gp, sku);
    this.tracePayment("ownership-timeout", { sku, owned, timeout });
    return owned;
  }
  ...
  async purchaseSkin(sku) {
    ...
    try {
      ...
      if (payments?.purchase) {
        if (payments.isAvailable === false) return false;
        if (this.hasPermanentPurchase(gp, sku)) return true;

        let resolvePurchaseEvent;
        let resolveCloseEvent;
        const purchaseEvent = new Promise((resolve) => { resolvePurchaseEvent = resolve; });
        const closeEvent = new Promise((resolve) => { resolveCloseEvent = resolve; });
        const onPurchase = (payload) => {
          this.registerPaymentState(payload);
          this.tracePayment("purchase-event", { sku, matches: this.paymentPayloadMatchesSku(payload, sku), payload });
          if (this.paymentPayloadMatchesSku(payload, sku)) resolvePurchaseEvent({ type: "purchase", payload });
        };
        const onClose = () => {
          this.tracePayment("payment-overlay-close", { sku });
          resolveCloseEvent({ type: "close" });
        };
        const onError = (error) => {
          this.tracePayment("purchase-error-event", { sku, error: String(error?.message ?? error) });
          console.warn("[SlitherArena] GamePush purchase error:", error);
        };

        try { payments.on?.("purchase", onPurchase); } catch (_error) { /* optional event API */ }
        try { payments.on?.("close", onClose); } catch (_error) { /* optional event API */ }
        try { payments.on?.("error:purchase", onError); } catch (_error) { /* optional event API */ }

        this.tracePayment("purchase-call", { sku, platform: this.platform });
        const purchaseCall = Promise.resolve()
          .then(() => payments.purchase({ tag: sku }))
          .then((receipt) => {
            this.registerPaymentState(receipt);
            this.tracePayment("purchase-promise-result", { sku, receipt });
            return { type: "result", receipt };
          })
          .catch((error) => ({ type: "error", error }));
        const timeoutSignal = new Promise((resolve) => setTimeout(() => resolve({ type: "timeout" }), 18000));

        const outcome = await Promise.race([purchaseCall, purchaseEvent, closeEvent, timeoutSignal]);

        try { payments.off?.("purchase", onPurchase); } catch (_error) { /* optional event API */ }
        try { payments.off?.("close", onClose); } catch (_error) { /* optional event API */ }
        try { payments.off?.("error:purchase", onError); } catch (_error) { /* optional event API */ }

        if (outcome?.type === "purchase") return true;
        if (outcome?.type === "result") {
          const receipt = outcome.receipt;
          // GamePush documents a successful promise as { product, purchase }. Product
          // metadata alone is not proof that OK's server callback completed.
          if (receipt?.purchase && this.paymentPayloadMatchesSku(receipt, sku)) return true;
          // Some OK wrappers return only `true` from the client callback. Treat it as
          // a reason to poll server ownership, not as final proof of delivery.
          if ((receipt === true || receipt?.success === true) && this.platform !== "ok") return true;
        }
        if (outcome?.type === "error") {
          console.warn("[SlitherArena] Purchase promise rejected.", outcome.error);
        }

        // On OK the native dialog may close before the GamePush callback has updated
        // the local purchases list. Refresh and briefly wait for permanent ownership.
        return await this.waitForPermanentPurchase(gp, sku);
      }
...
class SlitherArena {
  constructor(runtime) {
    ...
    this.pendingPurchase = this.readPendingPurchase();
    this.pendingPurchaseSkinId = this.pendingPurchase?.skinId ?? "";
    this.purchaseRecoveryRunning = false;
  ...
  readPendingPurchase() {
    try {
      const value = JSON.parse(globalThis.localStorage?.getItem(CFG.PENDING_PURCHASE_KEY) || "null");
      if (!value || typeof value !== "object") return null;
      const age = Date.now() - Number(value.startedAt || 0);
      if (!value.skinId || !value.sku || age < 0 || age > 24 * 60 * 60 * 1000) {
        globalThis.localStorage?.removeItem(CFG.PENDING_PURCHASE_KEY);
        return null;
      }
      return value;
    } catch (_error) {
  ...
  storePendingPurchase(skin) {
    const pending = { skinId: skin.id, sku: skin.sku, startedAt: Date.now(), platform: this.bridge.platform };
    this.pendingPurchase = pending;
    this.pendingPurchaseSkinId = skin.id;
    try { globalThis.localStorage?.setItem(CFG.PENDING_PURCHASE_KEY, JSON.stringify(pending)); } catch (_error) { /* optional */ }
    this.bridge.tracePayment("pending-purchase-saved", pending);
  }

  clearPendingPurchase(reason = "completed") {
    const previous = this.pendingPurchase ?? this.readPendingPurchase();
    this.pendingPurchase = null;
    this.pendingPurchaseSkinId = "";
    try { globalThis.localStorage?.removeItem(CFG.PENDING_PURCHASE_KEY); } catch (_error) { /* optional */ }
    this.bridge.tracePayment("pending-purchase-cleared", { reason, previous });
  }

  getPaymentReturnSignal(value = null) {
    const candidates = [];
    if (value != null) {
      try { candidates.push(typeof value === "string" ? value : JSON.stringify(value)); }
      catch (_error) { candidates.push(String(value)); }
    }
    try {
      const params = new URLSearchParams(globalThis.location?.search || "");
      candidates.push(params.get("custom_args") || "", params.get("payment") || "");
    } catch (_error) { /* no-op */ }
    for (const candidate of candidates) {
      const normalized = String(candidate || "").trim().toLowerCase();
      if (normalized === "ok" || normalized.includes("payment=ok")) return "ok";
      if (normalized === "cancel" || normalized === "error" || normalized.includes("payment=cancel") || normalized.includes("payment=error")) return "cancel";
    }
    return "";
  }

  async recoverPendingPurchase({ timeout = 24000, source = "return" } = {}) {
    if (this.purchaseRecoveryRunning) return false;
    const pending = this.pendingPurchase ?? this.readPendingPurchase();
    if (!pending) return false;
    const skin = SKINS.find((item) => item.id === pending.skinId && item.sku === pending.sku);
    if (!skin) {
      this.clearPendingPurchase("unknown-skin");
      return false;
    }
    this.pendingPurchase = pending;
    this.pendingPurchaseSkinId = skin.id;
    this.purchaseRecoveryRunning = true;
    this.bridge.tracePayment("pending-recovery-start", { source, pending });
    try {
      await this.bridge.initialize();
      const gp = this.bridge.gp ?? this.bridge.getGamePush?.();
      if (!gp?.payments) return false;
      const owned = await this.bridge.waitForPermanentPurchase(gp, skin.sku, timeout);
      ...
      this.clearPendingPurchase("ownership-confirmed");
      this.toast(this.t("toast.skinOpened", { name: this.getSkinTitle(skin) }));
      if (this.mode === "shop") this.showShop(this.shopPage);
      else this.syncUi();
      return true;
    } finally {
      this.purchaseRecoveryRunning = false;
  ...
  async start() {
    this.createDom();
    globalThis.__SLITHER_PAYMENT_TRACE__ = () => {
      try { return JSON.parse(globalThis.localStorage?.getItem(CFG.PAYMENT_TRACE_KEY) || "[]"); }
      catch (_error) { return []; }
    };
    ...
    const handleViewportResize = () => {
      const token = ++viewportFitToken;
      const refit = (renderPreviews = false) => {
        if (token !== viewportFitToken) return;
        this.resize();
        this.applyPlatformUi();
        this.updateControlHint();
        this.fitOverlayPanel(renderPreviews);
      };
      requestAnimationFrame(() => refit(true));
      // iOS VK/OK update iframe size, browser chrome and the platform banner in
      // several separate layout passes after rotation. Refit after each stable step.
      for (const delay of [70, 180, 360, 720]) setTimeout(() => refit(false), delay);
    };
    this.scheduleViewportFit = handleViewportResize;
    globalThis.addEventListener?.("resize", handleViewportResize);
    globalThis.addEventListener?.("orientationchange", handleViewportResize);
    ...
    if (typeof globalThis.ResizeObserver === "function") {
      this.viewportResizeObserver = new ResizeObserver(() => handleViewportResize());
      this.viewportResizeObserver.observe(this.root);
      this.viewportResizeObserver.observe(this.ui.overlay);
    }
    const restoreUiAfterExternalOverlay = () => {
      ...
      if (this.pendingPurchaseSkinId || this.readPendingPurchase()) {
        void this.recoverPendingPurchase({ source: "visibility/pageshow" });
      }
    };
    ...
    globalThis.addEventListener?.("message", (event) => {
      const signal = this.getPaymentReturnSignal(event?.data);
      if (!signal) return;
      this.bridge.tracePayment("ok-postmessage-return", { signal, data: event?.data });
      if (signal === "ok") void this.recoverPendingPurchase({ source: "postMessage" });
      if (signal === "cancel") this.clearPendingPurchase("platform-cancel");
    });
    ...
    void (async () => {
      ...
      // Merge instead of replacing: OK may reload the page after a successful payment
      // before the previous cloud sync completes, while localStorage already has the skin.
      if (!this.running && (this.mode === "menu" || this.mode === "boot")) {
        this.progress = mergeProgressSnapshots(this.progress, cloud);
        await this.reconcileOwnedPurchaseSkins({ refresh: true });
        this.ensureDailyProgress();
        this.ensureMissionProgress();
        this.syncUi();
        this.showMenu();
        const returnSignal = this.getPaymentReturnSignal();
        if (returnSignal) this.bridge.tracePayment("page-return-signal", { signal: returnSignal, href: globalThis.location?.href });
        if (returnSignal === "cancel") this.clearPendingPurchase("platform-cancel");
        else if (this.pendingPurchaseSkinId || this.readPendingPurchase()) {
          void this.recoverPendingPurchase({ source: returnSignal === "ok" ? "ok-redirect" : "boot" });
        }
      }
  ...
  createDom() {
    ...
    this.root.innerHTML = `
      <style>
        #slither-arena-root { position:fixed; inset:auto; left:0; top:0; width:100vw; height:100vh; --sa-vv-width:100vw; --sa-vv-height:100vh; --sa-vv-offset-left:0px; --sa-vv-offset-top:0px; --sa-occluded-bottom:0px; --sa-occluded-right:0px; z-index:2147482000; overflow:hidden; touch-action:none; overscroll-behavior:none; user-select:none; -webkit-user-select:none; -webkit-touch-callout:none; -webkit-user-drag:none; font-family:Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif; color:#f8fbff; }
        ...
        .sa-overlay { position:absolute; inset:0; display:flex; align-items:center; justify-content:center; padding:clamp(12px,3vw,28px); padding-bottom:max(clamp(12px,3vw,28px),calc(env(safe-area-inset-bottom) + var(--sa-occluded-bottom,0px) + 10px)); background:radial-gradient(circle at 50% 25%,rgba(34,55,92,.36),rgba(4,8,15,.72) 65%); opacity:0; visibility:hidden; pointer-events:none; transition:opacity .2s ease,visibility .2s ease; }
        .sa-overlay.visible { opacity:1; visibility:visible; pointer-events:auto; }
        #slither-arena-root.sa-sticky-safe .sa-overlay { padding-bottom:max(66px,calc(env(safe-area-inset-bottom) + var(--sa-occluded-bottom,0px) + 58px)); }
        #slither-arena-root.sa-sticky-safe .sa-panel { max-height:calc(var(--sa-vv-height,100dvh) - 78px) !important; }
        .sa-panel { position:relative; width:min(522px,100%); max-height:calc(var(--sa-vv-height,100vh) - 28px); overflow:auto; padding:clamp(20px,4vw,32px); border:1px solid rgba(255,255,255,.14); border-radius:28px; background:linear-gradient(145deg,rgba(27,42,68,.96),rgba(10,18,32,.98) 68%); box-shadow:0 28px 85px rgba(0,0,0,.52),inset 0 1px rgba(255,255,255,.10); scrollbar-width:none; }
        ...
        .sa-overlay {
          padding:max(10px,env(safe-area-inset-top)) max(10px,calc(env(safe-area-inset-right) + var(--sa-occluded-right,0px))) max(10px,calc(env(safe-area-inset-bottom) + var(--sa-occluded-bottom,0px))) max(10px,env(safe-area-inset-left));
        }
        .sa-panel.sa-auto-fit { box-shadow:0 18px 62px rgba(0,0,0,.50),inset 0 1px rgba(255,255,255,.10); }

        /* VK/OK render the sticky outside the game DOM and do not reliably report its
           occupied height to the iframe. Reserve the documented worst-case mobile area
           and keep the panel itself scrollable so every action remains reachable. */
        #slither-arena-root.sa-sticky-safe .sa-overlay {
          align-items:flex-start;
          overflow-x:hidden;
          overflow-y:auto;
          padding-bottom:max(132px,calc(env(safe-area-inset-bottom) + 124px + var(--sa-occluded-bottom,0px))) !important;
        }
        #slither-arena-root.sa-sticky-safe .sa-panel {
          max-height:calc(var(--sa-vv-height,100vh) - 154px) !important;
        ...
        @media (orientation:landscape) and (max-height:620px) {
          #slither-arena-root.sa-sticky-safe .sa-overlay {
            padding-bottom:max(96px,calc(env(safe-area-inset-bottom) + 88px + var(--sa-occluded-bottom,0px))) !important;
          }
          #slither-arena-root.sa-sticky-safe .sa-panel {
            max-height:calc(var(--sa-vv-height,100vh) - 112px) !important;
          }
        }
        ...
        /* v0.38: definitive no-overflow landscape menu.
           The prior three-column grid had fixed minimum column widths. In VK's
           landscape iframe the panel itself stayed centered, but the third column
           overflowed its right edge. Use two flexible columns with zero min-width,
           and center the panel independently from flex/grid intrinsic sizing. */
        @media (orientation:landscape) and (min-width:700px) {
          #slither-arena-root.sa-touch-landscape .sa-overlay {
          ...
          #slither-arena-root.sa-touch-landscape .sa-overlay > .sa-menu-panel {
            position:absolute !important;
            left:50% !important;
            top:50% !important;
          ...
          #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-menu-actions .sa-social-btn,
        ...
        @media (orientation:landscape) and (min-width:700px) and (max-height:390px) {
          #slither-arena-root.sa-touch-landscape .sa-overlay > .sa-menu-panel {
            width:calc(100% - 12px) !important;
            grid-template-columns:minmax(190px,.66fr) minmax(0,2.34fr) !important;
            padding:9px 12px !important;
            column-gap:12px !important;
            row-gap:5px !important;
          }
          #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-subtitle { display:none !important; }
          #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-title { font-size:32px !important; }
          #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-menu-progress { gap:6px !important; }
          #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-menu-actions { grid-auto-rows:minmax(33px,auto) !important; gap:5px !important; }
          #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-menu-actions .sa-play { min-height:36px !important; }
          #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-menu-actions .sa-btn,
          #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-menu-actions .sa-social-btn,
          #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-menu-actions .sa-controls-btn {
          ...
          #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-stat { min-height:40px !important; }
        }
        ...
        /* v0.35: definitive VK/OK menu fit. This is intentionally the last menu
           layer so legacy portrait/landscape rules cannot overlap the buttons. */
        ...
        #slither-arena-root .sa-overlay > .sa-menu-panel {
          display:block !important;
          width:min(500px,calc(100% - 4px)) !important;
          height:auto !important;
          min-height:0 !important;
          max-height:none !important;
          margin:0 !important;
          ...
          transform:scale(var(--sa-fit-scale,1)) !important;
          transform-origin:center center !important;
        }
        #slither-arena-root .sa-menu-panel .sa-menu-hero {
          display:block !important;
        ...
        #slither-arena-root .sa-menu-panel .sa-menu-emblem { display:none !important; }
        #slither-arena-root .sa-menu-panel .sa-brand {
          display:block !important;
          margin:0 0 3px !important;
          font-size:9px !important;
          line-height:1.1 !important;
        }
        #slither-arena-root .sa-menu-panel .sa-title {
          margin:0 !important;
        ...
        #slither-arena-root .sa-menu-panel .sa-subtitle {
          display:block !important;
          margin:6px 0 0 !important;
          font-size:11px !important;
          line-height:1.2 !important;
        }
        #slither-arena-root .sa-menu-panel .sa-menu-progress {
          display:flex !important;
          flex-direction:column !important;
          gap:6px !important;
          margin:8px 0 0 !important;
        }
        #slither-arena-root .sa-menu-panel .sa-league-card {
          display:grid !important;
          gap:3px !important;
          margin:0 !important;
          padding:8px 9px !important;
        ...
        #slither-arena-root .sa-menu-panel .sa-league-kicker { display:block !important; font-size:8px !important; }
        #slither-arena-root .sa-menu-panel .sa-league-title { font-size:12px !important; line-height:1.08 !important; }
        #slither-arena-root .sa-menu-panel .sa-league-row { font-size:9px !important; line-height:1.05 !important; }
        #slither-arena-root .sa-menu-panel .sa-progress-bar { height:7px !important; }
        #slither-arena-root .sa-menu-panel .sa-mission-teaser {
          display:flex !important;
          min-height:0 !important;
          margin:0 !important;
        ...
        #slither-arena-root .sa-menu-panel .sa-mission-teaser strong { font-size:11px !important; line-height:1.08 !important; }
        #slither-arena-root .sa-menu-panel .sa-mission-teaser span { font-size:9px !important; line-height:1.08 !important; }
        #slither-arena-root .sa-menu-panel .sa-mission-mini { min-width:58px !important; padding:4px 6px !important; font-size:9px !important; }
        #slither-arena-root .sa-menu-panel .sa-stat-row {
          display:grid !important;
          grid-template-columns:repeat(3,minmax(0,1fr)) !important;
          gap:6px !important;
        ...
        #slither-arena-root .sa-menu-panel .sa-stat small { min-height:0 !important; font-size:7px !important; line-height:1 !important; }
        #slither-arena-root .sa-menu-panel .sa-stat strong { margin-top:3px !important; font-size:13px !important; line-height:1 !important; }
        #slither-arena-root .sa-menu-panel .sa-menu-actions {
          display:grid !important;
          grid-template-columns:repeat(2,minmax(0,1fr)) !important;
          grid-template-rows:none !important;
        ...
        #slither-arena-root .sa-menu-panel .sa-menu-actions .sa-play {
          grid-column:1 / -1 !important;
          grid-row:auto !important;
        }
        #slither-arena-root .sa-menu-panel .sa-menu-actions .sa-btn,
        #slither-arena-root .sa-menu-panel .sa-menu-actions .sa-social-btn,
        #slither-arena-root .sa-menu-panel .sa-menu-actions .sa-controls-btn {
          grid-column:auto !important;
          grid-row:auto !important;
        ...
        #slither-arena-root .sa-menu-panel .sa-menu-actions .sa-play { font-size:13px !important; }
        #slither-arena-root .sa-menu-panel .sa-menu-actions svg { width:16px !important; height:16px !important; }

        /* Use root classes instead of CSS orientation: the platform banner can make
           media-query orientation briefly incorrect while the phone is rotating. */
        #slither-arena-root.sa-touch-landscape .sa-overlay > .sa-menu-panel {
          display:grid !important;
          width:min(760px,calc(100% - 4px)) !important;
          grid-template-columns:minmax(0,.88fr) minmax(0,1.12fr) !important;
          grid-template-areas:"hero progress" "stats actions" !important;
          column-gap:10px !important;
          row-gap:7px !important;
          padding:9px 11px !important;
          border-radius:18px !important;
        }
        #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-menu-hero { grid-area:hero !important; align-self:center !important; }
        #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-menu-progress { grid-area:progress !important; margin:0 !important; gap:5px !important; }
        #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-stat-row { grid-area:stats !important; margin:0 !important; align-self:start !important; }
        #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-menu-actions {
          grid-area:actions !important;
          grid-template-columns:repeat(3,minmax(0,1fr)) !important;
        ...
        #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-menu-actions .sa-play { grid-column:1 / -1 !important; }
        #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-title { font-size:clamp(24px,5.2vw,33px) !important; }
        #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-subtitle { display:none !important; }
        #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-league-kicker { display:none !important; }
        #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-league-card { padding:6px 8px !important; }
        #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-mission-teaser { padding:5px 7px !important; }
        #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-stat { min-height:35px !important; padding:5px !important; }
        #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-menu-actions .sa-btn { min-height:32px !important; padding:5px 6px !important; font-size:9px !important; }
        #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-menu-actions .sa-play { font-size:11px !important; }
        #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-social-label { display:inline !important; }



        /* v0.36: wide landscape menu. The previous two-column card was fitted by
           shrinking the whole panel, leaving most of VK's horizontal game area empty.
           On wide landscape viewports use three real columns instead: identity,
           progress, and controls. This keeps the card near scale 1 and the labels
           comfortably readable above a platform-owned bottom banner. */
        @media (orientation:landscape) and (min-width:700px) {
          #slither-arena-root.sa-touch-landscape .sa-overlay {
            padding:8px 12px !important;
          }
          #slither-arena-root.sa-touch-landscape .sa-overlay > .sa-menu-panel {
            ...
            grid-template-columns:minmax(220px,.82fr) minmax(290px,1.08fr) minmax(360px,1.30fr) !important;
            grid-template-rows:auto auto !important;
            grid-template-areas:
              "hero progress stats"
              "hero progress actions" !important;
          ...
          #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-menu-actions svg {
            width:18px !important;
            height:18px !important;
          }
        }

        /* v0.37: Safari/VK landscape centering. The root is no longer shifted by
           visualViewport.offsetLeft, and the wide card is explicitly constrained and
           centered inside the iframe's actual content box. */
        @media (orientation:landscape) and (min-width:700px) {
          #slither-arena-root.sa-touch-landscape .sa-overlay {
            box-sizing:border-box !important;
            width:100% !important;
        ...
        /* On extremely shallow browser frames retain the same wide layout but trim
           vertical decoration instead of scaling the complete menu to unreadable size. */
        @media (orientation:landscape) and (min-width:700px) and (max-height:390px) {
          #slither-arena-root.sa-touch-landscape .sa-overlay > .sa-menu-panel {
          ...
          #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-subtitle { display:none !important; }
          #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-title { font-size:34px !important; }
          #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-menu-progress { gap:7px !important; }
          #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-league-card { padding:7px 9px !important; }
          #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-mission-teaser { min-height:44px !important; padding:6px 8px !important; }
          #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-stat { min-height:44px !important; padding:6px 7px !important; }
          #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-menu-actions {
            grid-auto-rows:minmax(34px,auto) !important;
            gap:5px !important;
          }
          #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-menu-actions .sa-play { min-height:38px !important; }
          #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-menu-actions .sa-btn,
          #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-menu-actions .sa-social-btn,
          #slither-arena-root.sa-touch-landscape .sa-menu-panel .sa-menu-actions .sa-controls-btn {
        ...
        @media (max-width:360px) {
          #slither-arena-root .sa-overlay > .sa-menu-panel { padding:10px 11px !important; }
          #slither-arena-root .sa-menu-panel .sa-title { font-size:27px !important; }
          #slither-arena-root .sa-menu-panel .sa-subtitle { font-size:10px !important; }
          #slither-arena-root .sa-menu-panel .sa-menu-actions { gap:5px !important; }
        }
      </style>
      <div class="sa-shell">
  ...
  syncRootViewportBox() {
    ...
    const layoutWidth = Math.max(1, Math.round(globalThis.innerWidth ?? doc?.clientWidth ?? viewport?.width ?? 1));
    const layoutHeight = Math.max(1, Math.round(globalThis.innerHeight ?? doc?.clientHeight ?? viewport?.height ?? 1));
    const width = Math.max(1, Math.min(layoutWidth, Math.round(viewport?.width ?? layoutWidth)));
    const height = Math.max(1, Math.min(layoutHeight, Math.round(viewport?.height ?? layoutHeight)));
    const reportedOffsetLeft = Math.max(0, Math.round(viewport?.offsetLeft ?? 0));
    const reportedOffsetTop = Math.max(0, Math.round(viewport?.offsetTop ?? 0));
    const occludedRight = Math.max(0, layoutWidth - width);
    const occludedBottom = Math.max(0, layoutHeight - height);

    // A fixed element is already positioned relative to Safari's visual viewport.
    // Applying visualViewport.offsetLeft/offsetTop again double-shifts the game in
    // landscape, most visibly inside VK where the left platform rail is present.
    this.root.style.left = "0px";
    this.root.style.top = "0px";
    this.root.style.width = `${width}px`;
    this.root.style.height = `${height}px`;
    this.root.style.setProperty("--sa-vv-width", `${width}px`);
    this.root.style.setProperty("--sa-vv-height", `${height}px`);
    this.root.style.setProperty("--sa-vv-offset-left", `${reportedOffsetLeft}px`);
    this.root.style.setProperty("--sa-vv-offset-top", `${reportedOffsetTop}px`);
    this.root.style.setProperty("--sa-occluded-right", `${occludedRight}px`);
    this.root.style.setProperty("--sa-occluded-bottom", `${occludedBottom}px`);
  }

  resize() {
    if (!this.canvas) return;
    this.syncRootViewportBox();
  ...
  fitOverlayPanel(renderPreviews = false) {
    ...
    const isMenu = panel.classList.contains("sa-menu-panel");
    panel.style.setProperty("--sa-fit-scale", "1");
    panel.classList.remove("sa-auto-fit");

    // Old responsive layers used fixed heights and overflow:hidden. Measure the
    // complete menu, not the already-clipped card produced by those legacy rules.
    if (isMenu) {
    ...
    const paddingLeft = Number.parseFloat(overlayStyle?.paddingLeft) || 0;
    const paddingRight = Number.parseFloat(overlayStyle?.paddingRight) || 0;
    const paddingTop = Number.parseFloat(overlayStyle?.paddingTop) || 0;
    const paddingBottom = Number.parseFloat(overlayStyle?.paddingBottom) || 0;
    const reserve = isMenu ? 4 : 12;
    const availableWidth = Math.max(1, bounds.width - paddingLeft - paddingRight - reserve);
    const availableHeight = Math.max(1, bounds.height - paddingTop - paddingBottom - reserve);

    let rawWidth = Math.max(panel.offsetWidth || 0, panel.scrollWidth || 0, 1);
    let rawHeight = Math.max(panel.offsetHeight || 0, panel.scrollHeight || 0, 1);
    const isWideLandscapeMenu = isMenu
      && this.root?.classList?.contains("sa-touch-landscape")
      && bounds.width >= 700;
    if (isMenu) {
      const panelRect = panel.getBoundingClientRect();
      // In the definitive landscape layout every child is constrained to the panel.
      // Measuring a transient overflowing child here would shrink and displace the
      // whole card even after CSS has corrected the grid, so width uses the panel box.
      if (!isWideLandscapeMenu) {
        for (const child of panel.children) {
          const childRect = child.getBoundingClientRect();
          rawWidth = Math.max(rawWidth, childRect.right - panelRect.left);
        }
      }
      for (const child of panel.children) {
        const childRect = child.getBoundingClientRect();
        rawHeight = Math.max(rawHeight, childRect.bottom - panelRect.top);
      }
      const computed = globalThis.getComputedStyle?.(panel);
      rawHeight += Number.parseFloat(computed?.paddingBottom) || 0;
    }

    // The wide landscape menu uses horizontal space instead of shrinking the whole
    // card. Keep a readable floor during transient VK/Safari viewport changes.
    const minimumScale = isWideLandscapeMenu ? .82 : (isMenu ? .34 : .45);
    const scale = clamp(Math.min(1, availableWidth / rawWidth, availableHeight / rawHeight), minimumScale, 1);
  ...
  async reconcileOwnedPurchaseSkins({ refresh = false } = {}) {
    if (!this.bridge.isGamePushPlatform) return false;
    const gp = this.bridge.gp ?? this.bridge.getGamePush?.();
    if (!gp?.payments) return false;
    if (refresh) await this.bridge.refreshPayments(gp);

    const owned = SKINS.filter((skin) => skin.kind === "purchase" && skin.sku && this.bridge.hasPermanentPurchase(gp, skin.sku));
    ...
    for (const skin of owned) {
      ...
      if (!this.progress.purchaseSkins.includes(skin.id)) {
        this.progress.purchaseSkins.push(skin.id);
        changed = true;
      }
    }

    const pending = owned.find((skin) => skin.id === this.pendingPurchaseSkinId);
    if (pending && this.progress.selectedSkin !== pending.id) {
      this.progress.selectedSkin = pending.id;
      changed = true;
    }
    ...
    if (pending) this.clearPendingPurchase("reconciled");
  ...
  async purchaseSkin(skin) {
    ...
    this.storePendingPurchase(skin);
    this.toast(this.t("toast.openingPurchase"));
    const success = await this.bridge.purchaseSkin(skin.sku);
    let restoredOnReturn = this.progress.unlockedSkins.includes(skin.id);
    if (!success && !restoredOnReturn) {
      restoredOnReturn = await this.recoverPendingPurchase({ timeout: 12000, source: "purchase-flow" });
    }
    if (!success && !restoredOnReturn) {
      // Keep the pending marker for an OK redirect/reload. It is cleared only after
      // ownership is confirmed, explicit cancellation, or expiry.
    ...
    this.progress.selectedSkin = skin.id;
    await this.persistProgress();
    this.clearPendingPurchase("purchase-flow-complete");
    this.toast(this.t("toast.skinOpened", { name: this.getSkinTitle(skin) }));
    this.showShop();
  }
  ...
  showOverlay(html, panelClass = "") {
    ...
    for (const delay of [80, 220, 500]) {
      setTimeout(() => {
        if (this.ui?.overlay?.classList.contains("visible")) this.fitOverlayPanel(false);
      }, delay);
    }
  }
