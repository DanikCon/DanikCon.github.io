...
// Версия: GamePush v1.49.18 — Playgama Bridge для всех покупок в OK с безопасным откатом на GamePush; VK без изменений.
(async () => {
  "use strict";
  ...
  const CONFIG = {
    ...
    products: {
      ...
      frostDeck: "deck_frost7",
      amberDeck: "deck_amber7"
    },
  ...
  const DECKS = {
    ...
    frost7: { titleKey: "frost", kind: "purchase", product: "frostDeck", theme: { tableA: "#5f8fa8", tableB: "#183a52", accent: "#dff7ff", backA: "#b9ecff", backB: "#4f88b8", frontA: "#ffffff", frontB: "#e8f7ff" } },
    amber7: { titleKey: "amber", kind: "purchase", product: "amberDeck", theme: { tableA: "#8a5a1f", tableB: "#3d220d", accent: "#ffd77a", backA: "#d9902f", backB: "#7a4215", frontA: "#fffaf0", frontB: "#f6e4bf" } }
  };
  ...
  const platform = {
    ...
    okPaymentsBridge: null,
    okPaymentsBridgeInitPromise: null,
    okPaymentsBridgeReady: false,
    okPaymentsBridgeSupported: false,
    okPaymentsBridgeApiCallback: null,
    okPaymentsBridgePreviousApiCallback: null,
    okPaymentsBridgeCatalog: new Map(),
    okPaymentsBridgePurchases: new Set(),
    okPaymentsProvider: "gamepush",
    ...
    resolveGamePushPlatform() {
      ...
      // OK-hosted VK Mini Apps are explicitly marked with vk_client=ok. Check this first,
      // otherwise the generic VK platform value can incorrectly classify OK as VK.
      const vkClient = String(query.get("vk_client") || "").toLowerCase();
      if (vkClient === "ok") return "ok";
      const referrer = String(document.referrer || "").toLowerCase();
      const ancestors = (() => {
        try { return Array.from(location.ancestorOrigins || []).join(" ").toLowerCase(); }
        catch (_) { return ""; }
      })();
      if (/(^|\.)ok\.ru|okcdn\.ru/.test(referrer) || /(^|\.)ok\.ru|okcdn\.ru/.test(ancestors)) return "ok";
      const candidates = [
    ...
    okPaymentsBridgeProductId(value) {
      if (value === undefined || value === null) return "";
      if (typeof value === "string" || typeof value === "number") return String(value);
      if (typeof value !== "object") return "";
      const id = value.id ?? value.productId ?? value.productID ?? value.purchaseId ??
        value.product?.id ?? value.product?.productId ?? value.data?.id;
      return id === undefined || id === null ? "" : String(id);
    },

    okPaymentsBridgeProductConfig(productTag) {
      const product = this.commerceProducts.get(productTag) || {};
      const fallbackNames = {
        [CONFIG.products.removeAds]: "Отключение рекламы",
        [CONFIG.products.royalDeck]: "Колода Royal",
        [CONFIG.products.noirDeck]: "Колода Noir",
        [CONFIG.products.rubyDeck]: "Колода Ruby",
        [CONFIG.products.goldDeck]: "Колода Gold",
        [CONFIG.products.cosmosDeck]: "Колода Cosmos",
        [CONFIG.products.frostDeck]: "Колода Иней",
        [CONFIG.products.amberDeck]: "Колода Янтарь"
      };
      const name = String(product.name || product.title || fallbackNames[productTag] || productTag);
      const description = String(product.description || product.shortDescription || name);
      const rawPrice = product.priceValue ?? product.price ?? product.amount ?? 1;
      let numericPrice = Number(rawPrice);
      if (!Number.isFinite(numericPrice)) {
        const match = String(rawPrice).replace(",", ".").match(/\d+(?:\.\d+)?/);
        numericPrice = match ? Number(match[0]) : 1;
      }
      // OK uses integer OK units. Keep the platform catalogue price when GamePush exposes it.
      const price = Math.max(1, Math.round(Number.isFinite(numericPrice) ? numericPrice : 1));
      return {
        id: productTag,
        ok: {
          id: productTag,
    ...
    buildOkPaymentsBridgeConfig() {
      return {
        // The OK-hosted build contains VK launch parameters as well. Force the intended adapter,
        // otherwise Bridge can classify the host as VK and use the wrong payments implementation.
        forciblySetPlatformId: "ok",
        payments: Object.values(CONFIG.products).map(tag => this.okPaymentsBridgeProductConfig(tag))
      };
    },

    okPaymentsBridgeTimeout(promise, timeoutMs, label) {
      let timer = 0;
      const timeout = new Promise((_, reject) => {
        timer = window.setTimeout(() => reject(new Error(`Playgama ${label} timeout`)), timeoutMs);
      });
    ...
    async withOkPaymentsBridgeCallback(task) {
      const previous = globalThis.API_callback;
      if (typeof this.okPaymentsBridgeApiCallback === "function") {
        globalThis.API_callback = this.okPaymentsBridgeApiCallback;
      }
      try {
        return await task();
      } finally {
        const installed = globalThis.API_callback;
        if (typeof installed === "function" && installed !== previous) {
          this.okPaymentsBridgeApiCallback = installed;
        }
        if (previous === undefined) {
          try { delete globalThis.API_callback; } catch (_) { globalThis.API_callback = undefined; }
        } else {
          globalThis.API_callback = previous;
        }
      }
    },

    async ensureOkPaymentsBridge() {
      if (!isOkPurchaseContext()) return false;
      if (this.okPaymentsBridgeReady) return this.okPaymentsBridgeSupported;
      if (this.okPaymentsBridgeInitPromise) return this.okPaymentsBridgeInitPromise;

      this.okPaymentsBridgeInitPromise = (async () => {
        let displacedWindowBridge;
        const previousApiCallback = globalThis.API_callback;
        try {
          let bridge = window.playgamaBridge;
          if (!bridge || typeof bridge.initialize !== "function") {
            // The Playgama SDK reserves window.bridge. Preserve an unrelated host object, if any,
            // and keep the Playgama instance under window.playgamaBridge after loading.
            if (window.bridge && typeof window.bridge.initialize !== "function") {
              displacedWindowBridge = window.bridge;
              try { delete window.bridge; } catch (_) { window.bridge = undefined; }
            }
            await injectScript("https://bridge.playgama.com/v1/stable/playgama-bridge.js", 5000);
            bridge = window.playgamaBridge || window.bridge;
            if (displacedWindowBridge && window.bridge === bridge) window.bridge = displacedWindowBridge;
          }
          if (!bridge || typeof bridge.initialize !== "function") return false;

          bridge.engine = "construct";
          if (!bridge.isInitialized) {
            const config = this.buildOkPaymentsBridgeConfig();
            const configMarker = `__kosynka_playgama_ok_${Date.now()}.json`;
            const previousFetch = globalThis.fetch;
            const configFetch = (input, init) => {
              const requestUrl = typeof input === "string" ? input : String(input?.url || "");
              if (requestUrl.includes(configMarker)) {
            ...
            globalThis.fetch = configFetch;
            try {
              await this.okPaymentsBridgeTimeout(
                bridge.initialize({ configFilePath: configMarker }),
                10000,
                "initialization"
              );
            } finally {
              if (globalThis.fetch === configFetch) globalThis.fetch = previousFetch;
            }
          }

          this.okPaymentsBridge = bridge;
          this.okPaymentsBridgePreviousApiCallback = previousApiCallback;
          this.okPaymentsBridgeApiCallback = globalThis.API_callback;
          // GamePush owns the normal app callback. Activate the Playgama callback only while a
          // Playgama request is running, then restore the previous handler.
          if (globalThis.API_callback !== previousApiCallback) {
            if (previousApiCallback === undefined) {
              try { delete globalThis.API_callback; } catch (_) { globalThis.API_callback = undefined; }
            } else {
              globalThis.API_callback = previousApiCallback;
            }
          }

          const platformId = String(bridge.platform?.id || "").toLowerCase();
          const rawSupported = bridge.payments?.isSupported;
          const supported = platformId === "ok" &&
            (typeof rawSupported === "function" ? !!(await rawSupported.call(bridge.payments)) : rawSupported === true) &&
            typeof bridge.payments?.purchase === "function";

          this.okPaymentsBridgeReady = true;
          this.okPaymentsBridgeSupported = supported;
          this.okPaymentsProvider = supported ? "playgama" : "gamepush";
          if (supported) await this.refreshOkPaymentsBridgeCommerce();
          else console.warn("[Kosynka] Playgama payments are unavailable in OK; GamePush fallback is active", { platformId, rawSupported });
          return supported;
        } catch (error) {
          this.okPaymentsBridgeReady = true;
          this.okPaymentsBridgeSupported = false;
          this.okPaymentsProvider = "gamepush";
          console.warn("[Kosynka] Playgama OK payments initialization failed; GamePush fallback is active", error);
          if (displacedWindowBridge && !window.bridge) window.bridge = displacedWindowBridge;
          return false;
        } finally {
          // API_callback and window.bridge are restored above; no persistent global takeover is required.
        }
      })();

      return this.okPaymentsBridgeInitPromise;
    },

    async refreshOkPaymentsBridgeCommerce() {
      const bridge = this.okPaymentsBridge;
      if (!this.okPaymentsBridgeSupported || !bridge?.payments) return false;
      let changed = false;
      try {
        if (typeof bridge.payments.getCatalog === "function") {
          const catalog = await this.okPaymentsBridgeTimeout(
            this.withOkPaymentsBridgeCallback(() => bridge.payments.getCatalog()),
            3000,
            "catalogue"
          );
          if (Array.isArray(catalog)) {
            this.okPaymentsBridgeCatalog.clear();
            for (const item of catalog) {
              const id = this.okPaymentsBridgeProductId(item);
              if (id) this.okPaymentsBridgeCatalog.set(id, item);
            }
          }
        }
      } catch (error) {
        console.warn("[Kosynka] Playgama OK catalogue refresh failed", error);
      }
      try {
        if (typeof bridge.payments.getPurchases === "function") {
          const purchases = await this.okPaymentsBridgeTimeout(
            this.withOkPaymentsBridgeCallback(() => bridge.payments.getPurchases()),
            5000,
            "purchase restore"
          );
          if (Array.isArray(purchases)) {
            this.okPaymentsBridgePurchases.clear();
            for (const item of purchases) {
              const id = this.okPaymentsBridgeProductId(item);
              if (!id) continue;
              this.okPaymentsBridgePurchases.add(id);
              if (!this.purchasedProducts.has(id)) changed = true;
              this.purchasedProducts.add(id);
            }
          }
        }
      } catch (error) {
        console.warn("[Kosynka] Playgama OK purchases refresh failed", error);
      }
      return changed;
    },

    async purchaseViaOkPaymentsBridge(productTag) {
      if (!(await this.ensureOkPaymentsBridge())) return null;
      const bridge = this.okPaymentsBridge;
      if (!bridge?.payments?.purchase) return null;

      const config = this.okPaymentsBridgeProductConfig(productTag).ok;
      const options = {
        ...config,
        externalId: `${productTag}_${Date.now()}_${Math.floor(Math.random() * 1000000)}`
      };
      try {
        const receipt = await this.okPaymentsBridgeTimeout(
          this.withOkPaymentsBridgeCallback(() => bridge.payments.purchase(productTag, options)),
          60000,
          "purchase"
        );
        const resolvedTag = this.okPaymentsBridgeProductId(receipt) || productTag;
        this.okPaymentsBridgePurchases.add(productTag);
        this.okPaymentsBridgePurchases.add(resolvedTag);
        this.purchasedProducts.add(productTag);
        this.purchasedProducts.add(resolvedTag);
        this.okPaymentsProvider = "playgama";
        return {
          ok: true,
          code: "success",
          productTag: resolvedTag,
          requestedProductTag: productTag,
          provider: "playgama",
          source: "playgama-bridge",
          result: receipt
        };
      } catch (error) {
        // A platform can report an error after recording the transaction. Restore the purchase list
        // before deciding whether the player cancelled.
        try { await this.refreshOkPaymentsBridgeCommerce(); } catch (_) { }
        if (this.okPaymentsBridgePurchases.has(productTag) || this.purchasedProducts.has(productTag)) {
          return {
            ok: true,
            code: "success",
            productTag,
            requestedProductTag: productTag,
            provider: "playgama",
            source: "playgama-entitlement-after-error",
            error
          };
        }

        const message = String(error?.message || error || "").toLowerCase();
        const setupFailure = /not supported|unsupported|not available|not initialized|no platform product|product.+not found|catalog/.test(message);
        if (setupFailure) {
          console.warn("[Kosynka] Playgama cannot start this OK purchase; using GamePush fallback", error);
          this.okPaymentsBridgeSupported = false;
          this.okPaymentsProvider = "gamepush";
          return null;
        }
        console.warn("[Kosynka] Playgama OK purchase failed or was cancelled", error);
        return { ok: false, code: "cancelled", provider: "playgama", error };
      }
    },
    ...
    hasPurchase(productTag) {
      ...
      if (this.okPaymentsBridgePurchases.has(productTag)) return true;
      try {
    ...
    async syncEntitlements(targetProfile) {
      ...
      await this.refreshCommerce();
      if (isOkPurchaseContext()) {
        try {
          if (await this.ensureOkPaymentsBridge()) await this.refreshOkPaymentsBridgeCommerce();
        } catch (error) {
          console.warn("[Kosynka] Playgama entitlement restore skipped; GamePush data remains active", error);
        }
      }
      let changed = false;
      const productToSkin = {
        ...
        [CONFIG.products.frostDeck]: "frost7",
        [CONFIG.products.amberDeck]: "amber7"
      };
    ...
    async purchase(productTag) {
      ...
      // Only OK is rerouted. VK/Yandex keep the existing GamePush checkout untouched.
      if (this.kind === "gamepush" && isOkPurchaseContext()) {
        const bridgeResult = await this.purchaseViaOkPaymentsBridge(productTag);
      ...
      const purchaseIsRecorded = () => {
        try {
          if (this.hasPurchase(productTag)) return true;
        } catch (_) { /* Fall through to raw purchases. */ }
        try {
          const purchases = Array.isArray(this.gp?.payments?.purchases) ? this.gp.payments.purchases : [];
      ...
      const confirmedSuccess = (source, result = undefined, error = undefined) => {
        this.purchasedProducts.add(productTag);
        return {
          ok: true,
          code: "success",
      ...
      // OK has its own documented global API_callback for FAPI.UI.showPayment. GamePush can keep
      // its purchase Promise pending until a real focus/visibility transition, so listen to the
      // native OK callback in parallel and finish the in-game purchase UI as soon as OK itself
      // reports a successful web payment.
      const okCallbackWaiter = isOkPurchaseContext() ? createOkPaymentCallbackWaiter(productTag) : null;

      let rawSdkPromise;
      try {
        rawSdkPromise = this.gp.payments.purchase({ tag: productTag });
      } catch (error) {
        okCallbackWaiter?.cancel();
        if (purchaseIsRecorded()) return confirmedSuccess("entitlement-after-sync-throw", undefined, error);
        return { ok: false, code: "unavailable", error };
      }

      // Some adapters assign/replace API_callback while opening the payment dialog. Re-wrap it
      // immediately after purchase() starts and once more on the next macrotask.
      if (okCallbackWaiter) {
        installOkPaymentCallbackBridge();
        window.setTimeout(() => installOkPaymentCallbackBridge(), 0);
      }

      const sdkPromise = Promise.resolve(rawSdkPromise).then(
        receipt => ({ type: "sdk-resolve", receipt }),
        error => ({ type: "sdk-reject", error })
      );

      let pollTimer = 0;
      let pollResolve = null;
      const entitlementPromise = new Promise(resolve => {
        pollResolve = resolve;
        const check = () => {
          if (purchaseIsRecorded()) resolve({ type: "entitlement" });
        };
      ...
      try {
        const race = [sdkPromise, entitlementPromise];
        if (okCallbackWaiter) race.push(okCallbackWaiter.promise);
        let outcome = await Promise.race(race);

        if (outcome?.type === "ok-native-callback") {
          const result = String(outcome.result || "").toLowerCase();
          const data = String(outcome.data ?? "").toLowerCase();
          const delayedMobilePayment = data === "phone" || data === "card";

          // Web OK: result=ok means the payment completed successfully. Do not wait for the
          // GamePush Promise which is the component observed timing out in this integration.
          if (result === "ok" && !delayedMobilePayment) {
            sdkPromise.catch(() => {});
            return confirmedSuccess("ok-native-api-callback", outcome);
          }

          // Mobile phone/card callbacks may precede the actual entitlement according to OK docs,
          // so keep waiting for GamePush/entitlement in those two documented cases.
          if (result === "ok" && delayedMobilePayment) {
            outcome = await Promise.race([sdkPromise, entitlementPromise]);
          } else if (result === "cancel" || result === "error") {
            if (purchaseIsRecorded()) return confirmedSuccess("entitlement-after-ok-cancel", outcome);
            return { ok: false, code: "cancelled", result: outcome };
          }
        }

        if (outcome?.type === "entitlement") {
          sdkPromise.catch(() => {});
          return confirmedSuccess("live-entitlement");
        }

        if (outcome?.type === "sdk-reject") {
          console.warn("[Kosynka] GamePush purchase rejected; checking recorded entitlement", outcome.error);
          if (purchaseIsRecorded()) return confirmedSuccess("entitlement-after-reject", undefined, outcome.error);
          for (let i = 0; i < 12; i++) {
            await new Promise(resolve => window.setTimeout(resolve, 100));
            if (purchaseIsRecorded()) return confirmedSuccess("entitlement-after-reject-delay", undefined, outcome.error);
          }
          return { ok: false, code: "cancelled", error: outcome.error };
        }
        ...
        if (explicitFailure) {
          if (purchaseIsRecorded()) return confirmedSuccess("entitlement-after-failed-result", receipt);
        ...
        return {
          ...
          productTag: resolvedTag,
          requestedProductTag: productTag,
          source: "resolved-checkout",
          result: receipt
        };
      } finally {
        if (pollTimer) window.clearInterval(pollTimer);
        if (pollResolve) pollResolve({ type: "stopped" });
        okCallbackWaiter?.cancel();
      }
  ...
  function openStore(tab = shopTab) {
    ...
    const paid = ["royal", "noir", "ruby", "gold", "cosmos", "frost7", "amber7"];
  ...
  function isOkPurchaseContext() {
    if (platform.gamePushPlatform === "ok") return true;
    try {
      const query = new URLSearchParams(location.search || "");
      if (String(query.get("vk_client") || "").toLowerCase() === "ok") return true;
    } catch (_) { /* Ignore malformed URL state. */ }
    try {
      const source = `${document.referrer || ""} ${Array.from(location.ancestorOrigins || []).join(" ")}`.toLowerCase();
      return /(^|[/.])ok\.ru|okcdn\.ru/.test(source);
    } catch (_) {
      return false;
    }
  }

  const okPaymentCallbackListeners = new Set();

  function installOkPaymentCallbackBridge() {
    if (!isOkPurchaseContext()) return false;
    try {
      const current = globalThis.API_callback;
      if (current?.__kosynkaOkPaymentBridge === true) return true;
      const previous = typeof current === "function" ? current : null;
      const bridge = function(method, result, data) {
        // Notify Kosynka first. The listener only resolves a Promise and does not interfere with
        // GamePush's own callback; the previous handler is always called afterwards.
        try {
          if (String(method || "").toLowerCase() === "showpayment") {
            const payload = { method, result, data, receivedAt: Date.now() };
            for (const listener of Array.from(okPaymentCallbackListeners)) {
              try { listener(payload); } catch (error) {
                console.warn("[Kosynka] OK payment callback listener failed", error);
              }
            }
          }
        } catch (error) {
          console.warn("[Kosynka] OK payment callback bridge failed", error);
        }

        if (previous) {
          try { return previous.apply(this, arguments); }
          catch (error) {
            console.warn("[Kosynka] Previous API_callback failed", error);
          }
        }
        return undefined;
      };
      Object.defineProperty(bridge, "__kosynkaOkPaymentBridge", { value: true });
      Object.defineProperty(bridge, "__kosynkaPreviousApiCallback", { value: previous });
      globalThis.API_callback = bridge;
      return globalThis.API_callback === bridge;
    } catch (error) {
      console.warn("[Kosynka] Could not install OK API_callback bridge", error);
      return false;
    }
  }

  function createOkPaymentCallbackWaiter(productTag) {
    let active = true;
    ...
    const listener = payload => {
      if (!active) return;
      active = false;
      okPaymentCallbackListeners.delete(listener);
      resolvePromise?.({ type: "ok-native-callback", productTag, ...payload });
    };
    okPaymentCallbackListeners.add(listener);
    installOkPaymentCallbackBridge();
    return {
      promise,
      cancel() {
  ...
  let activePurchaseUiWatch = null;
  let activePurchaseUiWatchId = 0;

  function eventPurchaseTag(payload) {
    const candidates = [payload, payload?.product, payload?.purchase, payload?.data];
    for (const candidate of candidates) {
      if (!candidate) continue;
      try {
        const direct = platform.resolvePurchaseTag(candidate);
        if (direct) return direct;
      } catch (_) { }
      try {
        const tag = platform.productTagFrom(candidate);
        if (tag) return tag;
      } catch (_) { }
    }
    return "";
  ...
  function forceRefreshPurchaseStore() {
    // There is no Shop layout in this project: the store is the #k-modal DOM surface on Game.
    // Refresh the purchase tab immediately after an entitlement change.
    openStore("purchase");
    const modal = app?.querySelector("#k-modal");
    if (modal) {
      modal.dataset.locked = "0";
      // Force style/layout calculation now; a second macrotask render catches adapters that mutate
      // the platform overlay DOM in the same turn as their payment callback.
      void modal.offsetHeight;
    }
    window.setTimeout(() => {
      if (shopTab === "purchase") openStore("purchase");
    }, 0);
  }

  function startPurchaseUiWatch(productTag, { skinId = "", kind = "skin" } = {}) {
    activePurchaseUiWatch?.cancel?.();
    const id = ++activePurchaseUiWatchId;
    let done = false;
    let timer = 0;
    let resolveWatch = null;
    const payments = platform.kind === "gamepush" ? platform.gp?.payments : null;
    const promise = new Promise(resolve => { resolveWatch = resolve; });

    const cleanup = () => {
      if (timer) window.clearInterval(timer);
      timer = 0;
      try { payments?.off?.("purchase", onPurchase); } catch (_) { }
      try { payments?.off?.("close", onClose); } catch (_) { }
      try { payments?.off?.("error:purchase", onError); } catch (_) { }
      okPaymentCallbackListeners.delete(onOkCallback);
      if (activePurchaseUiWatch?.id === id) activePurchaseUiWatch = null;
    };

    const isRecorded = () => {
      try {
        if (platform.hasPurchase(productTag)) return true;
      } catch (_) { }
      try {
        const purchases = Array.isArray(payments?.purchases) ? payments.purchases : [];
        return purchases.some(item => {
          const tag = platform.resolvePurchaseTag(item) ||
            platform.productTagFrom(platform.commerceProductsById.get(platform.productIdFrom(item)));
          return tag === productTag;
        });
      } catch (_) {
        return false;
    ...
    const finishSuccess = async source => {
      ...
      cleanup();
      try {
        await handlePlatformPurchaseSuccess(productTag, skinId);
      } catch (error) {
        console.warn("[Kosynka] Immediate purchase entitlement apply failed", error);
      }
      // purchase() may still be pending inside GamePush. Release our UI/input lock independently;
      // finishExternalInteraction(null) invalidates the old token so its later finally is harmless.
      finishExternalInteraction();
      forceRefreshPurchaseStore();
      if (kind === "skin" && skinId && DECKS[skinId]) {
        toast(t("deckBought", { title: deckTitle(skinId) }));
      } else if (kind === "noads") {
        toast(t("adsDisabled"));
      }
      resolveWatch?.({ ok: true, code: "success", productTag, source });
      return true;
    };

    const checkNow = source => {
      if (done) return true;
      if (!isRecorded()) return false;
      void finishSuccess(source);
      return true;
    };

    function onPurchase(payload) {
      const tag = eventPurchaseTag(payload);
      if (!tag || tag === productTag || isRecorded()) void finishSuccess("gamepush-purchase-event");
    }
    function onClose() {
      window.setTimeout(() => checkNow("gamepush-close"), 0);
      window.setTimeout(() => checkNow("gamepush-close-delay"), 250);
    }
    function onError() {
      window.setTimeout(() => checkNow("gamepush-error-purchase"), 0);
      window.setTimeout(() => checkNow("gamepush-error-purchase-delay"), 250);
    }
    function onOkCallback(payload) {
      const result = String(payload?.result || "").toLowerCase();
      const callbackData = String(payload?.data ?? "").toLowerCase();
      const delayedMobilePayment = callbackData === "phone" || callbackData === "card";
      if (result === "ok" && !delayedMobilePayment) void finishSuccess("ok-native-api-callback-ui-watch");
      else if (result === "ok") {
        window.setTimeout(() => checkNow("ok-native-delayed-payment"), 250);
      }
    }

    try { payments?.on?.("purchase", onPurchase); } catch (_) { }
    try { payments?.on?.("close", onClose); } catch (_) { }
    try { payments?.on?.("error:purchase", onError); } catch (_) { }
    if (isOkPurchaseContext()) {
      okPaymentCallbackListeners.add(onOkCallback);
      installOkPaymentCallbackBridge();
    }
    timer = window.setInterval(() => checkNow("entitlement-poll"), 200);
    window.setTimeout(() => checkNow("entitlement-initial"), 0);
    ...
    const watch = {
      ...
      cancel() {
        ...
        cleanup();
        resolveWatch?.({ ok: false, code: "cancelled", productTag, source: "watch-cancelled" });
      }
    };
    activePurchaseUiWatch = watch;
    return watch;
  }
  ...
  async function purchaseSkin(id) {
    ...
    const watch = startPurchaseUiWatch(productTag, { skinId: id, kind: "skin" });
    const platformPromise = runExternalInteraction(
      () => platform.purchase(productTag),
      { ok: false, code: "unavailable" }
    ).then(purchase => ({ type: "platform", purchase }));
    const watchPromise = watch.promise.then(result => ({ type: "watch", result }));
    const outcome = await Promise.race([platformPromise, watchPromise]);

    // The detached watcher already applied the entitlement, released the interaction lock and
    // refreshed the purchase store. Do not wait for GamePush's still-pending purchase Promise.
    if (outcome?.type === "watch" && outcome.result?.ok) return;
    ...
    if (!purchase?.ok) {
      if (watch.checkNow("platform-result-failure-final-check")) return;
      watch.cancel();
      forceRefreshPurchaseStore();
  ...
  async function purchaseNoAds() {
    ...
    const productTag = CONFIG.products.removeAds;
    const watch = startPurchaseUiWatch(productTag, { kind: "noads" });
    const platformPromise = runExternalInteraction(
      () => platform.purchase(productTag),
      { ok: false, code: "unavailable" }
    ).then(purchase => ({ type: "platform", purchase }));
