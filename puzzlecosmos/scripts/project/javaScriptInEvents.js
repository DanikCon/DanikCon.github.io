...
const scriptsInEvents = {
	...
	async Gameplay_Event47_Act1(runtime, localVars)
	{
		// Лидерборд через прямой GamePush API: исправлен withMe='first' и синхронизация рекорда перед открытием.
		(async () => {
		    try {
		        if (typeof gp === 'undefined' || !gp.leaderboard || !gp.player) {
		            console.warn('GamePush SDK не готов: лидерборд не открыт');
		            return;
		        }
		
		        const currentScore = Number(runtime.globalVars.score || 0);
		        const savedScore = Number(gp.player.get('hscore') || 0);
		        const bestScore = Math.max(currentScore, savedScore);
		
		        if (bestScore > savedScore) {
		            gp.player.set('hscore', bestScore);
		            await gp.player.sync();
		        }
		
		        await gp.leaderboard.open({
		            orderBy: ['hscore'],
		            order: 'DESC',
		            limit: 10,
		            withMe: 'first',
		        ...
		        console.error('Ошибка открытия лидерборда:', err);
		    }
		})();
	},

	async Gameplay_Event52_Act3(runtime, localVars)
	{
		try {
	...
	async Gameplay_Event56_Act4(runtime, localVars)
	{
		const layer = runtime.layout.getLayer("game");
	...
	async Gameplay_Event56_Act9(runtime, localVars)
	{
		try {
		    if (window.puzzle333SaveState) window.puzzle333SaveState(runtime);
		} catch (err) {
	...
	async Gameplay_Event67_Act1(runtime, localVars)
	{
		// Открываем группу напрямую по URL: GamePush join-community на OK может не срабатывать.
		(() => {
	...
	async Gameplay_Event73_Act2(runtime, localVars)
	{
		try {
		    if (window.puzzle333ClearGameState) window.puzzle333ClearGameState();
		    else localStorage.removeItem('puzzle333_state_v4');
	...
	async Gameplay_Event77_Act1(runtime, localVars)
	{
...
window.addEventListener('resize', window.c3FxResizeHandler);
	...
	async Gameplay_Event78_Act2(runtime, localVars)
	{
...
window.mySyncTimer = setInterval(() => {
	...
	async Gameplay_Event78_Act3(runtime, localVars)
	{
	...
	async Gameplay_Event78_Act4(runtime, localVars)
	{
	...
	async Gameplay_Event78_Act5(runtime, localVars)
	{
		...
		(function () {
		    ...
		    window.puzzle333SaveState = function (rt) {
		        try {
		            ...
		            // На экране проигрыша не сохраняем поле, иначе игрок может вернуться в уже проигранное состояние.
		            if (layerVisible(rt, "lose")) return;
	...
	async Gameplay_Event79_Act1(runtime, localVars)
	{
		try {
		    if (window.puzzle333SaveState) window.puzzle333SaveState(runtime);
		} catch (err) {
