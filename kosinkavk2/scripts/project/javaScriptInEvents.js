...
(async () => {
  ...
  const platform = {
    ...
    bindPurchaseEvents() {
      ...
      this.gp.payments.on("purchase", payload => {
        const product = payload?.product || null;
        const purchase = payload?.purchase || null;
        let tag = this.productTagFrom(product) || this.productTagFrom(purchase);
        if (!tag) {
          const productId = this.productIdFrom(purchase) || this.productIdFrom(product);
          tag = this.productTagFrom(this.commerceProductsById.get(productId));
        }
        if (!tag) return;
        // The purchase event is the earliest authoritative success signal from GamePush.
        // Apply the permanent entitlement immediately instead of waiting for a page reload
        // or for gp.payments.purchases to refresh its cached list.
    ...
    async purchase(productTag) {
      try {
        ...
        if (this.kind === "gamepush" && this.gp?.payments?.purchase) {
          ...
          const successPayload = result?.success === true || Boolean(result?.product || result?.purchase);
          if (successPayload) this.purchasedProducts.add(productTag);
          await this.refreshCommerce();
          // Some platform caches update gp.payments.purchases a little later than the successful checkout.
          // Preserve the success returned by purchase() so the current session unlocks the item immediately.
          if (successPayload) this.purchasedProducts.add(productTag);
          const confirmed = this.hasPurchase(productTag) || successPayload;
  ...
  async function handlePlatformPurchaseSuccess(productTag) {
    ...
    // Refresh the store under the platform checkout overlay so the new state is visible
    // as soon as the user returns to the game, with no manual page reload.
