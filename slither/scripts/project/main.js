/*
  Slither Arena — Construct 3 JS runtime (GamePush + Playgama integration v0.16)
  ---------------------------------------------------------------
...
class PlatformBridge {
  ...
  getForcedPlatform() {
    try {
      ...
      return params.get("slither_platform") ?? params.get("slitherPlatform") ?? null;
    } catch (_error) {
      return null;
    }
  }
  ...
  detectPlatform() {
    ...
    if (host.includes("playgama")) return "playgama";
  ...
  async _initialize() {
    ...
    } else if (this.isGamePushPlatform) {
  ...
  async initGamePush() {
    ...
    // When the Construct plugin is active, let it own the SDK bootstrap. This avoids duplicate scripts.
    const addonLoaded = !!globalThis.C3?.Plugins?.Eponesh_GameScore;
    if (addonLoaded) {
      gp = await this.waitFor(() => this.getGamePush());
      if (gp) return gp;
      return null;
    }

    if (!this.isGamePushPlatform || !globalThis.document?.head) return null;
    const callbackName = `__slitherGamePushInit_${Math.random().toString(36).slice(2)}`;
    const src = `https://gs.eponesh.com/sdk/gamepush.js?projectId=${encodeURIComponent(PLATFORM_SDK.GAMEPUSH_PROJECT_ID)}&publicToken=${encodeURIComponent(PLATFORM_SDK.GAMEPUSH_PUBLIC_TOKEN)}&callback=${callbackName}`;
    try {
      gp = await new Promise((resolve, reject) => {
        const finish = (sdk) => {
          if (sdk) {
            ...
            reject(new Error("GamePush returned an empty SDK instance"));
          }
        };
        globalThis[callbackName] = (sdk) => {
          Promise.resolve(sdk?.ready).then(() => finish(sdk)).catch(reject);
        };
        const existing = document.querySelector(`script[data-slither-gamepush="${PLATFORM_SDK.GAMEPUSH_PROJECT_ID}"]`);
        if (existing) {
          this.waitFor(() => this.getGamePush()).then((sdk) => sdk ? resolve(sdk) : reject(new Error("GamePush SDK did not become ready")));
          return;
        }
        ...
        script.src = src;
        script.onerror = () => reject(new Error("GamePush SDK script could not be loaded"));
        document.head.appendChild(script);
        setTimeout(() => reject(new Error("GamePush SDK initialization timed out")), PLATFORM_SDK.SDK_WAIT_MS);
      });
    } finally {
...
class SlitherArena {
  ...
  async start() {
    ...
    this.progress = await this.bridge.loadProgress();
