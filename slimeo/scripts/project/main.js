/*
  Snake: King of the Arena — Construct 3 JS runtime (ads/reward/leaderboard/cloud fixes v0.30)
  ---------------------------------------------------------------
...
class PlatformBridge {
  ...
  promiseWithAdCloseFallback(promise, timeoutMs, timeoutValue) {
    // Some VK/OK webviews return control to the page after an interstitial but never
    // resolve the GamePush promise. Treat focus/visibility returning, or a short
    // safety timeout, as a closed interstitial so the post-game action can continue.
    ...
    return Promise.race([
      ...
      new Promise((resolve) => {
        ...
        const onResume = () => {
          ...
          setTimeout(() => finish({ success: true, closedByResume: true }), 260);
        };
  ...
  async showInterstitial() {
    ...
    try {
      ...
      if (gp?.ads?.showFullscreen) {
        ...
        const result = await this.promiseWithAdCloseFallback(gp.ads.showFullscreen({ showCountdownOverlay: true }), 20000, { success: true, timeout: true });
...
class SlitherArena {
  ...
  async continueAfterGameOver(destination) {
    ...
    try {
      ...
      // The next screen must open even if a social-network interstitial promise hangs.
      try {
