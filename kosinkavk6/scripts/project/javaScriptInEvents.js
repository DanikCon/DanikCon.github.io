...
// Версия: событийная обработка GamePush-покупок — UI обновляется по purchase event без перезапуска + полные подсказки.
(async () => {
  "use strict";
    ...
    async purchase(productTag) {
      try {
        ...
        if (this.kind === "gamepush" && this.gp?.payments?.purchase) {
          ...
          // Only one checkout can be started from this UI at once. Remove stale pending tags from an
          // older cancelled/aborted overlay so an argument-less SUCCESS event can be mapped exactly.
          for (const staleTag of [...this.pendingPurchaseTags]) {
            this.settlePurchaseWaiter(staleTag, { ok: false, code: "replaced" });
          }
          this.pendingPurchaseTags.clear();
          this.pendingPurchaseTags.add(productTag);

          // Subscribe logically BEFORE opening the native checkout. This covers adapters where
          // purchase() is fire-and-forget or returns an already-resolved value while the real payment
          // finishes later. The official `purchase` event is the definitive success signal.
          const eventSignal = this.waitForPurchaseSignal(productTag);
          let purchaseCall;
          try {
            purchaseCall = this.gp.payments.purchase({ tag: productTag });
          } catch (error) {
            this.pendingPurchaseTags.delete(productTag);
            this.settlePurchaseWaiter(productTag, { ok: false, code: "unavailable", error });
            throw error;
          }

          const isPromisePurchase = Boolean(purchaseCall && typeof purchaseCall.then === "function");
          if (!isPromisePurchase) {
            // Do not poll the startup purchase cache here: it is known to stay stale on some hosts.
            // Wait for GamePush's documented success/error event instead.
            return await eventSignal;
          }

          // Raw GamePush SDK promises normally resolve with { product, purchase }. Race the promise
          // against the event because platform wrappers can implement only one of the two correctly.
          const promiseSignal = Promise.resolve(purchaseCall).then(
            result => ({ source: "promise", result }),
            error => ({ source: "promise-error", error })
          );
          const first = await Promise.race([
            eventSignal.then(result => ({ source: "event", result })),
            promiseSignal
          ]);

          if (first.source === "event") return first.result;
          if (first.source === "promise-error") {
            this.pendingPurchaseTags.delete(productTag);
            this.settlePurchaseWaiter(productTag, { ok: false, code: "cancelled", error: first.error });
          ...
          if (explicitFailure) {
            this.pendingPurchaseTags.delete(productTag);
            this.settlePurchaseWaiter(productTag, { ok: false, code: status.includes("cancel") ? "cancelled" : "unavailable" });
            return { ok: false, code: status.includes("cancel") ? "cancelled" : "unavailable" };
          }

          const resolvedTag = this.resolvePurchaseTag(result) || productTag;
          const explicitSuccess = result === true || result?.success === true || Boolean(result?.product || result?.purchase || this.resolvePurchaseTag(result));
          if (explicitSuccess) {
            this.pendingPurchaseTags.delete(productTag);
            this.purchasedProducts.add(resolvedTag);
            this.purchasedProducts.add(productTag);
            this.settlePurchaseWaiter(productTag, { ok: true, code: "success", productTag: resolvedTag, source: "promise", result });
            return { ok: true, code: "success", productTag: resolvedTag, result };
          }

          // A Promise that resolves with no purchase payload is not proof of payment. Keep waiting
          // for the documented success/error event rather than forcing the player to reload.
          return await eventSignal;
        }
      } catch (error) {
