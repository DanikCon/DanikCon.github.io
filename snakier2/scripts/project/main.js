/*
  Snake: King of the Arena — Construct 3 JS runtime (ads/reward/leaderboard/stability fixes v0.31)
  ---------------------------------------------------------------
...
class PlatformBridge {
  ...
  getStableDevicePortrait() {
    // Sticky banners change visualViewport/innerHeight in VK and OK webviews.
    // Use the physical device orientation first so the banner cannot change the
    // result of this check and trigger a show -> close -> show loop.
    ...
    if (Number.isFinite(legacyOrientation)) return Math.abs(legacyOrientation) % 180 === 0;

    const screenWidth = Number(globalThis.screen?.width);
    const screenHeight = Number(globalThis.screen?.height);
    if (Number.isFinite(screenWidth) && Number.isFinite(screenHeight) && screenWidth > 0 && screenHeight > 0) {
      return screenHeight >= screenWidth;
    }

    // Last-resort fallback for unusual embedded browsers without Screen APIs.
    const { width, height } = this.getViewportSize();
    return height >= width;
  }

  shouldShowSocialSticky() {
    // VK/OK: show sticky on desktop windows and on mobile devices in portrait.
    ...
    return this.getStableDevicePortrait();
  }
  ...
  async setBannerVisible(visible, force = false) {
    ...
    // VK/OK mobile webviews fire resize/visualViewport resize when a sticky banner appears.
    // Re-calling showSticky() on that resize can make the native banner immediately close
    // or flicker. Only call the SDK when the desired visibility actually changes.
