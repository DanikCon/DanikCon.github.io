...
// Версия: GamePush v1.49.10 — OK purchase without reload: keep Construct runtime awake and confirm entitlement live; deck_frost6/deck_amber6.
(async () => {
  "use strict";
  ...
  const CONFIG = {
    ...
    products: {
      ...
      frostDeck: "deck_frost6",
      amberDeck: "deck_amber6"
    },
  ...
  const DECKS = {
    ...
    frost6: { titleKey: "frost", kind: "purchase", product: "frostDeck", theme: { tableA: "#5f8fa8", tableB: "#183a52", accent: "#dff7ff", backA: "#b9ecff", backB: "#4f88b8", frontA: "#ffffff", frontB: "#e8f7ff" } },
    amber6: { titleKey: "amber", kind: "purchase", product: "amberDeck", theme: { tableA: "#8a5a1f", tableB: "#3d220d", accent: "#ffd77a", backA: "#d9902f", backB: "#7a4215", frontA: "#fffaf0", frontB: "#f6e4bf" } }
  };
  ...
  const platform = {
    ...
    async syncEntitlements(targetProfile) {
      ...
      const productToSkin = {
        ...
        [CONFIG.products.frostDeck]: "frost6",
        [CONFIG.products.amberDeck]: "amber6"
      };
    ...
    async purchase(productTag) {
      ...
      // On OK the GamePush purchase Promise can stay pending until a real browser focus/visibility
      // transition even though the entitlement is already present in payments.has()/purchases.
      // Keep checking the authoritative entitlement in parallel with the SDK Promise so the game can
      // finish the checkout immediately without waiting for a tab switch or a page reload.
      const purchaseIsRecorded = () => {
        try {
          if (this.hasPurchase(productTag)) return true;
        } catch (_) { /* Fall through to the raw purchase list. */ }

        try {
          const purchases = Array.isArray(this.gp?.payments?.purchases)
            ? this.gp.payments.purchases
            : [];
      ...
      const sdkPromise = Promise.resolve()
        .then(() => this.gp.payments.purchase({ tag: productTag }))
        .then(
      ...
      try {
        const outcome = await Promise.race([sdkPromise, entitlementPromise]);

        if (outcome?.type === "entitlement") {
          // Swallow a later SDK rejection (notably OK purchase_timeout) after we have already
          // confirmed the real purchase, so it cannot surface as an unhandled rejection.
          sdkPromise.catch(() => {});
        ...
        if (outcome?.type === "sdk-reject") {
          ...
          // The entitlement cache can update a fraction after the timeout/reject callback.
          for (let i = 0; i < 12; i++) {
        ...
        // Resolve the dormant poll promise as a no-op if the SDK branch completed first.
  ...
  function openStore(tab = shopTab) {
    ...
    const paid = ["royal", "noir", "ruby", "gold", "cosmos", "frost6", "amber6"];
  ...
  function beginPurchaseRuntimeAwake() {
    if (!isOkPurchaseContext()) return null;
    try {
      const constructRuntime = typeof runtime !== "undefined" ? runtime : null;
      const sdk = constructRuntime?.sdk;
      if (!sdk || typeof sdk.setSuspended !== "function") return null;
      const guard = {
        sdk,
        previousAutoSuspend: Boolean(sdk.isAutoSuspendEnabled)
      };
      // OK's payment layer can leave document.visibilityState="hidden" while the payment UI is
      // already gone. Prevent Construct from auto-suspending so the entitlement watcher and UI
      // can continue running without requiring a real browser tab switch.
      sdk.isAutoSuspendEnabled = false;
      if (constructRuntime.isSuspended) sdk.setSuspended(false);
      return guard;
    } catch (error) {
      console.warn("[Kosynka] Could not disable Construct auto-suspend for OK purchase", error);
      return null;
    }
  }

  function endPurchaseRuntimeAwake(guard) {
    if (!guard?.sdk) return;
    try {
      guard.sdk.isAutoSuspendEnabled = guard.previousAutoSuspend;
    } catch (error) {
      console.warn("[Kosynka] Could not restore Construct auto-suspend after OK purchase", error);
    }
  }
  ...
  async function purchaseSkin(id) {
    ...
    const runtimeAwakeGuard = beginPurchaseRuntimeAwake();
    try {
      ...
      // The entitlement is already confirmed either by the SDK result or by live payments.has()/purchases.
      // Grant the exact deck that launched checkout and redraw the store before restoring auto-suspend.
      ...
      endPurchaseRuntimeAwake(runtimeAwakeGuard);
    }
  }
