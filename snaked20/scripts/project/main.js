...
globalThis.__SLITHER_BUILD__ = "28-stable-ui-rollback";
/*
...
class PlatformBridge {
  ...
  shouldShowSocialSticky() {
    // Keep the platform sticky enabled in both orientations. UI safety is handled by
    // the scrollable overlay lane, not by resizing/scaling every panel.
    return this.platform === "vk" || this.platform === "ok";
  }
...
class SlitherArena {
  ...
  async start() {
    ...
    this.bindInput();
    const syncOverlayTouchMode = () => {
      const visible = !!this.ui?.overlay?.classList?.contains("visible");
      if (this.root) this.root.style.touchAction = visible ? "pan-y" : "none";
    };
    try {
      new MutationObserver(syncOverlayTouchMode).observe(this.ui.overlay, { attributes: true, attributeFilter: ["class"] });
    } catch (_error) { /* old embedded webviews can safely skip this observer */ }
    syncOverlayTouchMode();
    this.resize();
  ...
  createDom() {
    ...
    this.root.innerHTML = `
      <style>
        ...
        /* v28 stable UI rollback -------------------------------------------------
           One rule set for every screen. No whole-panel scaling, no visualViewport
           translation, no absolute centering tricks. The overlay scrolls when the
           platform banner or browser chrome reduces the usable height. */
        #slither-arena-root {
        ...
        #slither-arena-root.sa-sticky-safe .sa-overlay {
          ...
          scroll-padding-bottom:max(128px,calc(env(safe-area-inset-bottom) + 118px));
        }
        #slither-arena-root .sa-overlay > .sa-panel,
        #slither-arena-root .sa-overlay > .sa-menu-panel,
        #slither-arena-root .sa-overlay > .sa-missions-panel,
        ...
        #slither-arena-root .sa-overlay > .sa-leaderboard-panel {
          ...
          display:block;
          flex:0 0 auto !important;
          width:min(548px,calc(100% - 4px)) !important;
        ...
        #slither-arena-root .sa-menu-panel .sa-menu-copy,
        ...
        #slither-arena-root .sa-missions-panel .sa-mission-grid,
        #slither-arena-root .sa-missions-panel .sa-mission-card {
          height:auto !important;
          min-height:0 !important;
          overflow:visible !important;
        }
        #slither-arena-root .sa-leaderboard-panel,
        #slither-arena-root .sa-leaderboard-list {
          overflow:visible !important;
        }
        ...
        @media (orientation:landscape) and (pointer:coarse) {
          ...
          #slither-arena-root.sa-sticky-safe .sa-overlay {
            padding-bottom:max(106px,calc(env(safe-area-inset-bottom) + 96px)) !important;
          }
          #slither-arena-root .sa-overlay > .sa-panel,
          #slither-arena-root .sa-overlay > .sa-menu-panel,
          #slither-arena-root .sa-overlay > .sa-missions-panel,
          ...
          #slither-arena-root .sa-overlay > .sa-gameover-panel,
          #slither-arena-root .sa-overlay > .sa-leaderboard-panel {
            width:min(860px,calc(100% - 8px)) !important;
        ...
        @media (max-width:420px) {
          #slither-arena-root .sa-overlay { padding:7px !important; }
          #slither-arena-root .sa-overlay > .sa-panel,
          #slither-arena-root .sa-overlay > .sa-menu-panel,
          #slither-arena-root .sa-overlay > .sa-missions-panel,
          #slither-arena-root .sa-overlay > .sa-controls-panel,
