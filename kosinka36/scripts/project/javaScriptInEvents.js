...
// Версия: GamePush v1.48 — правки модерации VK/OK: 3 подсказки на новую игру и мгновенная выдача покупок OK без ожидания синхронизации.
(async () => {
  "use strict";
  ...
  const CONFIG = {
    ...
    products: {
      ...
      frostDeck: "deck_frost1",
      amberDeck: "deck_amber1"
    },
  ...
  const DECKS = {
    ...
    frost: { titleKey: "frost", kind: "purchase", product: "frostDeck", theme: { tableA: "#5f8fa8", tableB: "#183a52", accent: "#dff7ff", backA: "#b9ecff", backB: "#4f88b8", frontA: "#ffffff", frontB: "#e8f7ff" } },
    amber: { titleKey: "amber", kind: "purchase", product: "amberDeck", theme: { tableA: "#8a5a1f", tableB: "#3d220d", accent: "#ffd77a", backA: "#d9902f", backB: "#7a4215", frontA: "#fffaf0", frontB: "#f6e4bf" } }
  };
  ...
  const platform = {
    ...
    async syncEntitlements(targetProfile) {
      ...
      const productToSkin = {
        ...
        [CONFIG.products.frostDeck]: "frost",
        [CONFIG.products.amberDeck]: "amber"
      };
    ...
    async purchase(productTag) {
      try {
        ...
        if (this.kind === "gamepush" && this.gp?.payments?.purchase) {
          ...
          // Mark the exact product before opening the native checkout. This is required on OK:
          // returning focus can happen before the SDK refreshes payments.purchases, so focus recovery
          // must know which product to re-check and grant without a full game restart.
          ...
          // OK can resolve purchase() after the native checkout succeeds while returning an empty or
          // platform-specific receipt. Do not block the player for several seconds waiting for the
          // GamePush purchases cache: a resolved checkout with no explicit failure is enough to grant
          // the permanent entitlement immediately on OK. Refresh the catalogue only in the background.
          if (this.gamePushPlatform === "ok") {
            this.pendingPurchaseTags.delete(productTag);
            this.purchasedProducts.add(productTag);
            Promise.resolve(this.refreshCommerce({ preserveSession: true, forceFetch: true })).catch(error =>
              console.warn("[Kosynka] OK purchase background catalogue refresh failed", error)
            );
            return { ok: true, code: "success", productTag, source: "ok-resolved-checkout", result: receipt };
          }

          // Other GamePush platforms keep the conservative retry path for adapters that return an
          // ambiguous result. This does not affect OK, where the entitlement is already visible.
          ...
          // Do not show a false failure. The focus/visibility recovery and purchase event listener
          // will continue checking this exact tag and apply it as soon as the platform reports ownership.
          return { ok: false, code: "pending", productTag, result: receipt };
        }
      } catch (error) {
        console.warn("[Kosynka] Purchase failed", error);
        // The documented SUCCESS event can win a race against a rejected/late checkout promise.
        // In that case the entitlement is already granted and must not be turned back into a failure.
        if (this.purchasedProducts.has(productTag)) {
          this.pendingPurchaseTags.delete(productTag);
          return { ok: true, code: "success", productTag, source: "purchase-event", error };
        }
        // A checkout error can still race a successful OK callback. Verify ownership once before
        // treating it as cancelled; otherwise keep no stale pending tag on a real failure.
        try {
          const confirmed = await this.confirmPurchase(productTag);
          if (confirmed) {
            this.pendingPurchaseTags.delete(productTag);
            this.purchasedProducts.add(productTag);
            return { ok: true, code: "success", productTag, error };
          }
        } catch (_) {}
        this.pendingPurchaseTags.delete(productTag);
  ...
  function openStore(tab = shopTab) {
    ...
    const paid = ["royal", "noir", "ruby", "gold", "cosmos", "frost", "amber"];
  ...
  async function purchaseSkin(id) {
    ...
    await handlePlatformPurchaseSuccess(confirmedTag);
    openStore("purchase");
