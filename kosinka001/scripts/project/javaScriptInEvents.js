...
// Версия: GamePush v1.49.3 — финальные deck_frost2/deck_amber2 + мгновенная выдача + окно ожидания покупки.
(async () => {
  "use strict";
  ...
  const platform = {
    ...
    async reconcilePendingPurchases() {
      ...
      try {
        // OK can return focus before its purchases cache is refreshed. Retry the SDK catalogue
        // for every pending product so the entitlement appears without restarting the game.
        for (const tag of [...this.pendingPurchaseTags]) {
          const confirmed = await this.confirmPurchase(tag);
    ...
    async purchase(productTag) {
      try {
        ...
        if (this.kind === "gamepush" && this.gp?.payments?.purchase) {
          ...
          // Keep one authoritative active checkout. This avoids OK success callbacks with an empty
          // payload being confused by stale pending tags from older purchase attempts.
          ...
          // IMPORTANT: payments.purchase() is the checkout operation itself. Once that promise resolves
          // without an explicit failure, the game grants the product selected by the player immediately.
          // We no longer wait for payments.purchases/has()/fetchProducts() or platform detection.
        ...
        // A documented purchase SUCCESS event may arrive before/while the checkout promise rejects.
        // If it already credited the session, keep the purchase successful.
