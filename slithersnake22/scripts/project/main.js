...
class SlitherArena {
  ...
  createDom() {
    ...
    this.root.innerHTML = `
      <style>
        ...
        /* v0.37 release hotfix: preserve the original design, but never crop menu content
           inside VK/OK/Safari. VisualViewport fitting in JS handles the actual visible
           height; these rules remove legacy internal clipping and fixed-height leftovers. */
        #slither-arena-root .sa-overlay {
          overflow:hidden !important;
          box-sizing:border-box !important;
        }
        ...
        #slither-arena-root .sa-overlay > .sa-shop-panel,
        #slither-arena-root .sa-overlay > .sa-gameover-panel,
        #slither-arena-root .sa-overlay > .sa-leaderboard-panel {
          height:auto !important;
          min-height:0 !important;
          max-height:none !important;
          overflow:visible !important;
  ...
  fitOverlayPanel(renderPreviews = false) {
    ...
    // iOS Safari and VK/OK WebViews may expose a layout viewport that is taller than
    // the pixels the player can actually see after browser chrome is applied. Anchor
    // menu overlays to VisualViewport when available, so centering and fitting use the
    // real visible rectangle rather than the hidden page area behind Safari/VK bars.
    const vv = globalThis.visualViewport;
    const rootRect = this.root?.getBoundingClientRect?.() ?? { left: 0, top: 0 };
    if (vv && Number.isFinite(vv.width) && Number.isFinite(vv.height) && vv.width > 0 && vv.height > 0) {
      overlay.style.left = `${Math.max(0, vv.offsetLeft - (rootRect.left || 0))}px`;
      overlay.style.top = `${Math.max(0, vv.offsetTop - (rootRect.top || 0))}px`;
    ...
    // Measure at natural size first. Do not clip the card while measuring: a few old
    // compact-grid rules can make the action rows extend beyond offsetHeight even
    // though they are still part of the visible menu.
    ...
    const viewportWidths = [bounds.width, globalThis.innerWidth, vv?.width, globalThis.document?.documentElement?.clientWidth]
      .map(Number).filter((value) => Number.isFinite(value) && value > 1);
    const viewportHeights = [bounds.height, globalThis.innerHeight, vv?.height, globalThis.document?.documentElement?.clientHeight]
      .map(Number).filter((value) => Number.isFinite(value) && value > 1);
    const visibleWidth = viewportWidths.length ? Math.min(...viewportWidths) : bounds.width;
    const visibleHeight = viewportHeights.length ? Math.min(...viewportHeights) : bounds.height;
    const reserve = this.isTouchInputEnvironment() ? 18 : 24;
    const availableWidth = Math.max(1, visibleWidth - reserve);
    const availableHeight = Math.max(1, visibleHeight - reserve);

    // scrollHeight is normally enough, but WebKit can under-report descendants that
    // overflow an old flex/grid box. Include the furthest visible descendant explicitly.
    const panelRect = panel.getBoundingClientRect();
    let contentRight = panelRect.right;
    let contentBottom = panelRect.bottom;
    for (const element of panel.querySelectorAll("*")) {
      const style = globalThis.getComputedStyle?.(element);
      if (style?.display === "none" || style?.visibility === "hidden") continue;
      const rect = element.getBoundingClientRect?.();
      if (!rect || !Number.isFinite(rect.right) || !Number.isFinite(rect.bottom)) continue;
      contentRight = Math.max(contentRight, rect.right);
      contentBottom = Math.max(contentBottom, rect.bottom);
    }
    const rawWidth = Math.max(panel.offsetWidth || 0, panel.scrollWidth || 0, contentRight - panelRect.left, 1);
    const rawHeight = Math.max(panel.offsetHeight || 0, panel.scrollHeight || 0, contentBottom - panelRect.top, 1);

    // One scale for the whole card: no scrolling, no hidden lower buttons.
