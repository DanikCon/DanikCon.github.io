...
// Версия: GamePush v1.49.16 — native FAPI payments for every OK purchase (all paid decks + remove_ads), platform auto-reload on success; VK unchanged; deck_frost7/deck_amber7.
(async () => {
  "use strict";
  ...
  const platform = {
    ...
    async purchase(productTag) {
      ...
      // The GamePush OK adapter records the purchase correctly on its backend, but its client
      // Promise remains suspended until a real browser focus transition. Bypass only that broken
      // client wrapper: use the official OK FAPI dialog with callback=false, so the OK container
      // itself reloads the app after the server validates a successful transaction. VK/Yandex keep
      // using the unchanged GamePush payments.purchase() flow below.
      if (isOkPurchaseContext()) return await startNativeOkPurchase(productTag);
  ...
  let okFapiLoadPromise = null;

  function okPurchaseFallbackName(productTag) {
    if (productTag === CONFIG.products.removeAds) return t("noAds");
    const skinId = purchasedSkinFromTag(productTag);
    return skinId && DECKS[skinId] ? deckTitle(skinId) : productTag;
  }

  function okPurchaseDescription(productTag, productName) {
    const product = platform.getProduct(productTag);
    const description = product?.description ?? product?.desc ?? product?.shortDescription;
    if (description !== undefined && description !== null && String(description).trim()) {
      return String(description).trim();
    }
    if (productTag === CONFIG.products.removeAds) return t("noAdsDesc");
    return `${t("deckPremium")}: ${productName}`;
  }

  function okPurchasePrice(productTag) {
    const product = platform.getProduct(productTag);
    const candidates = [
      product?.price,
      product?.amount,
  ...
  function loadOkFapiScript() {
    if (globalThis.FAPI?.UI?.showPayment) return Promise.resolve(globalThis.FAPI);
    if (okFapiLoadPromise) return okFapiLoadPromise;
    okFapiLoadPromise = new Promise((resolve, reject) => {
      const existing = document.querySelector('script[src*="api.ok.ru/js/fapi5.js"]');
      const finish = () => {
        if (globalThis.FAPI?.UI?.showPayment) resolve(globalThis.FAPI);
        else reject(new Error("OK FAPI loaded without UI.showPayment"));
      };
      if (existing) {
        existing.addEventListener("load", finish, { once: true });
        existing.addEventListener("error", () => reject(new Error("OK FAPI script failed")), { once: true });
      ...
      script.onerror = () => reject(new Error("OK FAPI script failed"));
      document.head.appendChild(script);
    });
    return okFapiLoadPromise;
  }

  async function ensureOkFapi() {
    const alreadyReady = globalThis.FAPI?.UI?.showPayment;
    const fapi = alreadyReady ? globalThis.FAPI : await loadOkFapiScript();

    // GamePush normally loads and initializes FAPI already. Do not reinitialize that shared SDK,
    // because doing so can replace its internal message channel. Initialize only when we loaded it.
    if (alreadyReady) return fapi;

    const params = (() => {
      try { return fapi.Util?.getRequestParameters?.() || {}; }
      catch (_) {
        try { return Object.fromEntries(new URLSearchParams(location.search || "")); }
        catch (_) { return {}; }
      }
    })();
    const apiServer = params.api_server || params.apiServer;
    const apiConnection = params.apiconnection || params.apiConnection;
    if (!apiServer || !apiConnection || typeof fapi.init !== "function") {
      throw new Error("OK FAPI launch parameters are missing");
    }
    await new Promise((resolve, reject) => {
      try {
        fapi.init(apiServer, apiConnection, resolve, error => reject(error || new Error("OK FAPI init failed")));
      } catch (error) {
        reject(error);
      }
    });
  ...
  async function startNativeOkPurchase(productTag) {
    if (!isOkPurchaseContext()) return { ok: false, code: "unavailable" };
    const product = platform.getProduct(productTag);
    const price = okPurchasePrice(productTag);
    if (!product || !price) {
      console.warn("[Kosynka] Native OK purchase unavailable: product or integer OK price is missing", productTag, product);
      return { ok: false, code: "unavailable" };
    }

    const name = platform.getProductName(productTag, okPurchaseFallbackName(productTag));
    const description = okPurchaseDescription(productTag, name);

    // Preserve the current deal/profile before OK replaces the iframe after successful payment.
    if (game && profile) {
      try { localWrite(buildPayload()); } catch (_) { }
    }

    try {
      const fapi = await ensureOkFapi();
      // callback="false" is the official OK mode where the platform itself reloads the
      // application after the developer's server validates the transaction. The product code and
      // price come from the same GamePush catalogue that the existing checkout used, so GamePush's
      // configured OK callback can keep validating/recording every product, including remove_ads.
      fapi.UI.showPayment(
        String(name),
        String(description),
        String(productTag),
      ...
      return { ok: true, code: "native_ok_started", productTag, price };
    } catch (error) {
      console.warn("[Kosynka] Native OK payment failed to open", error);
      return { ok: false, code: "unavailable", error };
    }
  }
  ...
  async function purchaseSkin(id) {
    ...
    // OK uses the native FAPI flow and lets the platform reload the iframe only after successful
    // server validation. Do not create GamePush watchers or grant the deck on the client here.
    if (platform.kind === "gamepush" && isOkPurchaseContext()) {
      ...
      if (!purchase?.ok || purchase?.code !== "native_ok_started") toast(t("purchaseFailed"));
      return;
    }
  ...
  async function purchaseNoAds() {
    ...
    // Same native OK flow as every premium deck. The no-ads entitlement is restored from
    // GamePush purchases on the platform-triggered reload; VK remains on its existing flow.
    if (platform.kind === "gamepush" && isOkPurchaseContext()) {
      const purchase = await runExternalInteraction(
        () => platform.purchase(productTag),
        { ok: false, code: "unavailable" }
