...
// Версия: GamePush v1.49.4 — финальные deck_frost4/deck_amber4 + отдельное подтверждение покупок для OK.
(async () => {
  "use strict";
  ...
  const platform = {
    ...
    bindPurchaseEvents() {
      ...
      this.gp.payments.on("close", () => {
        ...
        // Crucial for OK: close is independent from the purchase() Promise. As soon as the
        // native payment window closes, start checking the authoritative purchases cache.
        Promise.resolve(this.verifyPendingPurchase(tag, {
          maxAttempts: this.gamePushPlatform === "ok" ? 24 : 8,
          intervalMs: this.gamePushPlatform === "ok" ? 600 : 500
        })).catch(error => console.warn("[Kosynka] Purchase close verification failed", error));
      });
        ...
        if (this.kind === "gamepush" && this.gp?.payments?.purchase) {
          ...
          if (this.gamePushPlatform === "ok") {
            ...
            Promise.resolve(checkoutPromise).then(receipt => {
              ...
              if (!explicitSuccess) {
                Promise.resolve(this.verifyPendingPurchase(productTag, { maxAttempts: 24, intervalMs: 600 }))
                  .catch(error => console.warn("[Kosynka] Ambiguous OK checkout verification failed", error));
                return;
              }
              ...
              Promise.resolve(this.verifyPendingPurchase(productTag, { maxAttempts: 24, intervalMs: 600 }))
                .catch(verifyError => console.warn("[Kosynka] Rejected OK checkout verification failed", verifyError));
            });
            ...
            setTimeout(() => {
              ...
              Promise.resolve(this.verifyPendingPurchase(productTag, { maxAttempts: 24, intervalMs: 600 }))
                .catch(error => console.warn("[Kosynka] OK background purchase verification failed", error));
            }, 1000);
