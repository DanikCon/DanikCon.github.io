    ...
    if (globalThis.portalTimer <= 0) {
        ...
        if (!document.getElementById('victory-screen')) {
            ...
            if (currentNum >= TOTAL_LEVELS) {
                ...
                if (window.location.search.includes('logged_user_id') || window.location.search.includes('session_key')) {
                    gpPlatform = 'OK';
                } else if (typeof gp !== 'undefined' && gp.platform && gp.platform.type === 'OK') {
                    gpPlatform = 'OK';
                }

                if (socialContainer) {
                    socialContainer.innerHTML = `
                        <button id="btn-invite" class="btn-social btn-invite" style="pointer-events: auto; position: relative; z-index: 10000;">🤝 ${globalThis.t('btnInvite')}</button>
                        <button id="btn-subscribe" class="btn-social ${gpPlatform === 'VK' ? 'btn-vk' : 'btn-ok'}" style="pointer-events: auto; position: relative; z-index: 10000;">
                            ${gpPlatform === 'VK' ? 'VK' : 'OK'} ${globalThis.t('btnSubscribe')}
                        </button>
                    `;

                    const handleSocialClick = (type) => {
                        const inst = window.gp || window.gamepush;
                        if (!inst || !inst.social) {
                            console.warn("Плагин не найден, пробуем напрямую...");
                            alert("Социальные функции загружаются, подожди пару секунд!");
                            return;
                        }
                        ...
                        if (type === 'invite') {
                            ...
                            inst.social.joinCommunity({ groupId: myGroupId });
                        }
                    };

                    document.getElementById('btn-invite').onclick = () => handleSocialClick('invite');
                    document.getElementById('btn-subscribe').onclick = () => handleSocialClick('subscribe');
                }
