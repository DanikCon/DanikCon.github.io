...
class PlatformBridge {
  ...
  shouldShowSocialSticky() {
    // VK/OK sticky is used only on menu/overlay screens; gameplay explicitly closes it.
    // Do not derive visibility from visualViewport dimensions here: VK Direct Games
    // reserves about 110px when the sticky renders, which fires resize and can change
    // height >= width. That previously made the game close the banner immediately
    // after it appeared, producing a one-second flash.
    return this.platform === "vk" || this.platform === "ok";
  }
  ...
  async setBannerVisible(visible, force = false) {
    ...
    // The desired state is intentionally independent of the resized viewport, so the
    // resize handler cannot flip it to false and close the freshly rendered banner.
    // Only call the SDK when the desired visibility actually changes.
