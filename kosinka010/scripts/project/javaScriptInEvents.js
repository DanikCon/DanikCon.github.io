...
// Версия: GamePush v1.49.4a — deck_frost4/deck_amber4 + автоматическое обновление покупок OK после checkout.
(async () => {
  "use strict";
  ...
  const CONFIG = {
    ...
    products: {
      ...
      frostDeck: "deck_frost4",
      amberDeck: "deck_amber4"
    },
  ...
  const DECKS = {
    ...
    frost4: { titleKey: "frost", kind: "purchase", product: "frostDeck", theme: { tableA: "#5f8fa8", tableB: "#183a52", accent: "#dff7ff", backA: "#b9ecff", backB: "#4f88b8", frontA: "#ffffff", frontB: "#e8f7ff" } },
    amber4: { titleKey: "amber", kind: "purchase", product: "amberDeck", theme: { tableA: "#8a5a1f", tableB: "#3d220d", accent: "#ffd77a", backA: "#d9902f", backB: "#7a4215", frontA: "#fffaf0", frontB: "#f6e4bf" } }
  };
  ...
  const platform = {
    ...
    schedulePostCheckoutChecks(productTag) {
      ...
      this.clearPostCheckoutChecks(productTag);
      // Absolute delays after the native checkout closes/resolves. This reproduces the refresh that
      // previously happened only when the player switched to another browser tab and came back.
      ...
      const timers = delays.map(delay => setTimeout(() => {
        ...
        Promise.resolve(this.verifyPurchaseOnce(productTag, `post-checkout-${delay}ms`))
          .catch(error => console.warn("[Kosynka] Scheduled purchase refresh failed", error));
      }, delay));
    ...
    async syncEntitlements(targetProfile) {
      ...
      const productToSkin = {
        ...
        [CONFIG.products.frostDeck]: "frost4",
        [CONFIG.products.amberDeck]: "amber4"
      };
  ...
  function openStore(tab = shopTab) {
    ...
    const paid = ["royal", "noir", "ruby", "gold", "cosmos", "frost4", "amber4"];
