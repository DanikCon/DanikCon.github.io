...
// Версия 1.27: Game Ready отправляется вручную только на Yandex после preloader и полной готовности интерактивного UI.
(async () => {
  "use strict";
  ...
  const CONFIG = {
    ...
    gamePushBestScoreField: "solitaire_best_score", // Main field displayed and sorted in the leaderboard.
    gamePushLegacyBestScoreField: "score", // Also kept in sync for compatibility, but not displayed in the leaderboard.
    gamePushWinsField: "solitaire_wins",
  ...
  const I18N = Object.freeze({
    ...
    en: {
      ...
      wins:"<b>{count}</b> wins", streak:"<b>{count}</b> streak", record:"Record: <b>{count}</b>", bestTime:"Best time: <b>{time}</b>", hintsStat:"Hints: <b>{count}</b>", menuIntro:"Classic solitaire. Build every suit from Ace through King.", continueGame:"Continue game", startGame:"Start game", newGame:"New game", store:"Store", achievements:"Achievements", leaderboard:"Leaderboard", leaderboardInfo:"Top 10 by record", leaderboardWins:"Record", leaderboardUnavailable:"Leaderboard is not available right now", howTo:"How to play", settings:"Settings", backMenu:"Back to menu", inMenu:"Menu", achievementsOpen:"Unlocked: <b>{count}</b> of {total}.",
  ...
  function queueMoveFlight(cards) {
    ...
    if (captured.length) pendingFlights.push(captured);
  }

  function playPendingFlights() {
    if (!app || !pendingFlights.length) return;
    cancelAnimationFrame(flightFrame);
    // Первый кадр позволяет пересчитать высоту столбцов; во втором уже есть точные цели.
    flightFrame = requestAnimationFrame(() => {
      flightFrame = requestAnimationFrame(() => {
        ...
        batches.flat().forEach((flight, index) => {
          ...
          clone.classList.remove("tableau-card", "selected", "pulse", "drag-source", "deal-in", "draw-in", "flip-in", "arrival");
          ...
          target.classList.add("arrival");
          const duration = 205 + Math.min(index, 5) * 28;
          const animation = clone.animate([
            { left: `${flight.from.left}px`, top: `${flight.from.top}px`, width: `${flight.from.width}px`, height: `${flight.from.height}px`, transform: "rotate(-2deg) scale(.98)", opacity: .96 },
            { left: `${to.left}px`, top: `${to.top}px`, width: `${to.width}px`, height: `${to.height}px`, transform: "rotate(0deg) scale(1)", opacity: 1 }
          ], { duration, easing: "cubic-bezier(.18,.88,.25,1)", fill: "forwards" });
          animation.onfinish = () => {
            clone.remove();
  ...
  const platform = {
    ...
    async refreshCommerce() {
      ...
      try {
        ...
        // GamePush загружает каталог и покупки при инициализации SDK. Deprecated fetchProducts
        // используем только как совместимый fallback для старой версии плагина/кэша.
        ...
        if ((!products || !purchases) && typeof payments.fetchProducts === "function") {
          const data = await payments.fetchProducts();
          if (!products) products = Array.isArray(data) ? data : (data?.products || data?.items || []);
          if (!purchases) purchases = Array.isArray(data?.playerPurchases) ? data.playerPurchases
        ...
        for (const item of purchases) {
          let tag = this.productTagFrom(item);
          if (!tag) {
    ...
    async fetchBestScoreLeaderboard() {
      ...
      try {
        ...
        // visible rating is sorted by the project's actual field: solitaire_best_score.
        return await this.gp.leaderboard.fetch({
          orderBy: [CONFIG.gamePushBestScoreField],
          order: "DESC",
          limit: CONFIG.leaderboardLimit,
          withMe: "last",
          showNearest: 2,
          includeFields: [CONFIG.gamePushBestScoreField]
        });
      } catch (error) {
    ...
    async loginForPurchase() {
      ...
      try {
        ...
        // The account context changed: fetch permanent purchases before retrying checkout.
        await this.refreshCommerce();
    ...
    async purchase(productTag) {
      try {
        ...
        if (this.kind === "gamepush" && this.gp?.payments?.purchase) {
          // Do not open checkout for a guest profile. It keeps permanent entitlements tied to a real account.
          ...
          const result = await this.gp.payments.purchase({ tag: productTag });
          const cancelled = result === false || result?.success === false || result?.status === "cancelled" || result?.status === "canceled";
          if (cancelled) return { ok: false, code: "cancelled" };
          await this.refreshCommerce();
          const confirmed = this.hasPurchase(productTag) || result?.success === true || Boolean(result?.product || result?.purchase);
          return confirmed ? { ok: true, code: "success" } : { ok: false, code: "unavailable" };
        }
      } catch (error) {
  ...
  function findHintCandidates() {
    ...
    // Любая уже открывшаяся рубашка — действие без риска и с новой информацией.
    for (let pile = 0; pile < 7; pile++) {
    ...
    for (let pile = 0; pile < 7; pile++) {
      game.tableaus[pile].forEach((card, index) => {
    ...
    // Самый полезный ход в Косынке — тот, который открывает скрытую карту.
    for (const { origin, cards } of tableauOrigins) {
      for (let pile = 0; pile < 7; pile++) {
        ...
        if (opensHidden) {
          ...
          // Не выдаём перемещение короля в пустую колонку за «лучший» ход, если оно ничего не открывает.
          add({
            kind: "move",
            origin,
            destination: { zone: "tableau", pile },
            priority: 6900 + Math.min(120, cards.length * 10),
    ...
    // Перенос верхней карты на базу. Подсказка называет безопасным только ход,
    // после которого обе масти противоположного цвета не отстают более чем на одну карту.
    ...
    for (const { origin, card } of foundationOrigins) {
      ...
      if (opensHidden) {
        ...
        add({
          ...
          origin,
          destination: { zone: "foundation", pile },
          priority: 11100 + (freesWaste ? 750 : 0),
          message: freesWaste
            ? t("hintFoundationSafeWaste", { card: cardText(card) })
            : t("hintFoundationSafe", { card: cardText(card) })
        });
      } else {
        // Такой ход легален, но специально не ставится выше поиска новой информации.
        add({
          kind: "move",
          origin,
          destination: { zone: "foundation", pile },
    ...
    // Карта из сброса приоритетна: после переноса может стать доступной следующая карта.
    if (wasteTop) {
      for (let pile = 0; pile < 7; pile++) {
        if (!canPlaceOnTableau(wasteTop, game.tableaus[pile])) continue;
        const freesWaste = topWasteWouldBecomeUseful();
        add({
          kind: "move",
          origin: { zone: "waste", pile: 0 },
          destination: { zone: "tableau", pile },
          priority: freesWaste ? 12650 : 9650,
          message: freesWaste
    ...
    // Если на столе нет сильного хода, проверяем колоду. Имя закрытой карты не раскрываем.
    if (game.stock.length) {
      add({
        kind: "stock",
        priority: previewStockHasMove() ? 9100 : 7600,
        key: "stock",
        message: previewStockHasMove()
          ? t("hintStockMove")
          : t("hintStock")
      });
      ...
      add({
        ...
        priority: 7550,
        key: "recycle",
        message: t("hintRecycle")
      });
    }

    // Ход с базы назад на стол — только запасной вариант, когда обычных ходов не осталось.
  ...
  function autoFinish() {
    ...
    while (guard < 160 && !game.finished) {
      ...
      if (!move || !makeMove(move.origin, move.destination)) break;
      movedAny = true;
      guard++;
    }
  ...
  function leaderboardScoreFromPlayer(player) {
    if (!player || typeof player !== "object") return 0;
    const field = CONFIG.gamePushBestScoreField;
  ...
  function leaderboardRowsHtml(result) {
    ...
    return rows.slice(0, CONFIG.leaderboardLimit).map((player, index) => {
      ...
      const score = leaderboardScoreFromPlayer(player);
      return `<div class="achievement-row"><span class="achievement-mark">${position}</span><div><strong>${escapeHtml(name)}</strong><small>${escapeHtml(t("leaderboardWins"))}: <b>${score}</b></small></div></div>`;
    }).join("");
  }
  ...
  async function openLeaderboard() {
    ...
    try {
      ...
      const result = await Promise.race([platform.fetchBestScoreLeaderboard(), timeout]);
      const content = app?.querySelector("#k-leaderboard-content");
      if (!content) return;
  ...
  function purchaseLabel(productTag) {
    ...
    return price ? `${t("buy")} ${price}` : t("buy");
  }
  ...
  function deckTile(id) {
    ...
    else action = `<button class="deck-action" data-action="buy-skin" data-skin="${id}">${purchaseLabel(productTag)}</button>`;
  ...
  function openStore(tab = shopTab) {
    ...
    const noAds = shopTab === "purchase"
      ? `<article class="deck-tile deck-noads"><div class="deck-preview noads">${uiIcon("noads", "deck-noads-icon")}</div><div class="deck-copy"><strong>${escapeHtml(noAdsProductName)}</strong><small>${t("noAdsDesc")}</small></div>${profile.noAds ? `<button class="deck-action muted" disabled>${t("bought")}</button>` : `<button class="deck-action" data-action="buy-noads">${purchaseLabel(CONFIG.products.removeAds)}</button>`}</article>`
      : "";
    showModal(t("store"), `
      <div class="shop-tabs">
  ...
  function cardHtml(card, origin, extraClass = "", extraStyles = []) {
    ...
    if (!card.faceUp) return `<div class="card back ${effects} ${extraClass}" ${attributes}${inlineStyle} aria-label="${escapeHtml(t("faceDown"))}"></div>`;
    const color = suit.color || "black";
    return `<div class="card face ${color} ${selected} ${effects} ${extraClass}" ${attributes}${inlineStyle} aria-label="${cardText(card)}">
  ...
  function foundationHtml(pile, index) {
    ...
    return `<div class="pile foundation foundation-${suit.key.toLowerCase()}" data-zone="foundation" data-pile="${index}">${cardHtml(top, { zone: "foundation", pile: index })}</div>`;
  }
  ...
  function buildApp() {
    ...
    style.textContent = `
      ...
      .k-app{height:var(--viewport-h,100dvh);min-height:0;max-height:var(--viewport-h,100dvh);display:grid;grid-template-rows:auto minmax(0,1fr) auto;padding-top:var(--ad-reserve-top,0px);padding-right:var(--ad-reserve-w,0px);padding-bottom:var(--ad-reserve-h,0px);overflow:hidden;background:radial-gradient(115% 75% at 50% -15%,color-mix(in srgb,var(--tableA) 88%,white 12%),var(--tableA) 48%,var(--tableB));}
      ...
      /* Низняя безопасная зона существует только когда SDK действительно показал sticky-баннер. */
      #kosynka-c3-root.has-platform-banner[data-banner-placement="bottom"]{--ad-reserve-h:clamp(50px,7dvh,74px)}#kosynka-c3-root.has-platform-banner[data-banner-placement="top"]{--ad-reserve-top:clamp(54px,11dvh,110px)}#kosynka-c3-root.has-platform-banner[data-banner-placement="right"]{--ad-reserve-w:clamp(118px,13vw,190px)}.reward-text-action{gap:0!important}
  ...
  function handleCardClick(origin) {
    ...
    if (selection) {
      const target = origin.zone === "foundation" ? { zone: "foundation", pile: origin.pile } : { zone: "tableau", pile: origin.pile };
      if (makeMove(selection, target)) return;
    }
    selection = origin;
    render();
  }
  ...
  function handlePileClick(pile) {
    ...
    if (zone === "tableau" && pile.classList.contains("empty") && selection) {
      makeMove(selection, { zone: "tableau", pile: Number(pile.dataset.pile) });
      return;
    }
    if (zone === "foundation" && selection) makeMove(selection, { zone: "foundation", pile: Number(pile.dataset.pile) });
  }
  ...
  function beginDrag(event, origin) {
    ...
    // Захватываем указатель на самой карте. При захвате на корневом контейнере
    // браузер отправлял завершающий click в контейнер, а не в карту.
    try { event.target?.setPointerCapture?.(event.pointerId); } catch (_) { /* Pointer capture не поддержан. */ }
  }
  ...
  function moveDrag(event) {
    ...
    visual.style.left = `${event.clientX}px`;
    visual.style.top = `${event.clientY}px`;
  }
  ...
  function finishDrag(event) {
    ...
    // Обычный короткий клик должен пройти в обработчик click. Блокируем только
    // завершающий click после настоящего перетаскивания, иначе выбор карты кликом терялся.
    if (wasActive) suppressClicksUntil = Date.now() + 260;
    clearDragVisual();
    if (!wasActive) return;
    ...
    // Главное исправление: после любого неудачного перетаскивания повторно рисуем стол.
    // Карта гарантированно остаётся в исходной колонке и не может зависнуть под указателем.
    selection = null;
    render();
  }
  ...
  function bindEvents() {
    ...
    app.addEventListener("click", async event => {
      ...
      if (button) {
        ...
        else if (action === "autofinish") autoFinish();
    ...
    document.addEventListener("visibilitychange", () => {
      ...
      else recoverAfterPlatformUi();
    });
    window.addEventListener("focus", recoverAfterPlatformUi);
