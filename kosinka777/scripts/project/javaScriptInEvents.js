...
// Версия: GamePush v1.49.14 — OK-only full game reload after confirmed purchase; VK purchase flow unchanged; deck_frost7/deck_amber7.
(async () => {
  "use strict";
  ...
  function scheduleOkPurchaseReload() {
    // Full reload is an OK-only fallback for the platform lifecycle issue observed after checkout.
    // VK must keep its existing working purchase flow untouched.
  ...
  async function handlePlatformPurchaseSuccess(productTag, forcedSkinId = "") {
    ...
    // OK only: once the entitlement has been confirmed and persisted locally, restart the game.
    // This mirrors the real focus/visibility recovery that reliably refreshes GamePush purchases.
