...
class SlitherArena {
  ...
  createDom() {
    ...
    this.root.innerHTML = `
      <style>
        ...
        .sa-skin { min-width:0; min-height:183px; overflow:hidden; padding:9px; border:1px solid rgba(255,255,255,.11); border-radius:18px; background:rgba(255,255,255,.045); text-align:left; transition:transform .15s ease,background .15s ease,border-color .15s ease; }
        ...
        .sa-skin-preview { display:block !important; box-sizing:border-box !important; inline-size:100% !important; width:100% !important; max-inline-size:100% !important; max-width:100% !important; block-size:82px !important; height:82px !important; min-width:0 !important; border-radius:12px; background:rgba(5,11,21,.36); }
        ...
        .sa-skin { display:grid; grid-template-rows:76px minmax(0,1fr); min-height:157px; padding:8px; overflow:hidden; }
        ...
        .sa-skin { display:flex !important; flex-direction:column; justify-content:flex-start; min-height:142px !important; padding:7px !important; overflow:hidden !important; border-radius:17px; }
        ...
        /* FINAL MOBILE FIT: keep the original visual design, but make the whole menu
           one self-contained block. These high-specificity rules intentionally win
           over older phone/landscape experiments that gave the panel a fixed height
           while allowing the action grid to overflow below its border. */
        #slither-arena-root .sa-overlay {
          overflow:hidden !important;
          align-items:center !important;
          justify-content:center !important;
        ...
        #slither-arena-root .sa-menu-panel .sa-menu-actions .sa-controls-btn:last-child:nth-child(odd) {
          grid-column:1 / -1 !important;
        }
        @media (pointer:coarse) and (max-width:700px) {
          #slither-arena-root .sa-overlay > .sa-menu-panel {
            width:min(500px,calc(100% - 12px)) !important;
        ...
        @media (pointer:coarse) and (orientation:landscape) and (max-height:560px) {
          #slither-arena-root .sa-overlay > .sa-menu-panel {
            width:min(660px,calc(100% - 12px)) !important;
            padding:11px 13px !important;
          }
          #slither-arena-root .sa-menu-panel .sa-menu-emblem { display:none !important; }
          #slither-arena-root .sa-menu-panel .sa-menu-hero { display:block !important; }
          #slither-arena-root .sa-menu-panel .sa-title { font-size:29px !important; }
          #slither-arena-root .sa-menu-panel .sa-subtitle { margin:4px 0 0 !important; font-size:10px !important; }
          #slither-arena-root .sa-menu-panel .sa-menu-progress { margin-top:6px !important; gap:6px !important; }
          #slither-arena-root .sa-menu-panel .sa-stat-row { margin-top:6px !important; }
          #slither-arena-root .sa-menu-panel .sa-menu-actions {
  ...
  fitOverlayPanel(renderPreviews = false) {
    ...
    // Reset the previous transform before measuring. All menu content is kept in
    // normal document flow by the final CSS layer below, so the panel's own height
    // always includes its buttons instead of letting them spill out of the frame.
    ...
    // Force layout after the reset, then use the smallest trustworthy viewport.
    // visualViewport is especially important for iOS Safari/VK where browser chrome
    // can reduce the actually visible height without changing 100vh immediately.
    ...
    const visibleWidth = widthCandidates.length ? Math.min(...widthCandidates) : overlayRect.width;
    const visibleHeight = heightCandidates.length ? Math.min(...heightCandidates) : overlayRect.height;
    const inset = this.isTouchInputEnvironment() ? 12 : 20;
    const availableWidth = Math.max(1, visibleWidth - inset * 2);
    const availableHeight = Math.max(1, visibleHeight - inset * 2);

    // offset/scroll sizes are untransformed CSS pixels. The direct-child union is a
    // safety net for WebKit grid/flex edge cases and guarantees that the action grid
    // is part of the measured card height.
    ...
    for (const child of panel.children) {
      ...
      rawHeight = Math.max(rawHeight, rect.bottom - panelRect.top);
    }

    const scale = Math.max(0.1, Math.min(1, availableWidth / rawWidth, availableHeight / rawHeight));
    panel.style.setProperty('--sa-fit-scale', scale.toFixed(4));
    panel.classList.toggle('sa-auto-fit', scale < 0.997);
  }
  ...
  async claimRewardSkin(requestedSkin = null) {
    ...
    if (button) {
      // Do not replace textContent on the whole card: it removes the preview element and
  ...
  async purchaseSkin(skin) {
    ...
    this.showShop(returnPage);
    this.persistProgress().catch((error) => console.warn("[SlitherArena] Failed to persist purchased skin unlock.", error));
  }

  async selectSkin(skin) {
    if (!this.progress.unlockedSkins.includes(skin.id)) return;
    if (this.progress.selectedSkin === skin.id) return;
    this.progress.selectedSkin = skin.id;
    // Apply the selected state immediately in the wardrobe UI instead of waiting
    // for cloud/local persistence, which can be noticeably slow on mobile WebViews.
    this.showShop();
    this.persistProgress().catch((error) => console.warn("[SlitherArena] Failed to persist selected skin.", error));
  }
  ...
  getSkinPreviewSvg(skin, title = "") {
    if (!skin) return "";
    const safeId = String(skin.id || "skin").replace(/[^a-zA-Z0-9_-]/g, "-");
    const primary = skin.primary || "#63e6be";
    const secondary = skin.secondary || "#d3f9d8";
    const accent = skin.accent || secondary;
    const dashPatterns = new Set(["stripe", "bands", "prism", "ember", "tiger", "chevron", "aurora", "spark", "lava", "dragon", "crystal", "nebula"]);
    const accentPatterns = new Set(["scales", "petals", "galaxy", "chevron", "tiger", "spark", "moth", "nebula", "crystal"]);
    const dash = dashPatterns.has(skin.pattern) ? ' stroke-dasharray="13 8"' : "";
    const previewPath = "M31 61 C54 51 67 72 88 56 S124 35 145 45 S172 64 196 48";
    // Decorative dots follow the exact same centerline as the body. Fixed x/y
    // circles looked detached from the snake on curved sections of the preview.
    const accents = accentPatterns.has(skin.pattern)
      ? `<path d="${previewPath}" fill="none" stroke="${accent}" stroke-width="3.4" stroke-linecap="round" stroke-dasharray="0.01 18" opacity=".88"/>`
      : "";
    // Keep the crown centered above the head with only a small overlap at its base.
    // This mirrors the in-game crown placement instead of sitting across the face.
    const crown = skin.pattern === "crown"
      ? `<g transform="translate(202 31)" filter="url(#skin-crown-shadow-${safeId})">
          <path d="M-12 1 L-10 -9 L-4 -4 L0 -16 L5 -4 L11 -10 L12 1 Z" fill="url(#skin-crown-${safeId})" stroke="#fff" stroke-opacity=".42" stroke-width=".9" stroke-linejoin="round"/>
          <rect x="-12" y="0" width="24" height="4" rx="2" fill="#e9b92f"/>
        </g>`
      : "";
    return `<svg class="sa-skin-preview" data-skin-preview="${safeId}" viewBox="0 0 240 96" preserveAspectRatio="xMidYMid slice" role="img" aria-label="${this.escapeHtml(title)}" xmlns="http://www.w3.org/2000/svg">
      <defs>
        <linearGradient id="skin-bg-${safeId}" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stop-color="#0f2138"/><stop offset="1" stop-color="#07111f"/></linearGradient>
        <linearGradient id="skin-head-${safeId}" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stop-color="${secondary}"/><stop offset="1" stop-color="${primary}"/></linearGradient>
        <linearGradient id="skin-crown-${safeId}" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#fff3a3"/><stop offset=".42" stop-color="#ffd43b"/><stop offset="1" stop-color="#d99a16"/></linearGradient>
        <filter id="skin-glow-${safeId}" x="-30%" y="-50%" width="160%" height="200%"><feGaussianBlur stdDeviation="3.2" result="b"/><feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge></filter>
        <filter id="skin-crown-shadow-${safeId}" x="-50%" y="-80%" width="200%" height="220%"><feDropShadow dx="0" dy="1.5" stdDeviation="1.5" flood-color="#000" flood-opacity=".38"/></filter>
      </defs>
      <rect width="240" height="96" rx="12" fill="url(#skin-bg-${safeId})"/>
      <g stroke="rgba(255,255,255,.055)" stroke-width="1"><path d="M0 24H240M0 48H240M0 72H240"/><path d="M30 0V96M60 0V96M90 0V96M120 0V96M150 0V96M180 0V96M210 0V96"/></g>
      <path d="${previewPath}" fill="none" stroke="${primary}" stroke-width="19" stroke-linecap="round" stroke-linejoin="round" filter="url(#skin-glow-${safeId})" opacity=".96"/>
      <path d="${previewPath}" fill="none" stroke="${secondary}" stroke-width="6" stroke-linecap="round" stroke-linejoin="round" opacity=".78"${dash}/>
      ${accents}
      <circle cx="202" cy="46" r="14" fill="url(#skin-head-${safeId})"/>
      ${crown}
      <circle cx="207" cy="41" r="2.2" fill="white"/><circle cx="207" cy="51" r="2.2" fill="white"/>
      <circle cx="207.75" cy="41" r="0.95" fill="#132033"/><circle cx="207.75" cy="51" r="0.95" fill="#132033"/>
    </svg>`;
  }
    ...
    const cards = visible.map((skin) => {
      ...
      return `
        <button type="button" class="sa-skin ${owned ? "" : "locked"} ${selected ? "selected" : ""}" ${action} ${disabled}>
          <span class="sa-skin-preview-wrap">${this.getSkinPreviewSvg(skin, skinTitle)}${statusBadge}</span>
  ...
  renderSkinPreviews() {
    // Wardrobe previews are inline SVG now. This deliberately avoids canvas +
    // transformed/clipped compositing bugs seen in older iOS WebKit builds.
    // Keep the method so existing resize/showOverlay call sites remain harmless.
  }
