...
// Версия: подтверждение и мгновенное применение покупок без перезагрузки + полные подсказки.
(async () => {
  "use strict";
    ...
    async purchase(productTag) {
      try {
        ...
        if (this.kind === "gamepush" && this.gp?.payments?.purchase) {
          ...
          // Some platform wrappers resolve without returning the documented result object.
          // Do not grant optimistically: force-refresh the player's purchases and confirm the tag.
